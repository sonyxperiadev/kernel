...
#define ISLAND_VC_IPC_BLOCK_PHYS_BASE		((VC4_DIRECT_ACCESS_PHYS_ADDR_BASE) + 0x0)
#define ISLAND_VC_IPC_BLOCK_SIZE		SZ_2M
...
{
	WARN_ON(gpu_addr < VC4_DIRECT_ACCESS_PHYS_ADDR_BASE);

	return gpu_addr - (VC4_DIRECT_ACCESS_PHYS_ADDR_BASE) + (ARM_VC4_PHYS_ADDR_BASE);
}
...
static u32 read_and_ack_req_atomic(void)
{
	u32 req = readl((KONA_IPC_NS_VA) + (IPCOPEN_IPCASTATUS_OFFSET));
	writel(req, (KONA_IPC_NS_VA) + (IPCOPEN_IPCACLR_OFFSET));
	return req;
}

static void send_req_atomic(u32 req)
{
