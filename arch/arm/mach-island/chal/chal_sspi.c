...
#define FLD_SET(reg_val, fld_val, shift, mask) (reg_val) &= ~(mask); \
...
#define REG_FIFOTX_PIO_THOLD_ADDR(idx, reg) ((reg) + (idx) * SSPI_FIFOTX_PIO_THOLD_SIZE)
#define REG_FIFORX_PIO_THOLD_ADDR(idx, reg) ((reg) + (idx) * SSPI_FIFORX_PIO_THOLD_SIZE)
 ...
 * 
 */
CHAL_SSPI_HANDLE_t sspi_dev[NUM_CHAL_SSPI_BLOCK];
...
*
****************************************************************************/
CHAL_HANDLE chal_sspi_init(uint32_t baseAddr, int type_full)
{
    CHAL_SSPI_HANDLE_t *pDevice = (CHAL_SSPI_HANDLE_t*)NULL;
    uint32_t i;

    chal_dbgprintf(CDBG_INFO, "+chal_sspi_init\r\n");

   if(!baseAddr) 
    {
        chal_dbgprintf(CDBG_ERRO, "invalid argument\n");
        return (CHAL_HANDLE)pDevice;
    }

    /* Don't re-init a block*/
    for (i = 0; i < NUM_CHAL_SSPI_BLOCK; i++)
    ...
    if (i != NUM_CHAL_SSPI_BLOCK) {
        sspi_dev[i].base = baseAddr;
        if (baseAddr == SSP2_BASE_ADDR)
        {
        	sspi_dev[i].type = SSPI_TYPE_FULL;
        }
        else
        {
        	sspi_dev[i].type = SSPI_TYPE_LITE;
        }
        pDevice = &sspi_dev[i];
    }
    else
    {
        chal_dbgprintf(CDBG_ERRO, "ERROR: chal_sspi_init: bad instance\n");
    } 

    /* Return non-null value*/

    chal_dbgprintf(CDBG_INFO, "-chal_sspi_init(pDevice = x%x\r\n)", pDevice);
...
/****************************************************************************
*
*  Function Name: CHAL_SSPI_STATUS_t chal_sspi_set_type(
*                                  CHAL_HANDLE handle,
*                                  CHAL_SSPI_TYPE_t type)
*
*  Description: Set SSPI type. There two types of SSPI core, Full-version core
*               or Lite-version core.
*
****************************************************************************/
                                      ...
                                      CHAL_SSPI_TYPE_t type)
{
    ...
    for (i = 0; i < NUM_CHAL_SSPI_BLOCK; i++)
        if (handle == &sspi_dev[i]) {
            sspi_dev[i].type = type;
            return(CHAL_SSPI_STATUS_SUCCESS);
        }
    return(CHAL_SSPI_STATUS_ILLEGAL_HANDLE);
}
...
/****************************************************************************
*
*  Function Name: CHAL_SSPI_STATUS_t chal_sspi_set_frame(
*                                  CHAL_HANDLE handle,
*                                  uint32_t *frm_bitmap,
...
{
    CHAL_SSPI_HANDLE_t *pDevice = (CHAL_SSPI_HANDLE_t *)handle;
    uint32_t val=0, frm_idx=0, frm_idx0=0xFFFFFFFF, mode;
    ...
    if(pDevice->type == SSPI_TYPE_FULL)
        *frm_bitmap &= (1 << MAX_SSPI_FULL_FRAMES) - 1;
    else
        *frm_bitmap &= (1 << MAX_SSPI_LITE_FRAMES) - 1;
    ...
    case SSPI_PROT_I2C:
        ...
        *frm_bitmap &= ~(1<<frm_idx++);
        ...
        val = (2 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |
              (71 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
        ...
        /* RX_WORD_LENGTH: 9, RX_START: 6 */
        val = (word_len << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
        ...
        *frm_bitmap = 1 << frm_idx0;
         ...
         *  TX_EXTENDED_RIGHT 0x0, TX_EXTENDED_LEFT 0x0
         */
        val = ((word_len * 8) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |
              ((word_len * 8) << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_START_SHIFT) |
              (((word_len * 8)+8) << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_END_SHIFT) |
              0 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET), 
                         val);
    ...
    case SSPI_PROT_I2S_MODE1:
        ...
        /*
         *  TXOEN_START 0x0, TXOEN_END 0x00, TX_WORD_LENGTH 0xf, TX_START 0x0, 
         *  TX_EXTENDED_RIGHT 0x0, TX_EXTENDED_LEFT 0x0
         */
        val = (word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
        ...
        val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
              (1 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_RX_DEF_OFFSET), 
                         val);
        *frm_bitmap = 1 << frm_idx;
...
/* -da-                   SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_SCLK_ACTIVE_EDGE_MASK |*/
        ...
        if(!mode) {
            ...
            val = (0xF << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |
                ((((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) + 3)
                        << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
                  (0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_RATIO_SHIFT);
            CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                             pDevice->base + SSPIL_FRAME0_SCLK_DEF_OFFSET),
        ...
        /*
         *  TXOEN_START 0x0, TXOEN_END 0x00, TX_WORD_LENGTH 0xf, TX_START 0x2, 
         *  TX_EXTENDED_RIGHT 0x0, TX_EXTENDED_LEFT 0x0
         */
        val = (2 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |
              ((word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT);
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                         pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET),
                         val);
        ...
        val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
              (3 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                         pDevice->base + SSPIL_FRAME0_RX_DEF_OFFSET), 
                         val);
        *frm_bitmap = 1 << frm_idx;
    ...
    case SSPI_PROT_MONO_25B_PCM:
        ...
        if(!mode) {
            /*
             *  TXOEN_IDLE_VALUE: 0, TX_IDLE_VALUE: 0, CS_IDLE_VALUE: 0, 
             *  CS_SIZE: 32, CS_ACTIVE: 2
             */
            val = (((word_len + ext_bits) << 1) << 
                   SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_SIZE_SHIFT) |
                  2 << SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_ACTIVE_SHIFT;
        }
        else {
            /*
             *  TXOEN_IDLE_VALUE: 0, TX_IDLE_VALUE: 0, CS_IDLE_VALUE: 0, 
        ...
        if(!mode) {
            ...
            /*
             *  Since 2 clk cycles are required to rx/tx one bit of data, 
             *  TXOEN_END is set to (word_len << 1)+2 = word_len * 2+2
             */
            val = (2 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |
                (2 << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_START_SHIFT) |
                (((word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT)+2) <<
    ...
    case SSPI_PROT_STEREO_25B_PCM:
        ...
        if(!mode) {
            ...
            /*
             *  TXOEN_IDLE_VALUE: 0, TX_IDLE_VALUE: 0, CS_IDLE_VALUE: 0, 
             *  CS_SIZE: (16 + ext_bits)*2, CS_ACTIVE: 0, SCLK_ACTIVE_EDGE: 0
             */
            val = ((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) << 
                   SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_SIZE_SHIFT;
        }
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                         ...
                         val);
        if(!mode) {
            /* SCLK_START: 0, SCLK_END: (16 + ext_bits)*2, SCLK_RATIO: 1:2*/
            val = (((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
                        SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
                  (0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_RATIO_SHIFT);
            CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
        ...
        if(!mode) {
             ...
             *  TX_EXTENDED_RIGHT 0x0, TX_EXTENDED_LEFT 0x0
             */
            val = 1 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT |
                  (word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
        }
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET), 
    ...
    case SSPI_PROT_4CHAN_16B_TDM_PCM:
        ...
        *frm_bitmap &= ~(1<<frm_idx++);
        ...
        if(!mode) {
            ...
            /*
             *  TXOEN_IDLE_VALUE: 0, TX_IDLE_VALUE: 0, CS_IDLE_VALUE: 0, 
             *  CS_SIZE: 64, CS_ACTIVE: 0, SCLK_ACTIVE_EDGE: 0
             */
            val = (word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT) << 
                   SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_SIZE_SHIFT;
        }
         ...
         *  TXOEN_IDLE_VALUE: 0, TX_IDLE_VALUE: 0, CS_IDLE_VALUE: 0, 
         *  CS_SIZE: (word_len + ext_bits)*2, CS_ACTIVE: 2
         */
        if(!mode) {
            val = (((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) << 
                   SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_SIZE_SHIFT) |
            ...
            /*
             *  TXOEN_IDLE_VALUE: 0, TX_IDLE_VALUE: 0, CS_IDLE_VALUE: 0, 
             *  CS_SIZE: (word_len + ext_bits)*2, CS_ACTIVE: 0, SCLK_ACTIVE_EDGE: 0
             */
            val = ((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) << 
                   SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_SIZE_SHIFT;
        }
...
/****************************************************************************
*
*  Function Name: CHAL_SSPI_STATUS_t chal_sspi_set_frame_new(
*                                  CHAL_HANDLE handle,
*                                  uint32_t *frm_bitmap,
*                                  CHAL_SSPI_PROT_t prot,
*                                  uint32_t word_len,
...
*
****************************************************************************/
CHAL_SSPI_STATUS_t chal_sspi_set_frame_new(CHAL_HANDLE handle, 
                                       uint32_t *frm_bitmap,
                                       CHAL_SSPI_PROT_t prot,
                                       uint32_t word_len,
                                       uint32_t ext_bits,
                                       chal_sspi_frame_def_t* frameDef)
{
    CHAL_SSPI_HANDLE_t *pDevice = (CHAL_SSPI_HANDLE_t *)handle;
    uint32_t val=0, frm_idx=0, frm_idx0=0xFFFFFFFF, mode;
    ...
    case SSPI_PROT_SPI_MODE0:
        ...
        val = (((word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT)+1) <<
                    SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_SIZE_SHIFT) |
              (0 << SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_ACTIVE_SHIFT) |
              SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_CS_IDLE_VALUE_MASK;
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                             pDevice->base + SSPIL_FRAME0_CS_IDLE_DEF_OFFSET),
                         val);
        ...
        /* SCLK_START 0x1, SCLK_END 0x10, FIRST_ACTIVE_EDGE 0x1, SCLK_RATIO 1:2*/
//        val = (0x1 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  // JU orig
//        val = (0xF << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  
	val = (frameDef->frame_sclk_start << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  
//              ((word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
              ((word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
                    SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
              (0x1 << SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_SCLK_ACTIVE_EDGE_SHIFT);
         ...
         */
        val = (word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
//        val = val | (0 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT); // JU added
//        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_START_SHIFT); // JU added
//        val = val | (8 << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_END_SHIFT); // JU added
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET),
                         val);
        ...
        /* RX_WORD_LENGTH , 0x7, RX_START 0x1*/
         val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
              (0x1 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); // JU orig
//              (0x2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
    ...
    case SSPI_PROT_SPI_MODE1: /* temporarily use for 8 bit mode */
        while(!(*frm_bitmap & (1 << frm_idx)))
            frm_idx++;
        /*
         *  Default frame setup for SPI_MODE0: 
        ...
        /* SCLK_START 0x1, SCLK_END 0x10, FIRST_ACTIVE_EDGE 0x1, SCLK_RATIO 1:2*/
        val = (0x1 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  // JU orig
//        val = (0x0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  
//              ((word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
              ((word_len * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
                    SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
              (0x1 << SSPIL_FRAME0_CS_IDLE_DEF_FRAME0_SCLK_ACTIVE_EDGE_SHIFT);
...
//              (0x2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
        ...
        *frm_bitmap = 1 << frm_idx;
        break;
//    case SSPI_PROT_SPI_MODE2: /* temporarily use for 8 bit mode */
//    	        while(!(*frm_bitmap & (1 << frm_idx)))
//            frm_idx++;
//        /*
//         *  Default frame setup for SPI_MODE0: 
...
//         */
//        val = (word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
////        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT); // JU added
////        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_START_SHIFT); // JU added
//        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
//                            pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET),
//                         val);
...
//#if (CFG_GLOBAL_CHIP != FPGA11140)    
        ...
        /* SCLK_START: -1, SCLK_END: 33, SCLK_RATIO: 1:2*/
//        val = (SSPI_SCLK_CYCLE_MINUS1 << 	regular
//        val = (0 << 				interleave
	val = (frameDef->frame_sclk_start <<
                    SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |
              ((((word_len + ext_bits) << 1) + 1) << 
               SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
              (0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_RATIO_SHIFT);
         ...
         */
        val = (word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
//        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT);  // regular 
//        val = val | (0 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT); // interleave loopback
	val = val | (frameDef->frame_tx_start << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT);
        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_START_SHIFT);
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET),
                         val);
	...
	if (!mode)
	{
        	/* RX_WORD_LENGTH: 16, RX_START: 1*/
        	val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
        		(frameDef->frame_rx_start_master << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
//              (2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); regular
//              (1 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); interleave
	}
	else
	{
		/* slave */
		val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
        		(frameDef->frame_rx_start_slave << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
	}
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_RX_DEF_OFFSET), 
                         val);
	...
	while(!(*frm_bitmap & (1 << frm_idx)))
         ...
         */
        val = (word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT;
        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT);
        val = val | (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_START_SHIFT);
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET),
                         val);
        ...
        /* RX_WORD_LENGTH: 16, RX_START: 1*/
        val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
              (2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
//              (1 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); JU orig
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                            pDevice->base + SSPIL_FRAME0_RX_DEF_OFFSET), 
                         val);
        *frm_bitmap = 1 << frm_idx;
...
//        if(!mode) {
            /* SCLK_START: -1, SCLK_END: 35, SCLK_RATIO: 1:2*/
//            val = (0xF << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  // interleave
//            val = (0x0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |  // regular
            val = (frameDef->frame_sclk_start << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |
                ((((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) + 3)
                        << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
                  (0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_RATIO_SHIFT);
...
//        }
         ...
         *  TX_EXTENDED_RIGHT 0x0, TX_EXTENDED_LEFT 0x0
         */
//orig JU        val = (2 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |
//        val = (1 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |  // regular
//        val = (0 << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |  // interleave loopback
	val = (frameDef->frame_tx_start << SSPIL_FRAME0_TX_DEF_FRAME0_TX_START_SHIFT) |
        ((word_len-1) << SSPIL_FRAME0_TX_DEF_FRAME0_TX_WORD_LENGTH_SHIFT);
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                         pDevice->base + SSPIL_FRAME0_TX_DEF_OFFSET),
                         val);
...
//              (1 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);  // interleave
//		(2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);  // regular
//              (3 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); // JU orig
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                         pDevice->base + SSPIL_FRAME0_RX_DEF_OFFSET), 
                         val);
        *frm_bitmap = 1 << frm_idx;
...
//            val = (((word_len + ext_bits + 2) << 1) <<
        ...
        if(!mode) {
            /* SCLK_START: 0, SCLK_END: 35, SCLK_RATIO: 1:2*/
              val = (((word_len + ext_bits) * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
//            val = (((word_len + ext_bits + 2) * SSPI_MASTER_CLK_CYCLE_PER_BIT) <<
                   SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_END_SHIFT) |
                   (0x0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_START_SHIFT) |
                  (0 << SSPIL_FRAME0_SCLK_DEF_FRAME0_SCLK_RATIO_SHIFT);
            CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                                pDevice->base + SSPIL_FRAME0_SCLK_DEF_OFFSET),
                             val);
...
//                        SSPIL_FRAME0_TX_DEF_FRAME0_TXOEN_END_SHIFT) |
        ...
        if(!mode) {
            ...
            val = ((word_len-1) << 
                        SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
//                (3 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
//            	  (4 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
		(frameDef->frame_rx_start_master << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
        }
        else {
            val = ((word_len-1) <<
                        SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
//JU                  (3 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
            	  //(2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
            	  (frameDef->frame_rx_start_slave << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
        }
        CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
...
//                  (3 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
		(frameDef->frame_rx_start_master << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
            CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                ...
                val = ((word_len-1) << SSPIL_FRAME0_RX_DEF_FRAME0_RX_WORD_LENGTH_SHIFT) |
//                  (1 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); // multi block
//                  (2 << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT); // self loop
		(frameDef->frame_rx_start_slave << SSPIL_FRAME0_RX_DEF_FRAME0_RX_START_SHIFT);
            CHAL_REG_WRITE32(REG_FRAME_ADDR(frm_idx, 
                                pDevice->base + SSPIL_FRAME0_RX_DEF_OFFSET), 
                             val);
                                         ...
                                         CHAL_SSPI_PATT_TYPE_t type)
{
    ...
    if(pDevice->type == SSPI_TYPE_FULL) {
        ...
        if(patt_idx >= MAX_SSPI_LITE_PATTERNS) {
            return(CHAL_SSPI_STATUS_ILLEGAL_INDEX);
        }
    }
    ...
    case CHAL_SSPI_PATT_TYPE_I2C_STD_STOP:
        ...
        CHAL_REG_WRITE32(REG_PAT_CTL_ADDR(patt_idx,
                         ...
                         val);
        CHAL_REG_WRITE32( 
                        REG_PAT_ADDR(patt_idx,
                            pDevice->base + SSPIL_PATTERN_0_CS_OFFSET), 
                        SSPIL_PATTERN_0_CS_PATTERN0_CS_MASK);
        CHAL_REG_WRITE32(REG_PAT_ADDR(patt_idx, 
                            pDevice->base + SSPIL_PATTERN_0_SCLK_OFFSET), 
                         0xFC << SSPIL_PATTERN_0_SCLK_PATTERN0_SCLK_SHIFT);
        CHAL_REG_WRITE32(REG_PAT_ADDR(patt_idx, 
                            pDevice->base + SSPIL_PATTERN_0_TX_OFFSET), 
                                            ...
                                            CHAL_SSPI_PROT_t prot)
{
    ...
    if(mode) {
         ...
         */
        CHAL_REG_WRITE32(pDevice->base + SSPIL_STATE_MACHINE_CONTROL_OFFSET,
    ...
    case SSPI_PROT_SPI_MODE0:
         ...
         */
        CHAL_REG_WRITE32( 
                         pDevice->base + SSPIL_STATE_MACHINE_CONTROL_OFFSET,
                        (SSPIL_STATE_MACHINE_CONTROL_MASTER_CS0_IDLE_MASK | 
    ...
    case SSPI_PROT_4CHAN_16B_TDM_PCM:
         ...
         */
        CHAL_REG_WRITE32( 
                        pDevice->base + SSPIL_STATE_MACHINE_CONTROL_OFFSET, 0);
        break;
                                          ...
                                          chal_sspi_seq_conf_t *seq_conf)
{
    ...
    if(pDevice->type == SSPI_TYPE_FULL) {
        if(seq_idx >= MAX_SSPI_FULL_SEQUENCES) {
            return(CHAL_SSPI_STATUS_ILLEGAL_INDEX);
        }
    }
    else {
        if(seq_idx >= MAX_SSPI_LITE_SEQUENCES) {
            return(CHAL_SSPI_STATUS_ILLEGAL_INDEX);
        }
    }
    ...
    val = (seq_conf->opcode << SSPIL_SEQUENCE_0_SEQ0_OPCODE_SHIFT) |
          ...
          (seq_conf->tx_enable << SSPIL_SEQUENCE_0_SEQ0_TRANSMIT_SHIFT) |
                                      ...
                                      chal_sspi_task_conf_t *tk_conf)
{
    ...
    if(pDevice->type == SSPI_TYPE_FULL) {
        if(task_idx >= MAX_SSPI_FULL_TASKS) {
            return(CHAL_SSPI_STATUS_ILLEGAL_INDEX);
        }
    }
    else {
        if(task_idx >= MAX_SSPI_LITE_TASKS) {
            return(CHAL_SSPI_STATUS_ILLEGAL_INDEX);
        }
    }
    ...
    lsb_val |= (tk_conf->loop_cnt << SSPIL_TASK0_DESC_LSB_TASK0_LOOP_SHIFT) |
               (tk_conf->seq_ptr <<
                SSPIL_TASK0_DESC_LSB_TASK0_SEQ_POINTER_SHIFT) |
               (tk_conf->init_cond_mask <<
                SSPIL_TASK0_DESC_LSB_TASK0_INITIAL_CONDITION_SHIFT);
    CHAL_REG_WRITE32(
                    REG_TASK_ADDR(task_idx,
    ...
    CHAL_REG_WRITE32(
                    REG_TASK_ADDR(task_idx,
                        pDevice->base + SSPIL_TASK0_DESC_MSB_OFFSET),
                    msb_val);
                                            ...
                                            uint32_t num_chan)
{
    ...
    if(word_len > CHAL_SSPI_MAX_WORD_LEN)
        return CHAL_SSPI_STATUS_ILLEGAL_PARA;
    
    if(num_chan > CHAL_SSPI_MAX_CHANNELS)
        return CHAL_SSPI_STATUS_ILLEGAL_PARA;

    switch(trig)
    {
    case SSPI_CAPH_CLK_TRIG_4kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (4000 * word_len * num_chan);
        break;
        
    case SSPI_CAPH_CLK_TRIG_8kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (8000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_12kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (12000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_16kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (16000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_24kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (24000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_32kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (32000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_48kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (48000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_96kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (96000 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_44DOT1kHz:
        /*
        ...
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE /
                    (44100 * word_len * num_chan) - 1;
        break;

    case SSPI_CAPH_CLK_TRIG_22DOT05kHz:
        num_cycle = CHAL_SSPI_CAPH_CLOCK_RATE / (22050 * word_len * num_chan);
        break;

    case SSPI_CAPH_CLK_TRIG_11DOT025kHz:
                                           ...
                                           uint32_t *fifo_size)
{
    ...
    switch(fifo_id) {
    case SSPI_FIFO_ID_TX0:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_TX_0_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_TX_0_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_TX1:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_TX_1_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_TX_1_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_TX2:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_TX_2_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_TX_2_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_TX3:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_TX_3_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_TX_3_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_RX0:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_RX_0_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_RX_0_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_RX1:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_RX_1_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_RX_1_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_RX2:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_RX_2_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_RX_2_SIZE_MASK);
        break;

    case SSPI_FIFO_ID_RX3:
        *fifo_size = FLD_GET(val, SSPIL_FIFO_SHARING_FIFO_RX_3_SIZE_SHIFT, 
                             SSPIL_FIFO_SHARING_FIFO_RX_3_SIZE_MASK);
        break;
                                                ...
                                                uint32_t *fifo_thres) 
{
    ...
    if(fifo_id >= SSPI_FIFO_ID_TX0) {
        val = CHAL_REG_READ32(REG_FIFOTX_THOLD_ADDR(fifo_id-SSPI_FIFO_ID_TX0, 
                              pDevice->base + SSPIL_FIFOTX_0_THRESHOLD_OFFSET));
        *fifo_thres = FLD_GET(val, 
                              SSPIL_FIFOTX_0_THRESHOLD_FIFOTX0_TRESHOLD_SHIFT, 
                              SSPIL_FIFOTX_0_THRESHOLD_FIFOTX0_TRESHOLD_MASK);
    }
    else {
                                        ...
                                        CHAL_SSPI_FIFO_ID_t fifo_id)
{
    ...
    if(fifo_id >= SSPI_FIFO_ID_TX0) {
        reg = REG_FIFOTX_CTL_ADDR(fifo_id-SSPI_FIFO_ID_TX0, 
                                pDevice->base + SSPIL_FIFOTX_0_CONTROL_OFFSET);
        val = CHAL_REG_READ32(reg) | SSPIL_FIFOTX_0_CONTROL_FIFOTX0_RESET_MASK;
        CHAL_REG_WRITE32(reg, val);
        /* poll until reset bit goes back to 0*/
        
        while(CHAL_REG_READ32(reg) & SSPIL_FIFOTX_0_CONTROL_FIFOTX0_RESET_MASK)
            ;
//        while( val )
//        {
//            val = CHAL_REG_READ32(reg);
//            val |= SSPIL_FIFOTX_0_CONTROL_FIFOTX0_RESET_MASK;
//        }
    }
    else {
        reg = REG_FIFORX_CTL_ADDR(fifo_id, 
                                pDevice->base + SSPIL_FIFORX_0_CONTROL_OFFSET);
        val = CHAL_REG_READ32(reg) | SSPIL_FIFORX_0_CONTROL_FIFORX0_RESET_MASK;
        CHAL_REG_WRITE32(reg, val);
        /* poll until reset bit goes back to 0*/
        while(CHAL_REG_READ32(reg) & SSPIL_FIFORX_0_CONTROL_FIFORX0_RESET_MASK)
            ;
    }        
                                           ...
                                           uint16_t *fifo_level)
{
    ...
    if(fifo_id >= SSPI_FIFO_ID_TX0) {
        ...
        *fifo_level = FLD_GET(val,
                             SSPIL_FIFOTX_0_CONTROL_FIFOTX0_LEVEL_SHIFT, 
                                        ...
                                        
{
    ...
    if(fifo_id >= SSPI_FIFO_ID_TX0) {
        reg = REG_FIFOTX_PIO_THOLD_ADDR(fifo_id-SSPI_FIFO_ID_TX0, 
                            pDevice->base + SSPIL_FIFOTX_0_PIO_THRESHOLD_OFFSET);
        ...
        reg = REG_FIFORX_PIO_THOLD_ADDR(fifo_id-SSPI_FIFO_ID_RX0, 
                            pDevice->base + SSPIL_FIFORX_0_PIO_THRESHOLD_OFFSET);
                                        ...
                                        uint32_t off_on)
{
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_RX0:
        ...
        val = CHAL_REG_READ32(pDevice->base + SSPIL_DMA_RX0_CONTROL_OFFSET);
        FLD_SET(val, off_on, SSPIL_DMA_RX0_CONTROL_DMA_RX0_ENABLE_SHIFT,
                SSPIL_DMA_RX0_CONTROL_DMA_RX0_ENABLE_MASK);
        FLD_SET(val, fifo_id, SSPIL_DMA_RX0_CONTROL_DMA_RX0_LINK_FIFO_SHIFT,
                SSPIL_DMA_RX0_CONTROL_DMA_RX0_LINK_FIFO_MASK);
        CHAL_REG_WRITE32(pDevice->base + SSPIL_DMA_RX0_CONTROL_OFFSET, val);
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_RX1:
        ...
        val = CHAL_REG_READ32(pDevice->base + SSPIL_DMA_RX1_CONTROL_OFFSET);
        FLD_SET(val, off_on, SSPIL_DMA_RX1_CONTROL_DMA_RX1_ENABLE_SHIFT,
                SSPIL_DMA_RX1_CONTROL_DMA_RX1_ENABLE_MASK);
        FLD_SET(val, fifo_id, SSPIL_DMA_RX1_CONTROL_DMA_RX1_LINK_FIFO_SHIFT,
                SSPIL_DMA_RX1_CONTROL_DMA_RX1_LINK_FIFO_MASK);
        CHAL_REG_WRITE32(pDevice->base + SSPIL_DMA_RX1_CONTROL_OFFSET, val);
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_TX0:
        ...
        val = CHAL_REG_READ32(pDevice->base + SSPIL_DMA_TX0_CONTROL_OFFSET);
        FLD_SET(val, off_on, SSPIL_DMA_TX0_CONTROL_DMA_TX0_ENABLE_SHIFT,
                SSPIL_DMA_TX0_CONTROL_DMA_TX0_ENABLE_MASK);
        FLD_SET(val, fifo_id, SSPIL_DMA_TX0_CONTROL_DMA_TX0_LINK_FIFO_SHIFT,
                SSPIL_DMA_TX0_CONTROL_DMA_TX0_LINK_FIFO_MASK);
        CHAL_REG_WRITE32(pDevice->base + SSPIL_DMA_TX0_CONTROL_OFFSET, val);
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_TX1:
        ...
        fifo_id -= SSPI_FIFO_ID_TX0;
        val = CHAL_REG_READ32(pDevice->base + SSPIL_DMA_TX1_CONTROL_OFFSET);
        FLD_SET(val, off_on, SSPIL_DMA_TX1_CONTROL_DMA_TX1_ENABLE_SHIFT,
                SSPIL_DMA_TX1_CONTROL_DMA_TX1_ENABLE_MASK);
        FLD_SET(val, fifo_id, SSPIL_DMA_TX1_CONTROL_DMA_TX1_LINK_FIFO_SHIFT,
                SSPIL_DMA_TX1_CONTROL_DMA_TX1_LINK_FIFO_MASK);
        CHAL_REG_WRITE32(pDevice->base + SSPIL_DMA_TX1_CONTROL_OFFSET, val);
                                        ...
                                        uint32_t *off_on)
{
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_RX0:
        ...
        *(uint32_t *)fifo_id = FLD_GET(val,
                           ...
                           SSPIL_DMA_RX0_CONTROL_DMA_RX0_LINK_FIFO_MASK);
        *off_on = FLD_GET(val, SSPIL_DMA_RX0_CONTROL_DMA_RX0_ENABLE_SHIFT, 
                           SSPIL_DMA_RX0_CONTROL_DMA_RX0_ENABLE_MASK);
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_RX1:
        ...
        *(uint32_t *)fifo_id = FLD_GET(val,
                           ...
                           SSPIL_DMA_RX1_CONTROL_DMA_RX1_LINK_FIFO_MASK);
        *off_on = FLD_GET(val, SSPIL_DMA_RX1_CONTROL_DMA_RX1_ENABLE_SHIFT, 
                           SSPIL_DMA_RX1_CONTROL_DMA_RX1_ENABLE_MASK);
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_TX0:
        ...
        *(uint32_t *)fifo_id = FLD_GET(val,
                           ...
                           SSPIL_DMA_TX0_CONTROL_DMA_TX0_LINK_FIFO_MASK) +
                           SSPI_FIFO_ID_TX0;
        *off_on = FLD_GET(val, SSPIL_DMA_TX0_CONTROL_DMA_TX0_ENABLE_SHIFT, 
                           SSPIL_DMA_TX0_CONTROL_DMA_TX0_ENABLE_MASK);
    ...
    case SSPI_DMA_CHAN_SEL_CHAN_TX1:
        ...
        *(uint32_t *)fifo_id = FLD_GET(val, 
                           ...
                           SSPIL_DMA_TX1_CONTROL_DMA_TX1_LINK_FIFO_MASK) +
                           SSPI_FIFO_ID_TX0;
        *off_on = FLD_GET(val, SSPIL_DMA_TX1_CONTROL_DMA_TX1_ENABLE_SHIFT, 
                           SSPIL_DMA_TX1_CONTROL_DMA_TX1_ENABLE_MASK);
                            ...
                            uint32_t size)
{
    ...
    uint32_t entbase = REG_FIFO_ENTRYRX_ADDR(fifo_id, 
                          pDevice->base + SSPIL_FIFO_ENTRY0RX_OFFSET);
    ...
    case SSPI_PROT_4CHAN_16B_TDM_PCM:
        ...
        case 2:
            ...
            while(((CHAL_REG_READ32(ctlbase) & 
                 SSPIL_FIFORX_0_CONTROL_FIFORX0_LEVEL_MASK) >> 
                 SSPIL_FIFORX_0_CONTROL_FIFORX0_LEVEL_SHIFT) < 2)
                ;
                             ...
                             uint32_t size)
{
    ...
    ctlbase = REG_FIFOTX_CTL_ADDR(fifo_id, 
                                  pDevice->base + SSPIL_FIFOTX_0_CONTROL_OFFSET);
    entbase = REG_FIFO_ENTRYTX_ADDR(fifo_id, 
                            ...
                            uint32_t data)
{
    ...
    uint32_t entbase = REG_FIFO_ENTRYTX_ADDR(fifo_id, 
                          pDevice->base + SSPIL_FIFO_ENTRY0TX_OFFSET);
                                         ...
                                         uint32_t intr_mask)
{
    ...
    CHAL_REG_WRITE32(pDevice->base + SSPIL_INTERRUPT_ENABLE_OFFSET, intr_mask);
    return(CHAL_SSPI_STATUS_SUCCESS);
}
                                         ...
                                         uint32_t *intr_mask)
{
    ...
    *intr_mask=CHAL_REG_READ32(
                        pDevice->base + SSPIL_INTERRUPT_ENABLE_OFFSET);
    return(CHAL_SSPI_STATUS_SUCCESS);
}
                                             ...
                                             uint32_t *det_status)
{
    ...
    *status = CHAL_REG_READ32(pDevice->base + SSPIL_INTERRUPT_STATUS_OFFSET);
    if(det_status)
                                        ...
                                        uint32_t det_intr_mask)
{
    ...
    if(det_intr_mask)
        CHAL_REG_WRITE32(pDevice->base + SSPIL_DETAIL_INTERRUPT_STATUS_OFFSET,
                         det_intr_mask);

    CHAL_REG_WRITE32(pDevice->base + SSPIL_INTERRUPT_STATUS_OFFSET, intr_mask);
                                         ...
                                         uint32_t off_on)
{
    ...
    val = CHAL_REG_READ32(REG_TASK_ADDR(task_idx,  
                              pDevice->base + SSPIL_TASK0_DESC_MSB_OFFSET));
    FLD_SET(val, off_on, 
            SSPIL_TASK0_DESC_MSB_TASK0_ENABLE_SHIFT, 
            SSPIL_TASK0_DESC_MSB_TASK0_ENABLE_MASK);
...
EXPORT_SYMBOL(chal_sspi_init);
EXPORT_SYMBOL(chal_sspi_deinit);
EXPORT_SYMBOL(chal_sspi_rx0_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_rx1_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_rx2_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_rx3_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_tx0_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_tx1_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_tx2_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_tx3_get_dma_port_addr_offset);
EXPORT_SYMBOL(chal_sspi_set_type);
EXPORT_SYMBOL(chal_sspi_set_mode);
EXPORT_SYMBOL(chal_sspi_enable);
EXPORT_SYMBOL(chal_sspi_soft_reset);
EXPORT_SYMBOL(chal_sspi_set_frame);
EXPORT_SYMBOL(chal_sspi_set_pattern);
EXPORT_SYMBOL(chal_sspi_set_idle_state);
EXPORT_SYMBOL(chal_sspi_set_sequence);
EXPORT_SYMBOL(chal_sspi_set_task);
EXPORT_SYMBOL(chal_sspi_set_clk_divider);
EXPORT_SYMBOL(chal_sspi_get_clk_divider);
EXPORT_SYMBOL(chal_sspi_set_clk_src_select);
EXPORT_SYMBOL(chal_sspi_get_clk_src_select);
EXPORT_SYMBOL(chal_sspi_set_caph_clk_gen);
EXPORT_SYMBOL(chal_sspi_get_caph_clk_gen);
EXPORT_SYMBOL(chal_sspi_set_caph_clk);
EXPORT_SYMBOL(chal_sspi_set_fifo_size);
EXPORT_SYMBOL(chal_sspi_get_fifo_size);
EXPORT_SYMBOL(chal_sspi_set_fifo_pack);
EXPORT_SYMBOL(chal_sspi_get_fifo_pack);
EXPORT_SYMBOL(chal_sspi_set_fifo_endian);
EXPORT_SYMBOL(chal_sspi_get_fifo_endian);
EXPORT_SYMBOL(chal_sspi_set_fifo_threshold);
EXPORT_SYMBOL(chal_sspi_get_fifo_threshold);
EXPORT_SYMBOL(chal_sspi_fifo_reset);
EXPORT_SYMBOL(chal_sspi_set_fifo_repeat_count);
EXPORT_SYMBOL(chal_sspi_get_fifo_repeat_count);
EXPORT_SYMBOL(chal_sspi_get_fifo_level);
EXPORT_SYMBOL(chal_sspi_check_fifo_full);
EXPORT_SYMBOL(chal_sspi_check_fifo_empty);
EXPORT_SYMBOL(chal_sspi_set_fifo_pio_threshhold);
EXPORT_SYMBOL(chal_sspi_get_fifo_pio_threshhold);
EXPORT_SYMBOL(chal_sspi_enable_fifo_pio_start_stop_intr);
EXPORT_SYMBOL(chal_sspi_enable_dma);
EXPORT_SYMBOL(chal_sspi_dma_flush);
EXPORT_SYMBOL(chal_sspi_dma_set_burstsize);
EXPORT_SYMBOL(chal_sspi_dma_set_tx_size);
EXPORT_SYMBOL(chal_sspi_get_dma_status);
EXPORT_SYMBOL(chal_sspi_read_data);
EXPORT_SYMBOL(chal_sspi_write_data);
EXPORT_SYMBOL(chal_sspi_read_fifo);
EXPORT_SYMBOL(chal_sspi_write_fifo);
EXPORT_SYMBOL(chal_sspi_enable_intr);
EXPORT_SYMBOL(chal_sspi_enable_error_intr);
EXPORT_SYMBOL(chal_sspi_get_intr_mask);
EXPORT_SYMBOL(chal_sspi_get_error_intr_mask);
EXPORT_SYMBOL(chal_sspi_get_intr_status);
EXPORT_SYMBOL(chal_sspi_clear_intr);
EXPORT_SYMBOL(chal_sspi_enable_task);
EXPORT_SYMBOL(chal_sspi_enable_scheduler);
