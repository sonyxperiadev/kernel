...
#define CHAL_CAPH_CFIFO_PADDR_REG_SIZE  ((CPH_CFIFO_CH2_PADDR_OFFSET-CPH_CFIFO_CH1_PADDR_OFFSET)/sizeof(uint32_t))

/* Control register size for each channel */
#define CHAL_CAPH_CFIFO_CR_REG_SIZE     ((CPH_CFIFO_CPH_CR_2_OFFSET - CPH_CFIFO_CPH_CR_1_OFFSET)/sizeof(uint32_t))

/* CTL register size for each channel */
#define CHAL_CAPH_CFIFO_CTL_REG_SIZE    ((CPH_CFIFO_CPH_CTL_2_OFFSET - CPH_CFIFO_CPH_CTL_1_OFFSET)/sizeof(uint32_t))

/* Time stamp register size for each channel */
#define CHAL_CAPH_CFIFO_TS_REG_SIZE     ((CPH_CFIFO_CPH_CFIFO_TIMESTAMP_CH2_OFFSET-CPH_CFIFO_CPH_CFIFO_TIMESTAMP_CH1_OFFSET)/sizeof(uint32_t))
...
typedef struct
{
    ...
    uint16_t   addr[CHAL_CAPH_CFIFO_MAX_FIFOS];                  /* Size of each FIFO */
    uint16_t   size[CHAL_CAPH_CFIFO_MAX_FIFOS];                  /* Size of each FIFO */
...
/****************************************************************************
*
*  Function Name: CHAL_HANDLE chal_caph_cfifo_init(uint32_t baseAddress)
*
*  Description: init CAPH CFIFO block
*
****************************************************************************/
CHAL_HANDLE chal_caph_cfifo_init(uint32_t baseAddress)
{
    uint8_t  fifo;
...
/****************************************************************************
*
*  Function Name: uint16_t chal_caph_cfifo_get_fifo_addr(CHAL_HANDLE handle,
*			CAPH_CFIFO_e fifo)
*
*  Description: get CAPH CFIFO address
*
****************************************************************************/
uint32_t chal_caph_cfifo_get_fifo_addr(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo)
{
    uint32_t     base = ((chal_caph_cfifo_cb_t*)handle)->base;
    uint8_t      index;
    uint32_t     addr = 0;
    ...
    for(index = 0; index < CHAL_CAPH_CFIFO_MAX_FIFOS; index++)
    {
        if((1UL << index)&fifo)
        {
            addr = (base + index*CHAL_CAPH_CFIFO_PADDR_REG_SIZE*sizeof(uint32_t));
            break;
        }
...
/****************************************************************************
*
*  Function Name: uint16_t chal_caph_cfifo_get_fifo_addr_offset(CHAL_HANDLE handle,
*			CAPH_CFIFO_e fifo)
*
*  Description: get CAPH CFIFO address
*
****************************************************************************/
uint32_t chal_caph_cfifo_get_fifo_addr_offset(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo)
{
    uint8_t      index;
    uint32_t     addr = 0;
    ...
    for(index = 0; index < CHAL_CAPH_CFIFO_MAX_FIFOS; index++)
    {
        if((1UL << index)&fifo)
        {
            addr = (index*CHAL_CAPH_CFIFO_PADDR_REG_SIZE*sizeof(uint32_t));
            break;
        }
    }
...
/****************************************************************************
*
*  Function Name: uint16_t chal_caph_cfifo_get_fifo_addr(CAPH_CFIFO_e fifo)
*			
*
			...
			uint16_t thres2)
{
    ...
    thres = thres/sizeof(uint32_t);
    thres2 = thres2/sizeof(uint32_t);
...
/****************************************************************************
*
*  Function Name: uint32_t chal_caph_cfifo_read_fifo_status(CHAL_HANDLE handle,
*			CAPH_CFIFO_e fifo)
*
*  Description: read fifo status
*
****************************************************************************/
uint32_t chal_caph_cfifo_read_fifo_status(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo)
{
    uint32_t     base = ((chal_caph_cfifo_cb_t*)handle)->base;
    uint8_t      index;
...
*
****************************************************************************/
uint16_t chal_caph_cfifo_write_fifo(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo,
			uint32_t* data,
			uint16_t size,
			_Bool   forceovf)
...
*
****************************************************************************/
uint16_t chal_caph_cfifo_read_fifo(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo,
			uint32_t* data,
			uint16_t size,
			_Bool   forceudf)
...
/****************************************************************************
*
*  Function Name: uint32_t chal_caph_cfifo_read_timestamp(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo)
*
*  Description: read TS for channel 1 to 4
*
****************************************************************************/
uint32_t chal_caph_cfifo_read_timestamp(CHAL_HANDLE handle,
			CAPH_CFIFO_e fifo)
{
    uint32_t     base = ((chal_caph_cfifo_cb_t*)handle)->base;
    uint8_t      index;
