...
// Local Variables
//******************************************************************************
static ChalDmaVc4liteDev_t dmaDev = {0};
    ...
    )
{
    ...
    {
        chal_dprintf(CDBG_ERRO, "%s(): invalid channel parameter (%d)\n", __FUNCTION__, (int)channel);
        return;
    }
    ...
    )
{
    ...
    *intStatus = BRCM_READ_REG_IDX_FIELD_DMA(
        pDmaDev->baseAddr, 
        DMA_CS, 
        (channel*DMA_CHANNEL_ADDR_OFFSET),
        INT
...
{
    ChalDmaVc4liteDev_t *pDmaDev = &dmaDev;
    ChalDmaVc4liteCtrlBlk_t *pCtrlBlkList = (ChalDmaVc4liteCtrlBlk_t*)ctrlBlkList;
    DMA_DEBUG(chal_dprintf(CDBG_INFO, "%s()\n", __FUNCTION__));
    ...
    {
        chal_dprintf(CDBG_ERRO, "%s(): invalid control block address (0x%x)\n", __FUNCTION__, (unsigned int)(ctrlBlkList));
        return CHAL_DMA_VC4LITE_STATUS_INVALID_PARAMETER;
    }
    ...
    )
{
    ...
    chanStatus = BRCM_READ_REG_IDX_FIELD_DMA(
        pDmaDev->baseAddr, 
        DMA_CS, 
        (channel*DMA_CHANNEL_ADDR_OFFSET),
        ACTIVE
    ...
    )
{
    ...
    // check the state
    if (BRCM_READ_REG_IDX_FIELD_DMA(pDmaDev->baseAddr, DMA_CS, (channel*DMA_CHANNEL_ADDR_OFFSET),ERROR)) 
        chanState |= CHAL_DMA_VC4LITE_STATE_ERROR;

    if (BRCM_READ_REG_IDX_FIELD_DMA(pDmaDev->baseAddr, DMA_CS, (channel*DMA_CHANNEL_ADDR_OFFSET),DREQ_STOPS_DMA)) 
        chanState |= CHAL_DMA_VC4LITE_STATE_DREQ_PAUSED;

    if (BRCM_READ_REG_IDX_FIELD_DMA(pDmaDev->baseAddr, DMA_CS, (channel*DMA_CHANNEL_ADDR_OFFSET),PAUSED)) 
        chanState |= CHAL_DMA_VC4LITE_STATE_PAUSED;

    if (BRCM_READ_REG_IDX_FIELD_DMA(pDmaDev->baseAddr, DMA_CS, (channel*DMA_CHANNEL_ADDR_OFFSET),DREQ)) 
        chanState |= CHAL_DMA_VC4LITE_STATE_DREQ;

    if (BRCM_READ_REG_IDX_FIELD_DMA(pDmaDev->baseAddr, DMA_CS, (channel*DMA_CHANNEL_ADDR_OFFSET),END)) 
...
{
    ChalDmaVc4liteCtrlBlk_t*   pCurCtrlBlk  = (ChalDmaVc4liteCtrlBlk_t*)((cUInt32)ctlBlkList+ ctlBlkItemNum*sizeof(ChalDmaVc4liteCtrlBlk_t));
    ChalDmaVc4liteCtrlBlk_t*   pPrvCtrlBlk;
    cUInt32 i;

    // sanity check 
    if ((!ctlBlkList) || ((ctlBlkItemNum+1)*sizeof(ChalDmaVc4liteCtrlBlk_t)>ctlBlkMemSize) || (!curCtrlBlkInfo))
    {
    ...
    // sanity check 
    if ( ((curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_NONE) && (curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_NONE)) ||
         ((curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_NONE) && (curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_DSI0) && 
          (curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_SMI) && (curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_SPI_TX) &&
          (curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_SPI_RX) && (curCtrlBlkInfo->srcDreqID != CHAL_DMA_VC4LITE_DREQ_DSI1)) ||
         ((curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_NONE) && (curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_DSI0) && 
          (curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_SMI) && (curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_SPI_TX) &&
          (curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_SPI_RX) && (curCtrlBlkInfo->dstDreqID != CHAL_DMA_VC4LITE_DREQ_DSI1)) )
    {
        chal_dprintf(CDBG_ERRO, "%s(): invalid DMA parameters\n", __FUNCTION__);
        return CHAL_DMA_VC4LITE_STATUS_INVALID_PARAMETER;        
    }
...
if  (curCtrlBlkInfo->dstDreqID  != CHAL_DMA_VC4LITE_DREQ_NONE)
...
#else
#if defined(_RHEA_)
    pCurCtrlBlk->srcAddr      = ((cUInt32)curCtrlBlkInfo->srcAddr)&0x0FFFFFFF|0x40000000;
    pCurCtrlBlk->dstAddr      = ((cUInt32)curCtrlBlkInfo->dstAddr)&0x0FFFFFFF|0x40000000;
#else
    pCurCtrlBlk->srcAddr      = ((cUInt32)curCtrlBlkInfo->srcAddr)&0x0FFFFFFF|0xC0000000;
    pCurCtrlBlk->dstAddr      = ((cUInt32)curCtrlBlkInfo->dstAddr)&0x0FFFFFFF|0xC0000000;
#endif    
#endif
    ...
    { 
        ...
        pCurCtrlBlk->xferLen    = (curCtrlBlkInfo->xferLength.XferXYLen.yLen << 16) | curCtrlBlkInfo->xferLength.XferXYLen.xLen;
    }
    else
    {
    ...
    if (ctlBlkItemNum !=0)
    {
        pPrvCtrlBlk =  (ChalDmaVc4liteCtrlBlk_t*)((cUInt32)ctlBlkList+ (ctlBlkItemNum-1)*sizeof(ChalDmaVc4liteCtrlBlk_t));
