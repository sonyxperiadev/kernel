...
static void chal_aci_block_ctrl_arg( CHAL_HANDLE handle, CHAL_ACI_block_action_t action, CHAL_ACI_block_id_t id, va_list argp )
{
    ...
    case  CHAL_ACI_BLOCK_ACTION_CONFIGURE_FILTER:
        ...
        case CHAL_ACI_BLOCK_COMP2: {
            ...
            break; }
        case CHAL_ACI_BLOCK_ADC: /* The action applies to both ADC's */
            chal_aci_block_ctrl_arg( handle, action, CHAL_ACI_BLOCK_ADC2, argp );
            /* Fall through */
        case CHAL_ACI_BLOCK_ADC1: {
    ...
    case CHAL_ACI_BLOCK_ACTION_ADC_RANGE: {
        CHAL_ACI_range_config_adc_t range = (CHAL_ACI_range_config_adc_t)va_arg(argp, int);
        CHAL_ASSERT(id == CHAL_ACI_BLOCK_ADC);
        switch (range)
        {
        case CHAL_ACI_BLOCK_ADC_OFF:
            chal_aci_block_ctrl( handle, CHAL_ACI_BLOCK_ACTION_DISABLE, CHAL_ACI_BLOCK_ADC1 );
    ...
    case CHAL_ACI_BLOCK_ACTION_VREF: {
        CHAL_ACI_vref_config_t* vref_config = (CHAL_ACI_vref_config_t*)va_arg(argp, void*);
...
cVoid chal_aci_block_ctrl( CHAL_HANDLE handle, CHAL_ACI_block_action_t action, CHAL_ACI_block_id_t id, ... )
{
    va_list argp;
    va_start(argp, id);
...
*   @todo   <todo list> 
*/
cInt32 chal_aci_block_read( CHAL_HANDLE handle, CHAL_ACI_block_id_t id, CHAL_ACI_block_output_id_t output )
{
    switch (id)
    {
    case CHAL_ACI_BLOCK_COMP1:
