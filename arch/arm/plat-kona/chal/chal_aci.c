...
/* The register write delay should be 3 cycles @ 32768 Hz = 92us */
#if 0
#define HW_ACI_REGISTER_WRITE_SETTLE        CHAL_DELAY_MS(HW_ACI_REGISTER_WRITE_SETTLE_DELAY)
#else
				    ...
				    CHAL_ACI_block_id_t id, va_list argp)
{
	...
	case CHAL_ACI_BLOCK_ACTION_COMP_THRESHOLD:{
			...
			cUInt32 threshold = ((threshold_voltage * 255) / 2500);	// = (voltage * NumberOfSteps) / Reference Voltage;
			switch (id) {
			case CHAL_ACI_BLOCK_COMP:	/* The action applies to both Comperators */
				chal_aci_block_ctrl_arg(handle, action,
							CHAL_ACI_BLOCK_COMP2,
	...
	case CHAL_ACI_BLOCK_ACTION_INTERRUPT_ENABLE:
		...
		case CHAL_ACI_BLOCK_COMP:	/* The action applies to all Comperators */
			mask = ACI_INT_INV_COMP2INT_EN_MASK |
			    ACI_INT_COMP2INT_EN_MASK | ACI_INT_COMP1INT_EN_MASK;
			break;
		case CHAL_ACI_BLOCK_COMP1:
	...
	case CHAL_ACI_BLOCK_ACTION_MIC_BIAS:{
			...
			case CHAL_ACI_MIC_BIAS_ON:
				...
				// Set Bias Voltage
				if (bias_config->voltage == CHAL_ACI_MIC_BIAS_0_45V) {	//disable, bias = 0.45V
#ifdef CONFIG_AUDIOH_REGS
					...
					BRCM_WRITE_REG_FIELD(KONA_AUXMIC_VA,
							     AUXMIC_AUXEN,
							     MICAUX_EN, 0);
				} else if (bias_config->voltage == CHAL_ACI_MIC_BIAS_2_5V) {	//enable, bias = 2.5V
#ifdef CONFIG_AUDIOH_REGS
					BRCM_WRITE_REG_FIELD(KONA_AUDIOH_VA,
							     AUDIOH_AUDIORX_VMIC,
							     AUDIORX_VAUXMIC_CTRL,
	...
	case CHAL_ACI_BLOCK_ACTION_MIC_POWERDOWN_HIZ_IMPEDANCE:{
			...
			if (hiz == TRUE) {	/* Configure Hi-Z (21K) impedance when MIC bias is powered down */
				BRCM_WRITE_REG_FIELD(KONA_ACI_VA, ACI_MIC_BIAS,
						     MIC_AUX_BIAS_GND,
						     ACI_MIC_BIAS_MIC_AUX_BIAS_GND_CMD_IMPEDANCE_CONTROL_HI_Z);
			} else {	/* Configure Lo-Z (GND) impedance when MIC bias is powered down */
				BRCM_WRITE_REG_FIELD(KONA_ACI_VA, ACI_MIC_BIAS,
						     MIC_AUX_BIAS_GND,
						     ACI_MIC_BIAS_MIC_AUX_BIAS_GND_CMD_IMPEDANCE_CONTROL_LO_Z);
			}
			   ...
			   CHAL_ACI_block_output_id_t output)
{
	...
	case CHAL_ACI_BLOCK_ADC:{
			...
			if (output == CHAL_ACI_BLOCK_ADC_RAW) {
				...
				if (value >= (ACI_ADC1_DOUT_ADC1_DOUT_MASK & (~0x3))) {	/* ADC overflow */
					return -1;
				}
			} else {
				value =
				    BRCM_READ_REG(KONA_ACI_VA, ACI_ADC1_DOUTF);
				if (value >= (ACI_ADC1_DOUTF_ADC1_DOUTF_MASK & (~0x3))) {	/* ADC overflow */
					return -1;
				}
			}

			/* Convert to mV */
			if (BRCM_READ_REG_FIELD(KONA_ACI_VA, ACI_ADC_CTRL, ADC_FS_BIG_SMALL_B) == ADC_FS_BIG_SMALL_B_LOW_RANGE) {	/*  ADC full scale: 1150 mV */
				ret = ((1150UL * (value)) / 0x3FC);	/* = Ref Voltage * Reading / Steps */
			} else {	/* ADC full scale: 2300 mV */
				ret = ((2300UL * (value)) / 0x3FC);	/* = Ref Voltage * Reading / Steps */
			}
			return ret;
		}
	default:
