...
/*                            P R O T O T Y P E S                             */
/******************************************************************************/
cUInt8 chal_keypad_calculate_pin_mask(cUInt8 pin);
	...
	)
{
	...
	// The constant 0x077CB531 is a DeBruijn sequence that produces a unique pattern
	// of bits in the high 5 bits for each possible bit position that it is multiplied 
	// against.
	// The usage of (val & -val) isolates only the rightmost '1' bit from val. This
	// ensures that the multiplier only has 1 bit set to '1'.

	return MultiplyDeBruijnBitPosition[(cUInt8)(((val & -val) * 0x077CB531U) >> 27)];
...
cUInt8 chal_keypad_calculate_pin_mask(cUInt8 n)
{
	...
	if ((n>8) || (n<1))		// n should never be bigger than 8 as the hardware supports 8x8 
							// matrix maximum.
	{
		return 0;
	}
...
//
// Parameters:		baseAddr		(in)	hardware configuration of keypad to initialize
// 
// Note: When this function completes, the keypad hardware is configured as requested
//        and key interrupts are enabled.
//  
//***************************************************************************
...
void chal_keypad_set_enable(CHAL_HANDLE handle, Boolean enable)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, ENABLE, ((enable==TRUE)? 1:0) );
}
...
void chal_keypad_set_pullup_mode(CHAL_HANDLE handle, Boolean pullUp)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, MODE, ((pullUp==TRUE)? 1:0) );
}
...
void chal_keypad_set_column_filter(CHAL_HANDLE handle, Boolean enable, CHAL_KEYPAD_DEBOUNCE_TIME_t debounce)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, COLFILTERENABLE, ((enable==TRUE)? 1:0) );
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, COLUMNFILTERTYPE, debounce);
}
...
void chal_keypad_set_status_filter(CHAL_HANDLE handle, Boolean enable, CHAL_KEYPAD_DEBOUNCE_TIME_t debounce)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, STATUSFILTERENABLE, ((enable==TRUE)? 1:0) );
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, STATUSFILTERTYPE, debounce);
}
...
void chal_keypad_set_column_width(CHAL_HANDLE handle, cUInt32 columns)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, COLUMNWIDTH, (columns -1) );
}
...
void chal_keypad_set_row_width(CHAL_HANDLE handle, cUInt32 rows)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, ROWWIDTH, (rows -1) );
}
...
void chal_keypad_set_row_output_control(CHAL_HANDLE handle, cUInt32 rows)
{
	...
	// use rows as output
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPIOR,ROWOCONTRL, rowMask);
}
...
void chal_keypad_set_column_output_control(CHAL_HANDLE handle, cUInt32 columns)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPIOR,COLUMNOCONTRL, columnMask);
}
...
void chal_keypad_set_interrupt_mask(CHAL_HANDLE handle, cUInt32 rows, cUInt32 columns)
{
	...
	pKpdDev->imr0Val = tempReg;		// save the imr0 value as we need it when enabling/disabling interrupts
	...
	pKpdDev->imr1Val = tempReg;		// save the imr1 value as we need it when enabling/disabling interrupts
...
void chal_keypad_swap_row_and_column(CHAL_HANDLE handle, Boolean swap)
{
	...
	BRCM_WRITE_REG_FIELD( pKpdDev->baseAddr, KEYPAD_KPCR, SWAPROWCOLUMN, ((swap==TRUE)? 1:0) );
}
...
// 
//***************************************************************************
void chal_keypad_retrieve_key_event_registers(CHAL_HANDLE handle, CHAL_KEYPAD_REGISTER_SET_t *regState)
{
	CHAL_KEYPAD_DEVICE_t *pKpdDev = (CHAL_KEYPAD_DEVICE_t *) handle;
...
//
// Description: Take a set of keypad registers recorded when an interrupt occurred
//    and decode the key event contained in them.
//
// Parameters:	
													...
													CHAL_KEYPAD_KEY_EVENT_LIST_t keyEvents)
{
	...
	mask = isr;
	if (!pKpdDev->pullUpMode)		// if pull-down mode, the SSR bits logic is negated
	{
		ssr = ~ssr;
	}

	for (i=0;i<bitCount;i++)			// loop through all set bits, as each indicates a key event
	{
		keyEvent = &keyEvents[*numKeyEvents];

		firstbit = mask & (-mask);		// this isolates the rightmost '1' bit from mask
		...
		keyEvent->keyId = bitNumber(firstbit) + (regSet*32);	// raw KeyId is of the form (C*8)+R:
																//    C = column number 
																//    R = row number

		mask &= (~firstbit);			// mask out the bit we just processed the event for
...
// 		of the key events contained in it and add them to the Key Event FIFO.
//
// Parameters:	none
// 
//***************************************************************************
...
//	chal_dprintf( CDBG_INFO, "Key Event Regs: %lx %lx %lx %lx",regState->ssr0,regState->ssr1, regState->isr0,regState->isr1);
...
#define CHAL_INTERRUPT_EVENT_FIFO_LENGTH		4		// max number of interrupt event that can be stored
														// must be a power of 2 for fifo operations to work

#define CHAL_KEYPAD_KEY_ACTION_FIFO_LENGTH      4      	// max number of key event that can be stored
														//     since each interrupt register set can have a 
														//      max of 4 simultaneous events it must be at least 4
														// must be a power of 2 for fifo operations to work

#define MAX_KEY_EVENT_PER_INTERRUPT             4		// max valid key events in one interrupt - hardware defined
...
typedef struct
{
	...
	cUInt32		isr0;
	cUInt32		isr1;
} CHAL_KEYPAD_INTERRUPT_EVENT_t;      // register set data for 1 interrupt event.
...
} CHAL_KEYPAD_INTERRUPT_FIFO_t;			// interrupt event Q - register data from an interrupt to be processed later.
...
} CHAL_KEYPAD_KEY_ACTION_FIFO_t;	// key event Q - key events decoded from interrupt events.

#define FIFO_FULL(fifo)				(((fifo.head - fifo.tail) & (fifo.length-1)) >= fifo.length)
...
/*                            P R O T O T Y P E S                             */
/******************************************************************************/
void chal_keypad_store_event(void);
cUInt8 chal_keypad_calculate_pin_mask(cUInt8 pin);
...
// Description: initialize the keypad csl driver
//
// Parameters:		config		(in)	hardware configuration of keypad to initialize
// 
// Note: When this function completes, the keypad hardware is configured as requested
//        and key interrupts are enabled.
//  
...
//#endif
	...
	imr0Val = tempReg;		// save the imr0 value as we need it when enabling/disabling interrupts
	...
	imr1Val = tempReg;		// save the imr1 value as we need it when enabling/disabling interrupts
...
// 
//***************************************************************************
void chal_keypad_decode_interrupt_event_registers(cUInt32 regSet, cUInt32 bitCount, cUInt32 ssr, cUInt32 isr)
{
	cUInt32 mask, i, firstbit;
	CHAL_KEYPAD_EVENT_t *keyEvent;

	mask = isr;
	if (!keypadConfig.pullUpMode)		// if pull-down mode, the SSR bits logic is negated
	{
		ssr = ~ssr;
	}
	...
	for (i=0;i<bitCount;i++)			// loop through all set bits, as each indicates a key event
	{
		...
		keyEvent->keyId = bitNumber(firstbit) + (regSet*32);	// raw KeyId is of the form 0xCR where:
																//    C = column number 
																//    R = row number
...
void chal_keypad_process_interrupt_events()
{
	...
	if ((!FIFO_EMPTY(interruptEventQ)) && (!FIFO_FULL(keyActionQ)))
	{
		...
		if ((bc1+bc2)<=MAX_KEY_EVENT_PER_INTERRUPT)
		{
			if (bc1)
			{
				chal_keypad_decode_interrupt_event_registers(0, bc1,
															 interruptEvent->ssr0,
															 interruptEvent->isr0);
			}
			...
			if (bc2)
			{
				chal_keypad_decode_interrupt_event_registers(1, bc2,
															 interruptEvent->ssr1,
															 interruptEvent->isr1);
			}
		...
		{
			// appears to be an error (too many bits set in ISR) clear the interrupts to reset and hopefully continue
...
Boolean chal_keypad_get_action_from_Q(CHAL_KEYPAD_EVENT_t *event)
{
	...
	if (!FIFO_EMPTY(keyActionQ))
	{
		...
		event->keyAction = keyActionQ.actionQ[keyActionQ.head].keyAction;
