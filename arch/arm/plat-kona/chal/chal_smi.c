...
//                
//*****************************************************************************
cUInt32 chal_smi_calc_smil_len (
    cUInt32               bytes,        // FIFO in transfer size in bytes
    ...
    )
{
    ...
    switch ( in_col_mode )
    {
        case CHAL_SMI_CM_16B_RGB565 :
            switch ( bus_width )
            {
                case CHAL_SMI_BUSW_08B :  smilL = bytes;        break;
                case CHAL_SMI_BUSW_09B :
                case CHAL_SMI_BUSW_16B :
                case CHAL_SMI_BUSW_18B :  smilL = bytes>>1;     break;
                default:
                    break;
            }
            break;
        case CHAL_SMI_CM_32B_RGB888 :
            switch ( bus_width )
            {
                case CHAL_SMI_BUSW_08B :  smilL = (bytes*3)>>2; break;
                case CHAL_SMI_BUSW_09B :  smilL = bytes>>1;     break;
//              case CHAL_SMI_BUSW_16B :  smilL = (bytes*3)>>2; break;
                case CHAL_SMI_BUSW_16B :  smilL = (bytes*3)>>3; break;
                case CHAL_SMI_BUSW_18B :  smilL = bytes;        break;
                default:
                    break;
            }
            break;
//      case CHAL_SMI_CM_NAND :
        ...
        //    18-bit bus     xxxxxxxx xxxxxx00 00000000 00000000
            switch ( bus_width )
            {
                case CHAL_SMI_BUSW_08B: smilL = bytes;          break;
                case CHAL_SMI_BUSW_09B: smilL = bytes>>1;       break;
                case CHAL_SMI_BUSW_16B: smilL = bytes>>1;       break;
                case CHAL_SMI_BUSW_18B: smilL = bytes>>2;       break;
                default:
                    break;
            }
            break;
    ...
    )
{
    ...
    if ( mode->m68 )                   valueRd |= SMI_DSR0_MODE68_MASK;
    if ( mode->setupFirstTrasferOnly ) valueRd |= SMI_DSR0_FSETUP_MASK;
    
    maskWr |= SMI_DSW0_WFORMAT_MASK | SMI_DSW0_WSWAP_MASK;
    if ( mode->swap )                  valueWr |= SMI_DSW0_WSWAP_MASK;
...
//                
//*****************************************************************************
void chal_smi_setup_rd_timing ( 
    CHAL_HANDLE         handle, 
    cUInt32             bank, 
    pCHAL_SMI_TIMING    timing 
    )
{
    pCHAL_SMI_T         pDev  = (pCHAL_SMI_T) handle;
    cUInt32             mask  = 0;
    ...
    mask =   SMI_DSR0_RDREQ_MASK     // for now not an option, hardcoded to 0
           ...
           | SMI_DSR0_RHOLD_MASK    
           | SMI_DSR0_RPACE_MASK    
           | SMI_DSR0_RSTROBE_MASK  
           | SMI_DSR0_RSETUP_MASK;    
           //SMI_DSR0_MODE68_MASK    // handled from setup
           //SMI_DSR0_FSETUP_MASK    // handled from setup
           
    if ( timing->paceAll ) value |= SMI_DSR0_RPACEALL_MASK;
...
//        "BANK[%02d] ADDR[0x%08X] DATA[0x%08X]\n",  bank, addr, data );
    ...
    BRCM_WRITE_REG ( pDev->baseAddr, 
        SMI_DCS, SMI_DCS_EANBLE_MASK | SMI_DCS_WRITE_MASK | SMI_DCS_START_MASK ); 
}    
...
//
//*****************************************************************************
void chal_smi_fifo_acc ( 
    CHAL_HANDLE     handle,
    pCHAL_FIFO_ACC  fifoMode
    )
{
    pCHAL_SMI_T pDev = (pCHAL_SMI_T) handle;
    cUInt32     mask;
    ...
    mask =  SMI_CS_PXLDAT_MASK  
          ...
          | SMI_CS_PVMODE_MASK  // PV MODE  for now hardcode to 0=DISABLE
          | SMI_CS_PAD_MASK;    // PADDING  for now hardcode to 0=0 BYTE PADDING

    value = 0;
    
    if( fifoMode->isPixelFormat != 0 ) value |= SMI_CS_PXLDAT_MASK;
    if( fifoMode->isWrite       != 0 ) value |= SMI_CS_WRITE_MASK;
    if( fifoMode->isTE          != 0 ) value |= SMI_CS_TEEN_MASK;
