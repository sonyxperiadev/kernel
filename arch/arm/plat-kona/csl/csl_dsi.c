...
static void cslDsi1UpdateTask (void)
{
	...
	for (;;)
	{
		...
		if( updMsg.updReq.cslLcdCb )
		{
			updMsg.updReq.cslLcdCb(res, 
				&updMsg.updReq.cslLcdCbRec );
		} 
		else
		{
			LCD_DBG	( LCD_DBG_ERR_ID, "[CSL DSI] %s: "
	...
	Boolean		isTE  )
{
	...
	if( txPkt.msgLen <= 2 )
	{
		...
		{
		    res	= CSL_LCD_MSG_SIZE; goto exit_err;
		}
	} 
	else	 
	{
		...
		else
		{
			...
			if( chalRes == CHAL_DSI_RX_NO_PKT )
			{
				LCD_DBG(LCD_DBG_ERR_ID,	"[CSL DSI][%d] %s: "
					...
					"No Data Received\n",
					dsiH->bus, __FUNCTION__, txPkt.vc ); 
				res = CSL_LCD_INT_ERR; goto exit_ok;
			}    
			else
			{
				goto exit_ok;	
	...
	CSL_LCD_HANDLE*	    dsiCmVcH  )	
{
	...
	{
		*dsiCmVcH = NULL;
		res = CSL_LCD_INST_COUNT; goto exit_err;
	}
	else
	{
		cmVcH =	&dsiH->chCm[i];
	...
	case LCD_IF_CM_I_RGB888U:	  
		...
		default:
			...
			res = CSL_LCD_COL_MODE;	goto exit_err;
			break;
		}    
		break;
	...
	else
	{
		...
		{ 
			...
			cslDsiWaitForStatAny_Poll( dsiH, 
				CHAL_DSI_STAT_PHY_D0_STOP, NULL, 10 );
			dsiH->ulps = FALSE;
		}
	}
