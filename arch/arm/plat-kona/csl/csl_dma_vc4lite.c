...
DMA_VC4LITE_STATUS_t csl_dma_vc4lite_init(void)
{
	...
	printk(KERN_ERR "the virt addr=0x%08x phy addr=0x%08x acp addr=%08x\n", dmaCtrlBlkList, virt_to_phys(dmaCtrlBlkList), dmaCtrlBlkListPhys);
        ...
        {		
            ...
            pdma->chan[i].chanSema = OSSEMAPHORE_Create(1, OSSUSPEND_PRIORITY);
            if ( !pdma->chan[i].chanSema)
            {
                dprintf(1, "%s: channel semaphore %d creation failed\n", __FUNCTION__, i);
                return DMA_VC4LITE_STATUS_FAILURE;
            }
...
#else
        if( request_irq(BCM_INT_ID_MM_DMA_CHAN1, bcm_vc4l_dma_interrupt, IRQF_DISABLED |
            IRQF_NO_SUSPEND, "BRCM VC4L DMA1", NULL) <0)
        {
    ...
    )
{
    ...
    memcpy(&pdma->chan[chanID].chanInfo, pChanInfo, sizeof(DMA_VC4LITE_CHANNEL_INFO_t));
...
DMA_VC4LITE_STATUS csl_dma_vc4lite_release_channel(DMA_VC4LITE_CHANNEL_t chanID)
{
    ...
    if (pdma->chan[chanID].used) 
    {
        if (chal_dma_vc4lite_reset_channel(pdma->handle, chanID) != CHAL_DMA_VC4LITE_STATUS_SUCCESS)
        {
        ...
        memset(&pdma->chan[chanID].chanInfo, 0, sizeof(DMA_VC4LITE_CHANNEL_INFO_t));
    }
...
DMA_VC4LITE_STATUS csl_dma_vc4lite_start_transfer(DMA_VC4LITE_CHANNEL_t chanID)
{
    ...
    if (chal_dma_vc4lite_enable_int(pdma->handle, chanID) != CHAL_DMA_VC4LITE_STATUS_SUCCESS)
    {
...
DMA_VC4LITE_STATUS csl_dma_vc4lite_stop_transfer(DMA_VC4LITE_CHANNEL_t chanID)
{
    ...
    if (chal_dma_vc4lite_abort_transfer(pdma->handle, chanID) != CHAL_DMA_VC4LITE_STATUS_SUCCESS)
    {
...
{    
    ...
    dmaCtrlBlkInfo.dstDreqID = dma_vc4lite_per_map(pdma->chan[chanID].chanInfo.dstID);    
    ...
    if (chal_dma_vc4lite_build_ctrlblk_list(
           ...
           pdma->chan[chanID].dmaChanCtrlBlkMemSize,
           &dmaCtrlBlkInfo) != CHAL_DMA_VC4LITE_STATUS_SUCCESS)
	{
...
)
{
    ...
    if (pdma->chan[chanID].chanInfo.xferMode == DMA_VC4LITE_XFER_MODE_LINERA)
	{
        dprintf(1, "%s: invalid input parameters\n", __FUNCTION__);
        return DMA_VC4LITE_STATUS_INVALID_INPUT;
	}
        ...
        if (pdma->chan[chanNum].irqStatus != 0)
        {
            if (pdma->chan[chanNum].chanInfo.callback)
            {
                if ((pdma->chan[chanNum].chanState != CHAL_DMA_VC4LITE_STATE_INVALID) && 
                    (pdma->chan[chanNum].chanState | CHAL_DMA_VC4LITE_STATE_END))
                    pdma->chan[chanNum].chanInfo.callback(DMA_VC4LITE_CALLBACK_SUCCESS);
                else
                    pdma->chan[chanNum].chanInfo.callback(DMA_VC4LITE_CALLBACK_FAIL);
            }
...
{   
    ...
    case DMA_VC4LITE_CLIENT_MEMORY:
        perID = CHAL_DMA_VC4LITE_DREQ_NONE; break;

    case DMA_VC4LITE_CLIENT_DSI0:
        perID = CHAL_DMA_VC4LITE_DREQ_DSI0; break;

    case DMA_VC4LITE_CLIENT_DSI1:
        perID = CHAL_DMA_VC4LITE_DREQ_DSI1; break;

    case DMA_VC4LITE_CLIENT_SMI:
        perID = CHAL_DMA_VC4LITE_DREQ_SMI; break;

    case DMA_VC4LITE_CLIENT_SPI_TX:
        perID = CHAL_DMA_VC4LITE_DREQ_SPI_TX; break;

    case DMA_VC4LITE_CLIENT_SPI_RX:
        perID = CHAL_DMA_VC4LITE_DREQ_SPI_RX; break;
