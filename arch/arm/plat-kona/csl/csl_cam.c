...
/******************************************************************************
 Static functions
 *****************************************************************************/
static Int32 csl_cam_set_buffer_params( CSL_CAM_HANDLE cslCamH, CSL_CAM_BUFFER_PTR_st_t buffers);
...
// Check status bits    
    ...
    if ( (chal_val & CHAL_CAM_INT_DATA_END) || (chal_val & CHAL_CAM_INT_DATA_FRAME_END) )
    {
...
// Enable clock    
    ...
    // Enable Clock Select 1
        ...
        #endif
            PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_DIGI_CH1, CLOCK_ON);
            PRM_set_clock_speed (cslCamDrv.prmClientId, RESOURCE_DIGI_CH1, clock_freq);                       
        }
        else
        {
    #if defined(PMUX_INCLUDED)
    ...
    #endif
            PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_DIGI_CH0, CLOCK_ON);
            PRM_set_clock_speed (cslCamDrv.prmClientId, RESOURCE_DIGI_CH0, clock_freq);                       
        }
    }
    else if ( enable == 0 )
    {
    // Disable Dig Clk1
        if (clk_select == 1)
        {
            PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_DIGI_CH1, CLOCK_OFF);
        }
        else
        {
    // Disable Clock Select 0
            PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_DIGI_CH0, CLOCK_OFF);
        }
    }
    ...
    if ( chal_cam_clock(cslCamDrv.chalCamH, (cUInt32)clk_select, (cUInt32)clock_freq, (cBool)enable) != CHAL_OP_OK )
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[cslCamClock][Info] : chal_cam_clock():  ERROR!\n") );
        success |= CSL_CAM_BAD_CLK;
    }
#endif    
    return success;
...
// Set interface pointer
   ...
   // Register Power Management Client Id
        ...
        {
            success |= CSL_CAM_ID;
            CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_init][Error] : PRM_client_register() FAILED \n");
        }  
#endif          
...
Int32 csl_cam_exit( void )
{
    ...
    // De-Register Power Management Client Id
        ...
        {
            success |= CSL_CAM_ID;
            CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_exit][Error] : PRM_client_deregister() FAILED \n");
        }    
#endif        
        cslCamDrv.init = 0;
    } 
...
// need to guard against NULL params being passed
    *cslCamH = NULL;
    ...
    if( intfCfg )
    {
        if ( cslCamDrv.init != 1 )
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] : Not Initiailized \n") );
            return ( CSL_CAM_NOT_INIT );
        }
    // Check for maximum channels already opened
        if (cslCamDrv.use_count == CAM_MAX_HANDLES)
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] : MAX handles allocated! \n") );
    ...
    // If Channel opened, check Interface & AFE port is same
        ...
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] : Channel opened, Interface must be same! \n") );
    ...
    // Get available channel
        ...
        while ( (instance < CAM_MAX_HANDLES) && (camH == NULL) )
        {
            ...
            { 
                ...
                DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam][Info] : csl_cam_open(): Instance=0x%x \n", instance) );
            }
            instance++;
        }
    // Verify channel available
        if (camH == NULL)
        {            
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] : No Channels available! \n") );
    ...
    // If Initial Channel opening on Interface
        ...
        // Update Interface Info
            ...
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam][Info] : csl_cam_open(): intf=0x%x afe=0x%x chan=0x%x input=0x%x time_out=0x%x \n", 
    ...
    // Set Camera CSIx Phy & Clock Registers
            ...
            {
                // CSI0_LP			
                    if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI0, CLOCK_ON) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): FAILED \n") );
                    }
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI0, 0, 0, 4, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): FAILED \n") );
                    }
                // CSI0_AXI			
                    if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI0_AXI, CLOCK_ON) != BRCM_PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): RESOURCE_CSI0_AXI: FAILED \n") );
                    }
                // CSI1_LP			
                    if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI1, CLOCK_ON) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): RESOURCE_CSI1: FAILED \n") );
                    }
                // CSI1_AXI	
                    if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI1_AXI, CLOCK_ON) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): RESOURCE_CSI1_AXI: FAILED \n") );
                    }
                // CSI1_DIV		
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI1, 0, 0, 4, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): FAILED \n") );
                    }
                // CSI1_PHY_DIV		
                    ...
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI1_CAMPIX, 8, 0, 0, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): RESOURCE_CSI1_CAMPIX: FAILED \n") );
                    }
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI1_BYTE1, 8, 0, 0, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): RESOURCE_CSI1_BYTE1: FAILED \n") );
                    }
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI1_BYTE0, 8, 0, 0, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): RESOURCE_CSI1_BYTE0: FAILED \n") );
                    }
            } 
            else 
            {
                ...
                // CSI0_AXI			
                    if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI0_AXI, CLOCK_ON) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): RESOURCE_CSI0_AXI: FAILED \n") );
                    }
                ...
                // CSI0_PHY_DIV		
                    ...
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI0_CAMPIX, 8, 0, 0, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): RESOURCE_CSI0_CAMPIX: FAILED \n") );
                    }
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI0_BYTE1, 8, 0, 0, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): RESOURCE_CSI0_BYTE1: FAILED \n") );
                    }
                    if (PRM_set_clock_divider(cslCamDrv.prmClientId, RESOURCE_CSI0_BYTE0, 8, 0, 0, 0) != PRM_OK)
                    {
                        success |= CSL_CAM_BAD_CLK;
                        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_divider(): RESOURCE_CSI0_BYTE0: FAILED \n") );
                    }
            }
        #endif     
        ...
        // Set Interface
            ...
            chal_cam_intf_st.afe_port = cslCamChalPortAFE(cslCamDrv.intf_cfg.afe_port);
        ...
        /* Register the CSI2/CCP2 interrupt handler */
           ...
           // Clear any irq
            ...
            // Register Isr 
                {
                	int ret = request_irq(cslCamDrv.int_id, cslCamFrameLisr, IRQF_DISABLED,
            				"rhea csl_cam interrupt", (void *)NULL);
                	if (ret < 0) 
                	{
                		printk(KERN_ERR "Unable to register Interrupt for Rhea CSL_CAM interface\n");
                		success |=  CSL_CAM_ERR;
            	    }
            	}
        ...
        // Cfg Lane Enable 
            ...
            chal_cam_param_st.param = (CHAL_CAM_LANE_SELECT_t)(CHAL_CAM_DATA_LANE_0 | CHAL_CAM_CLK_LANE);
        ...
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  chal_cam_xxx: FAILED \n") );
        }
    }
    else
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  params: FAILED \n") );
    }
...
// If all handles closed on this interface, close everything
    if (cslCamDrv.use_count == 0)
    {
        ...
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_close][Error] :  csl_cam_reset(): CSL_CAM_RESET_SWR: FAILED \n") );
        }
        if ( (success |= csl_cam_reset( cslCamH, CSL_CAM_RESET_ARST )) )
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_close][Error] :  csl_cam_reset(): CSL_CAM_RESET_ARST: FAILED \n") );
        }
    // Disable all Lanes  
        chal_cam_param_st.intf = cslCamChalIntf(cslCamDrv.intf_cfg.intf);
        chal_cam_param_st.chan = cslCamChalPortChan(camH->port_chan_sel);
        ...
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_close][Error] : ERROR!\n") );
    ...
    // Power Down Analog
        ...
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_close][Error] :  chal_cam_set_analog_pwr(): CHAL_CAM_ANALOG: FAILED \n") );
            success |= CSL_CAM_ERR;
        }
        chal_cam_param_st.select = CHAL_CAM_ANALOG;
        if (chal_cam_set_analog_pwr(cslCamDrv.chalCamH, &chal_cam_param_st) != CHAL_OP_OK)
    ...
    // Disable Clocks
        ...
        {
            // CSI1_LP			
                if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI1, CLOCK_OFF) != PRM_OK)
                {
                    success |= CSL_CAM_BAD_CLK;
                    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): FAILED \n") );
                }
            // CSI1_AXI			
                if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI1_AXI, CLOCK_OFF) != PRM_OK)
                {
                    success |= CSL_CAM_BAD_CLK;
                    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): RESOURCE_CSI1_AXI: FAILED \n") );
                }
        ...
        {
            // CSI0_LP			
                if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI0, CLOCK_OFF) != PRM_OK)
                {
                    success |= CSL_CAM_BAD_CLK;
                    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): FAILED \n") );
                }
            // CSI0_AXI			
                if (PRM_set_clock_state(cslCamDrv.prmClientId, RESOURCE_CSI0_AXI, CLOCK_OFF) != PRM_OK)
                {
                    success |= CSL_CAM_BAD_CLK;
                    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_open][Error] :  PRM_set_clock_state(): RESOURCE_CSI0_AXI: FAILED \n") );
                }
...
Int32 csl_cam_get_handle( CSL_CAM_HANDLE* cslCamH )
{
    ...
    *cslCamH = NULL;
    ...
    else
    {
        ...
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_get_handle][Error] :  No Handle available \n") );
    }
    return success;
}
...
// Panic
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_intf_control][Info] : ERROR!\n") );
    }    
...
Int32 csl_cam_set_input_mode( CSL_CAM_HANDLE cslCamH, pCSL_CAM_INPUT_st inputMode )
{
    ...
    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_mode][Info] : mode = 0x%x\n", inputMode->input_mode) );
...
// Cfg Lane Enables 
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_mode][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Set Image buffer 0
    if (imageAddr_0 == NULL)
    {        
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_addr][Error] : ERROR! imageAddr_0 is NULL. No image buffers \n") );    
        success |= CSL_CAM_BAD_PARAM;
    }
    else if ( (imageAddr_1 == NULL) || (camH->state.active == 0) || (memcmp(&camH->state.image_buffer_0, imageAddr_0, sizeof(CSL_CAM_BUFFER_st_t)) != 0) )
    {   
        memcpy( (void *)&camH->state.image_buffer_0, (const void *)imageAddr_0, sizeof(CSL_CAM_BUFFER_st_t) );        
        buffers.image0Buff = imageAddr_0;
        camH->state.bufferIndex = 0; // set current buffer index.
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_addr][Info] : Image0 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
...
// Set Image buffer 1
    if (imageAddr_1 == NULL)
    {    
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_addr][Info] : Image1 -> NULL \n")); 
        memset( (void *)&camH->state.image_buffer_1, 0, sizeof( CSL_CAM_BUFFER_st_t ) );            
        camH->state.bufferIndex = 0; // set current buffer index.
    }
    else if ( (camH->state.active == 0) || (memcmp((void *)&camH->state.image_buffer_1, (void *)imageAddr_1, sizeof(CSL_CAM_BUFFER_st_t)) != 0) )
    {
        memcpy( (void *)&camH->state.image_buffer_1, (const void *)imageAddr_1, sizeof(CSL_CAM_BUFFER_st_t) );   
        buffers.image1Buff = imageAddr_1;    
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_addr][Info] : Image1 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
...
// Set Data Addr
    if (dataAddr == NULL)
    {    
        memset( (void *)&camH->state.data_buffer_0, 0, sizeof( CSL_CAM_BUFFER_st_t ) );
    }
    else if ( (camH->state.active == 0) || (memcmp((void *)&camH->state.data_buffer_0, (void *)dataAddr, sizeof(CSL_CAM_BUFFER_st_t)) != 0) )
    {
        memcpy( (void *)&camH->state.data_buffer_0, (const void *)dataAddr, sizeof(CSL_CAM_BUFFER_st_t) );            
        buffers.data0Buff = dataAddr;    
    }
    
    
    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_addr][Info] : Image0 addr=0x%x, Image1 addr=0x%x, dataAddr=0x%x \n", camH->state.image_buffer_0.start_addr, camH->state.image_buffer_1.start_addr, camH->state.data_buffer_0.start_addr) );  
    
    success |= csl_cam_set_buffer_params(cslCamH, buffers);    
    if (success)
    {        
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_input_addr][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    return success;
}
...
Int32 csl_cam_set_dbl_buf( CSL_CAM_HANDLE cslCamH, CSL_CAM_BUFFER_PTR_st_t cslBuffers )
    ...
    if ( (cslBuffers.image0Buff == NULL) || (cslBuffers.image1Buff == NULL) || (cslBuffers.data0Buff == NULL) || (cslBuffers.data1Buff == NULL))
    {
...
// Set Image buffer 0
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_dbl_buf][Info] : Image0 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
                           cslBuffers.image0Buff->start_addr, cslBuffers.image0Buff->size, cslBuffers.image0Buff->line_stride, cslBuffers.image0Buff->buffer_wrap_en) );  
        memcpy( &camH->state.image_buffer_0, cslBuffers.image0Buff, sizeof(CSL_CAM_BUFFER_st_t) );        
        camH->state.bufferIndex = 0; // set current buffer index.
// Set Image buffer 1
        memcpy( &camH->state.image_buffer_1, cslBuffers.image1Buff, sizeof(CSL_CAM_BUFFER_st_t) );   
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_dbl_buf][Info] : Image1 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
                       cslBuffers.image1Buff->start_addr, cslBuffers.image1Buff->size, cslBuffers.image1Buff->line_stride, cslBuffers.image1Buff->buffer_wrap_en) );  
   
// Set Data buffer 0
        memcpy( &camH->state.data_buffer_0, cslBuffers.data0Buff, sizeof(CSL_CAM_BUFFER_st_t) );            
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_dbl_buf][Info] : Data0 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
                       cslBuffers.data0Buff->start_addr, cslBuffers.data0Buff->size, cslBuffers.data0Buff->line_stride, cslBuffers.data0Buff->buffer_wrap_en) );  
// Set Data buffer 1
        memcpy( &camH->state.data_buffer_1, cslBuffers.data1Buff, sizeof(CSL_CAM_BUFFER_st_t) );            
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_dbl_buf][Info] : Data1 -> addr=0x%x, size=%d, line_stride=%d, buffer_wrap_en=%d \n", 
                       cslBuffers.data1Buff->start_addr, cslBuffers.data1Buff->size, cslBuffers.data1Buff->line_stride, cslBuffers.data1Buff->buffer_wrap_en) );  

    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_dbl_buf][Info] : Image0=0x%x, Image1=0x%x, data0=0x%x data1=0x%x \n", camH->state.image_buffer_0.start_addr, camH->state.image_buffer_1.start_addr, camH->state.data_buffer_0.start_addr, camH->state.data_buffer_1.start_addr) );  
 ...
 *
 ***********************************************************/
static Int32 csl_cam_set_buffer_params( CSL_CAM_HANDLE cslCamH, CSL_CAM_BUFFER_PTR_st_t cslBuffers)
{
    Int32                       success = CSL_CAM_OK;                   
    CAM_HANDLE                  camH = (CAM_HANDLE)cslCamH;
    CHAL_CAM_STATUS_CODES       chal_status = CHAL_OP_OK;
...
// Set Image buffer
    ...
    // ACP Compatible Memory Configuration  
        if (cslBuffers.image0Buff->mem_type & CSL_CAM_MEM_TYPE_ACP_COMPATIBLE)
        {
        ...
        {          
            ...
            image1.buf_wrap_enable = cslBuffers.image1Buff->buffer_wrap_en;  
        // ACP Compatible Memory Configuration  
            if (cslBuffers.image1Buff->mem_type & CSL_CAM_MEM_TYPE_ACP_COMPATIBLE)
            {
...
// Set Data Buffer
    ...
    // ACP Compatible Memory Configuration  
        if (cslBuffers.data0Buff->mem_type & CSL_CAM_MEM_TYPE_ACP_COMPATIBLE)
        {
        ...
        {       
            ...
            data1.buf_wrap_enable =  cslBuffers.data1Buff->buffer_wrap_en;  
        // ACP Compatible Memory Configuration  
            if (cslBuffers.data1Buff->mem_type & CSL_CAM_MEM_TYPE_ACP_COMPATIBLE)
            {
...
// Update Buffers base on buffering flag    
    ...
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_buffer_params][Info] :  Dbl Buffer Enabled \n") );
        chal_status |= chal_cam_cfg_dbl_buffer(cslCamDrv.chalCamH, &chal_cam_buffer_cfg_st);
    }
    else
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_buffer_params][Info] :  Dbl Buffer Disabled \n") );
...
// Interrupts
    ...
    {
        success |= CSL_CAM_BAD_PARAM;
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_frame_control][Error] :  LINE_START/LINE_END not allowed!\n") );
    }
...
// Line Count
    DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_frame_control][Info] :  intr=0x%x line_count=%d\n",chal_cam_frame_cfg_st.interrupts,frameCtrl->int_line_count) );
...
// Capture mode
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_frame_control][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Image Channel Id's
    ...
    chal_cam_image_id_st.image_data_id_4 = (cUInt8)imageCtrl->image_data_id4;
    chal_cam_image_id_st.image_data_id_5 = (cUInt8)imageCtrl->image_data_id5;
    chal_cam_image_id_st.image_data_id_6 = (cUInt8)imageCtrl->image_data_id6;
    chal_cam_image_id_st.image_data_id_7 = (cUInt8)imageCtrl->image_data_id7;
    chal_status |= chal_cam_cfg_image_id(cslCamDrv.chalCamH, &chal_cam_image_id_st);
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_image_type_control][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// FSP Decoding
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_data_type_control][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Config packet captures
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_packet_control][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Parameter
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_lane_control][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Decode Pixel unpacking
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_pipeline_control][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Channel
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_set_image_window][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Check status bits    
    *intStatus =  (CSL_CAM_INTERRUPT_t)(cslCamChalIntrStatus( chal_cam_param_st.param ));
    return raw_status;
}
...
// Csl status bits from Chal
    *rxStatus = (CSL_CAM_RX_STATUS_t)cslCamChalRxStatus(chal_cam_param_st.param);
    return raw_status;
}
...
// Lane Status    
    ...
    *laneStatus = cslCamChalLaneStatus( (CHAL_CAM_LANE_STATE_t)chal_cam_param_st.param );
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_get_lane_status][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Channel
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_trigger_capture][Info] : ERROR!\n") );
    }
    else
    {
    // Update Capture mode
...
// Verify Receiver was opened
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_rx_start][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
...
// Verify Receiver is active
    ...
    // Set Receiver to Inactive
        ...
        if ( (success |= csl_cam_reset( cslCamH, CSL_CAM_RESET_SWR )) )
        {
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_rx_stop][Error] :  csl_cam_reset(): CSL_CAM_RESET_SWR: FAILED \n") );
        }
        if ( (success |= csl_cam_reset( cslCamH, CSL_CAM_RESET_ARST )) )
        {
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_rx_stop][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
    success |= cslCamChalResult(chal_status);
    return success;
...
// Reset 
    ...
    if (chal_status != CHAL_OP_OK)
    {
        DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_reset][Info] : ERROR!\n") );
    }
    DBG_OUT_REGISTER_DISPLAY( csl_cam_register_display( cslCamH ));
...
// CallBack Event
    switch (event)
    {
        ...
        default:
            success |= CSL_CAM_BAD_PARAM;
            DBG_OUT(CSLCAM_DBG(CSLCAM_DBG_ID,  "[csl_cam_register_event_callback][Info] : ERROR! No Selection\n") );
            break;
    }
    return success;
}
