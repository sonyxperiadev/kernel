...
static int      cslSmiSetSmiClk ( pSMI_SPI_HANDLE pSmi, 
                    pCSL_SMI_CLK pSmiClkCfg );
...
static int cslSmiSetSmiClk ( pSMI_SPI_HANDLE pSmi, pCSL_SMI_CLK pSmiClkCfg )
{
    ...
    if( prmRes != PRM_OK )
    {
        LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] %s: Failed To Set SMI Clock\n\r",
            __FUNCTION__ );
        return ( -1 );
    }
    ...
    if( prmRes != PRM_OK )
    {
        LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] %s: Failed To Start SMI Clock\n\r",
            __FUNCTION__ );
        return ( -1 );
    }
    ...
    {
        LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] %s: SMI CLock State Mismatch\n\r",
            __FUNCTION__ );
        return ( -1 );
    }
...
static void cslSmiUpdateTask (void)
{
    ...
    for (;;)
    {
        ...
        osStat = OSSEMAPHORE_Obtain ( eofDmaSema, 
            TICKS_IN_MILLISECONDS ( updMsg.timeOutMsec ) );
        ...
        if ( osStat != OSSTATUS_SUCCESS)
        {
            ...
            LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI/SPI] %s: DMA end TimeOut\n\r",
                __FUNCTION__ );
        }
        ...
        else
        {
            ...
            if ( res == CSL_LCD_OK )
            {
                res = cslSmiWaitForInt ( updMsg.smiH, updMsg.timeOutMsec );
            }
            else
            {
                cslSmiDisInt ( updMsg.smiH );
                LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI/SPI] %s: DONE INT "
                    "TimeOut\r\n",  __FUNCTION__ );
            }    
        }
        
        if ( !updMsg.smiH->hasLock ) OSSEMAPHORE_Release ( smiSema );		
...
static CSL_LCD_RES_T cslSmiWaitForInt( pSMI_SPI_HANDLE pSmi, UInt32 tout_msec )
{
    ...
    if ( osRes != OSSTATUS_SUCCESS )
    {
        ...
        {
            LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] %s: ERR Timed Out!\n\r",
                __FUNCTION__  ); 
            res = CSL_LCD_OS_TOUT;
        }    
        else
        {
            LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] %s: ERR OS Err...\n\r",
                __FUNCTION__ ); 
            res = CSL_LCD_OS_ERR;
        }    
    } 
...
#else
    {
    	int ret = request_irq(BCM_INT_ID_SMI, cslSmi_LISR, IRQF_DISABLED,
				"rhea smi interrupt", (void *)NULL);
    	if (ret < 0) {
		printk(KERN_ERR "Unable to register Interrupt for Rhea SMI interface\n");
		res = FALSE;
	}
    }
...
//                
//*****************************************************************************
static int cslSmiCalcTiming ( 
    pSMI_SPI_HANDLE     pSmi,
    ...
    )
{
    ...
    if ( pChalTiming->strobe > pChalTimingMax->strobe ) res = -1;
    if ( pChalTiming->pace   > pChalTimingMax->pace   ) res = -1;
    if ( pChalTiming->hold   > pChalTimingMax->hold   ) res = -1;
    if ( pChalTiming->setup  > pChalTimingMax->setup  ) res = -1;
    if( res == -1 )
    {
        LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] %s: ERR Timing Reg Overflow\n\r",
            __FUNCTION__ );
    }
...
//
//*****************************************************************************
static CSL_LCD_RES_T cslSmi2cHal (
    pSMI_SPI_HANDLE pSmi,
    CSL_SMI_CTRL_T* smiCfg    
    )
{
...
//          break;
    ...
    if( cslSmiCalcTiming ( pSmi, &smiCfg->rdTiming, &pSmi->cfg.smiCfg.rdTiming, 
        &chalTimingMax   ) != 0 ) 
    {    
        return ( CSL_LCD_BUS_CFG );
    }    
    ...
    else
    {
        ...
        if ( cslSmiCalcTiming ( pSmi, &smiCfg->wrTiming_m, 
                &pSmi->cfg.smiCfg.wrTiming_m, &chalTimingMax   ) != 0 ) 
        {    
            return ( CSL_LCD_BUS_CFG );
        }    
    }
    ...
    )
{
    ...
    if ( !pSmi->hasLock ) OSSEMAPHORE_Obtain ( smiSema, TICKS_FOREVER );
    ...
    chal_smi_dir_write_acc ( pSmi->chalH, pSmi->cfg.smiCfg.bank, 
        cmnd ? pSmi->cfg.smiCfg.addr_c : pSmi->cfg.smiCfg.addr_d, data );
    ...
    )
{
    ...
    chal_smi_dir_read_acc ( pSmi->chalH, pSmi->cfg.smiCfg.bank, 
        cmnd ? pSmi->cfg.smiCfg.addr_c : pSmi->cfg.smiCfg.addr_d );
...
//                
//*****************************************************************************
CSL_LCD_RES_T CSL_SMI_WrRdDataProg (
    CSL_LCD_HANDLE      smiH,  
    pSMI_PROG_ACC       acc
    )
{
    ...
    else
    {
        ...
        {   // BLOCKING                                 
            ...
            status = OSSEMAPHORE_Obtain ( eofDmaSema, 
                TICKS_IN_MILLISECONDS ( acc->timeOutMsec ) );
		...
		//panic("No VC4 DMA int back\n");
                ...
                {
                    LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SMI] CSL_SMI_WrRdDataProg: "
                        "ERR EOFDMA Timeout!\n\r" ); 
                    res = CSL_LCD_OS_TOUT;
                }
                else
...
#endif    
            status = OSQUEUE_Post ( updReqQ, (QMsg_t*) &updMsg, TICKS_NO_WAIT );
            if ( status != OSSTATUS_SUCCESS )
            {
                cslSmiDisInt ( pSmi );
                cslSmiSpiDmaStop ( updMsg.dmaCh );   
...
CSL_LCD_RES_T CSL_SMI_Close ( CSL_LCD_HANDLE smiH )  
{
    ...
    if( pSmi->open )
    {
        ...
        if ( pSmi->prm_ID >= 0 )
        {
            PRM_set_clock_state( pSmi->prm_ID, RESOURCE_SMI, CLOCK_OFF );
...
//
//*****************************************************************************
CSL_LCD_RES_T CSL_SMI_Open (
    CSL_SMI_CTRL_T*     busCfg,    
    CSL_LCD_HANDLE*     smiH 
    )
{
...
//                 32-bit(4B) <- 0x09
//*****************************************************************************
CSL_LCD_RES_T CSL_SPI_VC4L_LossiRead ( 
    CSL_LCD_HANDLE  spiH,
    ...
    )
{
    ...
    if ( !pSpi->hasLock ) OSSEMAPHORE_Obtain ( smiSema, TICKS_FOREVER );

    if ( pSpiRdAcc->spiDiv != 0 )
    { 
        if ( chal_spivc4l_set_clk_div ( pSpi->chalH, pSpiRdAcc->spiDiv ) == -1 )
        {
            if ( !pSpi->hasLock ) OSSEMAPHORE_Release ( smiSema );
            LCD_DBG ( LCD_DBG_ERR_ID, "[CSL SPI VC4L] %s: "
...
CSL_LCD_RES_T CSL_SPI_VC4L_LossiWrite ( 
    CSL_LCD_HANDLE  spiH, 
    ...
    )
{
    ...
    {
        if ( chal_spivc4l_set_clk_div ( pSpi->chalH, pSpiWrAcc->spiDiv ) == -1 )
        {
            if ( !pSpi->hasLock ) OSSEMAPHORE_Release ( smiSema );
    ...
    if( !pSpiWrAcc->isDma )
    {
        ...
        while ( i < accLen )
        {
            ...
            chal_spivc4l_wr_fifo ( pSpi->chalH, pSpiWrAcc->pBuff[i++] );   
        }
    ...
    else
    {
        ...
        while ( chal_spivc4l_get_cs( pSpi->chalH ) & CHAL_SPIVC4L_CS_CLEAR );
...
#endif	
            status = OSSEMAPHORE_Obtain ( eofDmaSema, 
                TICKS_IN_MILLISECONDS ( pSpiWrAcc->timeOutMsec ) );
