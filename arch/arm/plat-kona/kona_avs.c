...
static int otp_read(int row, struct mon_val* mon_val)
{
	...
	if(row < 0)
		return -EINVAL;
	mon_val->val0 = (146 << MONITOR_VAL0_SHIFT) | (180 << MONITOR_VAL1_SHIFT) | (95 << MONITOR_VAL2_SHIFT) ;
	mon_val->val1 =  170;
...
int kona_avs_get_mon_val(struct avs_info* avs_inf_ptr)
{
	...
	if(avs_inf_ptr->pdata->flags & AVS_READ_FROM_MEM)
	{
		...
		pr_info("%s: AVS_READ_FROM_MEM => mem adr = %x\n",__func__,avs_inf_ptr->pdata->param);
		BUG_ON(avs_inf_ptr->pdata->param == 0);
		mem_ptr = ioremap_nocache(avs_inf_ptr->pdata->param,sizeof(struct mon_val));
	...
	if(!ret)
	{
		...
		avs_inf_ptr->monitor_val0 = (mon_val.val0 >> MONITOR_VAL0_SHIFT) & MONITOR_VAL_MASK;
		avs_inf_ptr->monitor_val1 = (mon_val.val0 >> MONITOR_VAL1_SHIFT) & MONITOR_VAL_MASK;
		avs_inf_ptr->monitor_val2 = (mon_val.val0 >> MONITOR_VAL2_SHIFT) & MONITOR_VAL_MASK;
		avs_inf_ptr->monitor_val3 = (mon_val.val1 >> MONITOR_VAL3_SHIFT) & MONITOR_VAL_MASK;
...
static int kona_avs_drv_probe(struct platform_device *pdev)
{
	...
	BUG_ON((pdata->flags & AVS_TYPE_OPEN) && (pdata->flags & AVS_TYPE_BOOT));
	BUG_ON((pdata->flags & AVS_READ_FROM_OTP) && (pdata->flags & AVS_READ_FROM_MEM));
