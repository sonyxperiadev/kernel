...
static struct avs_pdata avs_pdata = {
	.avs_info_base_addr = (int)HW_IO_PHYS_TO_VIRT(0x3404E39C),
	.a7_regl_name = "csr_uc",
	.pwrwdog_base = (int)KONA_PWRWDOG_VA,
	.irdrop_vreq = 1200000,
};
	...
	for (i = 0; i < ARRAY_SIZE(ion_heap_reserve_datas); i++) {
		...
		if (ion_scan_pdata(reserve_data))
			pr_info("ion: From pdata %16s Type(%d) Size(%3dMB) Range(%08lx - %08lx)\n",
					...
					(long unsigned int)reserve_data->base,
					(long unsigned int)reserve_data->limit);
#else
		...
		if (!reserve_data->status)
			pr_info("ion: Reserve %16s %3dMB (%08lx - %08lx) ***\n",
					...
					(long unsigned int)reserve_data->base,
					(long unsigned int)reserve_data->base +
					(long unsigned int)reserve_data->size);
	}
}
#endif /* CONFIG_ION */
