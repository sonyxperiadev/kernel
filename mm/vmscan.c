				      ...
				      bool force_reclaim)
{
	...
	while (!list_empty(page_list)) {
		 ...
		 *    thread might enter reclaim, and deadlock if
		 *    it waits on a page for which it is needed to do
		 *    the write (loop masks off __GFP_IO|__GFP_FS for
		 *    this reason); but more thought would probably show more
		 *    reasons.
		 *
