...
*
* 	@file	include/linux/broadcom/2157SDB/IPCInterface.h
*
 ...
 * ============================================================
 */
#ifdef UNDER_CE /* For WinMo UNDER_CE is defined */
#include "csl_types.h"
 ...
 * receiving CAPI2 responses and Indication
 */
...
/* Used for sending Audio commands from Audio device driver
 * on the AP to the DSP on the CP
 */
#define	IPC_EP_AudioControl_AP	 7
/* Used for receiving Audio commands from the Audio device driver
 * on the AP which have to be sent to the DSP
 */
...
/* Used for sending and receiving Samsung IPC4.0 messages on CP */
#define IPC_EP_SSIPC40Cp		11
/* Used for sending and receiving Samsung IPC4.0 messages on AP */
#define IPC_EP_SSIPC40Ap		12
...
#define CP_DUMP_LEN			16*1024*1024
#endif
...
typedef IPC_U32 (*IPCPSCheckUartLowPowerAllowedFPtr_T)(void);
...
/* ***************************************
 * Returns number of free buffers for the pool
 */
...
IPC_ReturnCode_T IPC_CpEnableUartLowPowerMode();
IPC_ReturnCode_T IPC_CpDisableUartLowPowerMode();
IPC_ReturnCode_T IPC_ApEnableUartLowPowerMode(void);
IPC_ReturnCode_T IPC_ApDisableUartLowPowerMode(void);
IPC_ReturnCode_T IPC_ApResetUartLowPowerMode(void);
 ...
 */
/* ************************************** */
/* Size in units of U32 */
#ifdef GPS_LTO_DATA_IN_PDS
#define IPC_PERSISTENT_DATA_SIZE (0x4000 + 0xA00 + 256)
/* 64K = GPS LTO data + 10K General GPS Data +
 * a bit a room for other small persistent data
 */
#else
#define IPC_PERSISTENT_DATA_SIZE (0xA00 + 256)
/* 10K General GPS Data + a bit a room for other small persistent data */
#endif
 ...
 * ============================================================
 * Property Interface
 * Properties are 32 bit values that are read\write by one
 * processor but read only by the other processor.
 * This allows atomic access to be guaranteed.
 * These are used to allow information to be shared without the
 * overhead of CAPI2 message exchanges. Useful for device drivers
