...
typedef struct vcp_host_dec_cbks
{
   ...
   */
   void (*free)(
      VCP_FRAMEBUF *decframep,      /**< (i) Ptr to frame buffer to free */
      void *privdata                /**< (i) Registered private data */
   );
...
int vcp_open_encoder(
   ...
   VCP_HOST_ENC_CBKS *cbks,         /**< (i) Host callbacks */
...
int vcp_open_decoder(
   const char *devname,             /**< (i) Device name */
   VCP_HOST_DEC_CBKS *cbks,         /**< (i) Host callbacks */
   void *cbdata,                    /**< (i) Private data for callbacks */
   VCP_HDL *handlep                 /**< (o) If success, valid handle */
);
...
int vcp_add_conference(
   VCP_HDL dec_hdl,                 /**< (i) Handle to VCP decode channel (source) */
   VCP_HDL enc_hdl                  /**< (i) Handle to VCP encode channel */
);
...
int vcp_set_conference_param(
   VCP_HDL hdl,                     /**< (i) Handle to VCP channel holding the conference */
...
int vcp_get_encoder_stats(
   VCP_HDL handle,                  /**< (i) Handle to VCP channel */
   int reset,                       /**< (i) 1 to reset stats after reading, otherwise no change */
