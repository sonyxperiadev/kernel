            ...
            if((fifo == CAPH_PASSCH1_INFIFO) || (fifo == CAPH_PASSCH2_INFIFO) || (fifo == CAPH_PASSCH3_INFIFO) || (fifo == CAPH_PASSCH4_INFIFO))
            {
                ...
                {  /* HARRYN: need changes here for Rhea B0 */
                    case CAPH_STEREO_16BIT:
                        fmt = 0x00;  /* Rhea A0/B0 - 16bit packed stereo */
                        break;
                    case CAPH_MONO_16BITP:
                        fmt = 0x01; /* Rhea B0: 16bit packed mono, A0: 16bit unpacked mono */
                        break;
                    case CAPH_STEREO_24BIT:
                        fmt = 0x02;  /* Rhea B0 - 24bit unpacked stereo */
                        break;
                    case CAPH_MONO_24BIT:
                        fmt = 0x03;  /* Rhea B0 - 24bit unpacked mono */
                        break;
                    case CAPH_STEREO_16BITUP:
                        fmt = 0x04; /* Rhea B0: 16bit unpacked stereo */
                        break;
                    case CAPH_MONO_16BIT:
                        fmt = 0x05; /* Rhea B0: 16bit unpacked mono */
                        break;
                    case CAPH_STEREO_24BITUP:
                        fmt = 0x06;  /* Rhea B0 - 24bit unpacked stereo */
                        break;
                    default:
                        break;
                }
                ...
                {   /* 1) fmt must be mono. 2) passthrough channel 1 to be also set to mono */
                    //reg_val = fmt << SRCMIXER_PASSTHROUGH_CHANNEL1_CTRL_PASSTHROUGH_CHANNEL1_FORMAT_SHIFT;
                    //BRCM_WRITE_REG(srcm_cb->base,  SRCMIXER_PASSTHROUGH_CHANNEL1_CTRL, reg_val);
                ...
                {   /* HARRYN: might not need the first two statements */
                    reg_val = fmt << SRCMIXER_PASSTHROUGH_CHANNEL2_CTRL_PASSTHROUGH_CHANNEL2_FORMAT_SHIFT;
                    BRCM_WRITE_REG(srcm_cb->base,  SRCMIXER_PASSTHROUGH_CHANNEL2_CTRL, reg_val);
    ...
    for(mindex = 0; mindex < 4; mindex++)
    {
        if((1UL << mindex) == mixerOutput)
        {
            ...
            for(findex = 0; findex < CHAL_CAPH_SRCM_MAX_FIFOS; findex++)
            {
                if((1UL << findex) & chnl)
                {
                    idx = (chal_caph_srcm_gain_ctrl_index[mindex]*SRCMIXER_MIX_CH_GAIN_CTRL_OFFSET + findex);
                    reg_val = BRCM_READ_REG_IDX(srcm_cb->base, SRCMIXER_SRC_M1D0_CH1M_GAIN_CTRL, idx);
                    ...
                    BRCM_WRITE_REG_IDX(srcm_cb->base, SRCMIXER_SRC_M1D0_CH1M_GAIN_CTRL, idx, reg_val);
                }
            }
...
/****************************************************************************
*
*  Function Name: cUInt8 chal_caph_get_passthrough_channels_count(cUInt8 bStereo)
*
