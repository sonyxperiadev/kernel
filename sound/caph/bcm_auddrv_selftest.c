...
xReadValue, xResultArray, xErrorCode) \
  ...
  for (i = 0 ; i < (xChecks) ; i++) { \
	ST_MDBG("GLUE_SELFTEST::CHECKBIT_AND_ASSIGN_ERROR(%u):" \
	 ...
	 ((TestValue) & (1 << i))); \
	if (xResultArray[i] == ST_SELFTEST_PASS) {\
	  if ((xAssertLevel == 1))  { \
...
/* Audio */
	ST_SELFTEST_SHORTED_GROUND,
	ST_SELFTEST_SHORTED_POWER,
	ST_SELFTEST_BAD_CONNECTION,
	ST_SELFTEST_BAD_CONNECTION_OR_GROUND,
	ST_SELFTEST_BAD_CONNECTION_OR_POWER,
	ST_SELFTEST_SHORTED_GROUND_ACI,
	ST_SELFTEST_SHORTED_GROUND_BIAS,
	ST_SELFTEST_SHORTED_POWER_ACI,
	ST_SELFTEST_SHORTED_POWER_BIAS,
	ST_SELFTEST_RESULT_CNT
} Selftest_Results_t;
...
typedef enum Selftest_Digimic_Setup_e {
	ST_SELFTEST_DIGIMIC_ALL,
	ST_SELFTEST_DIGIMIC_MIC1,
	ST_SELFTEST_DIGIMIC_MIC2,
	ST_SELFTEST_DIGIMIC_MIC3,
	ST_SELFTEST_DIGIMIC_MIC4
...
typedef struct SelftestDataDmic_t {
	ResultFormat_e	rf;
	int		mic;
} SelftestDataDmic_t;
...
typedef struct SelftestData_t {
	SelftestDataDmic_t    dmic;
	SelftestDataStandard_t  hs;
	SelftestDataStandard_t  ihf;
} SelftestData_t;
					  ...
					  dac_ctrl_t writedata)
{
	...
	ST_DBG("GLUE_SELFTEST::st_audio_audiotx_set_dac_ctrl(): Pre-AUDIOH_DAC_CTRL: 0x%08X", ctrl);

	ctrl &= (~AUDIOH_DAC_CTRL_AUDIOTX_TEST_EN_MASK);
	ctrl &= (~AUDIOH_DAC_CTRL_AUDIOTX_BB_STI_MASK);

	ctrl |=  ((writedata.AUDIOTX_TEST_EN)) & 3 << AUDIOH_DAC_CTRL_AUDIOTX_TEST_EN_SHIFT;
	ctrl |=  ((writedata.AUDIOTX_BB_STI) & 3) << AUDIOH_DAC_CTRL_AUDIOTX_BB_STI_SHIFT;

	ST_DBG("GLUE_SELFTEST::st_audio_audiotx_set_dac_ctrl(): AUDIOH_DAC_CTRL: 0x%08X", ctrl);
	chal_audio_audiotx_set_dac_ctrl(audiohandle, ctrl);

	{
		u32 val;
		val = BRCM_READ_REG(KONA_AUDIOH_VA, AUDIOH_DAC_CTRL);
		ST_DBG("GLUE_SELFTEST::std_selftest_hs(): AUDIOH_DAC_CTRL: 0x%08X", val);
	}
}
...
static 	dac_ctrl_t Stored_dac_ctrl_Value;
...
void st_audio_store_registers(selftest_store_e test)
{
	...
	if (TestActive[SELFTEST_DMIC] ||
	    TestActive[SELFTEST_IHF] ||
	    TestActive[SELFTEST_HS]) {
		/* Store clock settings  */
		StoredClkGate = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_AUDIOH_CLKGATE);
		StoredPolicyFreq = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY_FREQ);
		StoredPolicyMask[0] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY0_MASK1);
		StoredPolicyMask[1] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY1_MASK1);
		StoredPolicyMask[2] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY2_MASK1);
		StoredPolicyMask[3] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY3_MASK1);
		StoredPolicyMask[4] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY0_MASK2);
		StoredPolicyMask[5] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY1_MASK2);
		StoredPolicyMask[6] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY2_MASK2);
		StoredPolicyMask[7] = BRCM_READ_REG(KONA_HUB_CLK_BASE_VA, KHUB_CLK_MGR_REG_POLICY3_MASK2);
	}
	...
	case SELFTEST_DMIC:
		...
		if (IS_ERR(regl_hv7ldo)) {
			...
			regl_hv7ldo_orig_state = regulator_is_enabled(regl_hv7ldo);
...
void st_audio_restore_registers(selftest_store_e test)
{
	CHAL_HANDLE audiohandle;
	struct regulator *regl_hv7ldo = NULL;
	audiohandle  =  chal_audio_init(KONA_AUDIOH_VA, KONA_SDT_BASE_VA);
	...
	case SELFTEST_DMIC:
		...
		ST_DBG("GLUE_SELFTEST:: 1:pinmux_set_pin_config(%u)", DmicStoredValue[0].name);
		pinmux_set_pin_config(&DmicStoredValue[0]);
		ST_DBG("GLUE_SELFTEST:: 2:pinmux_set_pin_config(%u)", DmicStoredValue[1].name);
		pinmux_set_pin_config(&DmicStoredValue[1]);
		ST_DBG("GLUE_SELFTEST:: 3:pinmux_set_pin_config(%u)", DmicStoredValue[2].name);
		pinmux_set_pin_config(&DmicStoredValue[2]);
		ST_DBG("GLUE_SELFTEST:: 4:pinmux_set_pin_config(%u)", DmicStoredValue[3].name);
...
{
	u8  ResultArray1[IHF_NUMBER_OF_SUBTESTS1] = {ST_SELFTEST_PASS,
		ST_SELFTEST_PASS};
	u8  ResultArray2[IHF_NUMBER_OF_SUBTESTS2] = {ST_SELFTEST_PASS,
		ST_SELFTEST_PASS};
	...
	CHECKBIT_AND_ASSIGN_ERROR(1, IHF_NUMBER_OF_SUBTESTS1, ReadValue,
				  ResultArray1, ST_SELFTEST_SHORTED_POWER);
...
{
	u8  ResultArray[HA_NUMBER_OF_SUBTESTS]  =  {ST_SELFTEST_PASS,
		ST_SELFTEST_PASS,
		ST_SELFTEST_PASS,
		ST_SELFTEST_PASS};
	u8  ReadValue; /* Value read from PMU */
	...
	/* Subtest 1 + 2 + 3 + 4  */
	/**************************/
	ST_DBG("GLUE_SELFTEST::std_selftest_hs()  RA[0] = %u,RA[1] = %u,RA[2] = %u,RA[3] = %u",
...
static void std_selftest_dmic(struct SelftestUserCmdData_t *cmddata)
{
	...
	case ST_SELFTEST_DIGIMIC_ALL:
		...
		TestMic[3] = true;
		break;
	case ST_SELFTEST_DIGIMIC_MIC1:
		TestMic[0] = true;
		break;
	case ST_SELFTEST_DIGIMIC_MIC2:
		TestMic[1] = true;
		break;
	case ST_SELFTEST_DIGIMIC_MIC3:
		TestMic[2] = true;
		break;
	case ST_SELFTEST_DIGIMIC_MIC4:
		TestMic[3] = true;
		break;
	}
	...
	for (Mic = 0 ; Mic < MAX_DIGIMIC_COUNT ; Mic++) {
		...
		if (DigiMicInterruptReceived == true) {
			...
			Status[(Mic*DIGIMIC_SUBTESTS_PER_MIC)+DIGIMIC_COMM_TEST]    =  ST_SELFTEST_PASS;
		} else {
		...
		if (Status[(Mic*DIGIMIC_SUBTESTS_PER_MIC)+DIGIMIC_COMM_TEST] == ST_SELFTEST_PASS) {
			...
			gpio_free(DQ_CONNECTION[MicIf]);
			if (DigiMicInterruptReceived == true) {
				/* b.    Interrupt received  = > ST_SELFTEST_PASS */
				ST_DBG("GLUE_SELFTEST::std_selftest_digimic() Interrupt Received");
				Status[(Mic*DIGIMIC_SUBTESTS_PER_MIC)+DIGIMIC_COMM_TEST]    =  ST_SELFTEST_PASS;
			} else {
				/* a.    Timeout  = > No connection - Test failed */
	...
	/* Find return code */
	if (((Status[0] != 0) && (Status[0] != ST_SELFTEST_NOT_TESTED)) ||
	    ((Status[1] != 0) && (Status[1] != ST_SELFTEST_NOT_TESTED)) ||
	    ((Status[2] != 0) && (Status[2] != ST_SELFTEST_NOT_TESTED)) ||
	    ((Status[3] != 0) && (Status[3] != ST_SELFTEST_NOT_TESTED))) {
...
int BrcmAudioSelftest(void)
{
	...
	if (cmddata.testStatus == ST_SELFTEST_PASS) {
