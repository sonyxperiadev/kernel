...
#define SHARED_INVALID_CELL    ((UInt16) 0xffff)    // Invalid cell ID
...
//#define        DSP_FEATURE_TBF_FASTCMD        // disabled since theis feature is not fully debugged with arm code
...
{// Command						   Code		Arguments (** = unused command)
 // ========================	   ====		====================================
    ...
    *                    0x7E  
    *  \par Description 
    *       This command is used for configuring the ARM2SP interface for mixing/playing 8kHz PCM data to either 
    ...
    *              
    *       To start ARM2SP: ARM enables ARM2SP by COMMAND_SET_ARM2SP. 
    ...
    *
    */    
	COMMAND_SET_ARM2SP,						// 0x7E		( check interface doc )
    ...
    *              
    *  \see USB_Headset_Interface, STATUS_USB_HEADSET_BUFFER, SampleRateC_t
    */
    COMMAND_USB_HEADSET,			// 0x83		( set pg_aud_usb_enable = arg0, [b15|b14|b13:b10|b9|b8|b7:b4|b3:b2|b1|b0]=[DL_GAIN|0|DL_SRC|0|UL_GAIN|UL_SRC|xx|DL|UL] )
	...
	COMMAND_SET_BT_NB,				// 0x87	( check interface doc )
    ...
    *              COMMAND_AUDIO_CONNECT, MM_VPU_RECORD, MM_VPU_PLAYBACK, COMMAND_MM_VPU_DISABLE
    */
    COMMAND_MM_VPU_ENABLE,					// 0x94
   /** \HR */
   /** \par Module
    *                    Audio 
    *  \par Command Code         
    ...
    */
	COMMAND_MM_VPU_DISABLE,                 // 0x95
	CORE_TL3210_DUMP,						// Core dump to MTT at any time of processing
...
{// Status						   Code		Arguments (** = unused status)
 // ========================	   ====		====================================
    ...
    *       STATUS_PRAM_CODEC_INPUT_EMPTY, requesting the ARM to send more downlink data immediately, 
    ...
    *       to the ARM with a status reply STATUS_PRAM_CODEC_OUTPUT_FULL, 
    ...
    *                    0x36  
    *  \par Description
    *       This reply is part of the ARM2SP interface for mixing/playing 8kHz PCM data to either 
    *       Uplink or Downlink. \BR 
    ...
    *                                            else send back Arm2SP_flag received in the COMMAND_SET_ARM2SP command \BR
    *              \htmlonly 
    *              <pre>
    *                             {
    *                               bits15:11 - rsv
    ...
    *       WB-AMR encoded data. 
    *
    *       This recording is enabled using COMMAND_MM_VPU_ENABLE  command. 
    *
    ...
    *
    *
    *  \see MM_VPU_RECORD, MM_VPU_Interface, COMMAND_MM_VPU_ENABLE, shared_encoder_OutputBuffer,
    ...
    *
    *       This interface is enabled using COMMAND_USB_HEADSET  command. 
    *
    ...
    *
    *              @param  UInt16 pg_aud_usb_enable (This is the same first parameter received in COMMAND_USB_HEADSET)
    ...
    *
    *
    *  \see USB_Headset_Interface, COMMAND_USB_HEADSET
    *
    */	
	...
	STATUS_BT_NB_BUFFER_DONE,			// 0x3D	( arg0=isr ptr, arg1=bk ptr(used by arm or dsp bk) )
	STATUS_AUDIR2_BUFFER_DONE,			// 0x3E	( arg0=isr ptr, arg1=bk ptr(used by arm or dsp bk) )
	...
	STATUS_voif_BUFFER_READY,			// 0x45
	STATUS_MODEM_DATA_READY,			// 0x46
	STATUS_GPIO_MISMATCH,				// 0x47
	STATUS_ARM2SP2_EMPTY				// 0x48
	...
	STATUS_ERROR_E23,			0xec23		// Report an error on frame size over maximum allowed by HW (qbc_next_frame, qbc_prev_adjust, (next_next_rxmode<<12) | (next_next_slot<<8) | next_next_cell).
    ...
    STATUS_ERROR_CORE1			0XEF00		// Status Cache error, Invalid cache selection configuration
    */
} Status_t;
/**
 * @}
...
typedef struct
{
	...
	UInt16 peak_index;
	UInt16 Equ_dir;
	UInt16 IQ_Data[156*2];
	UInt16 Derot_IQ[156*2];
	UInt16 ChEst[12];
	UInt16 FFCoef[10];
	UInt16 ChState[10];
	UInt16 EqInput[2*2*60];
	UInt16 EqOutput[2*58];
	
} ModemLoggingBuffers_t;

typedef struct
{
	ModemLoggingBuffers_t modem_data_logging[MAX_RX_SLOTS];
	
} ModemLoggingSlotData_t;

typedef enum
{
    LOG_MODEM_DATA_RAW_I_Q	= 0x1,
	LOG_MODEM_DATA_DEROT_IQ = 0x2,
	LOG_MODEM_DATA_CH_EST 	= 0x4,
	LOG_MODEM_DATA_FF_COEF 	= 0x8,
	LOG_MODEM_DATA_CH_STATE	= 0x10,
	LOG_MODEM_DATA_EQ_INPUT = 0x20,
	LOG_MODEM_DATA_EQ_OUT	= 0x40,
...
{  
	...
	UInt32 spirsr[2][16];			// RFSPI_SPIRSR[0-31]
...
#define Nc 						16
#define	ALPHA_DEV				45875       /* 0.70 in Q16 */
#define INE_NOISE               16	    	/* 16.0 scaled as 23,8 */
#define	INE_NOISE_DB32			385 		/*floor(db(16)/2*32) */
#define	UPDATE_CNT_THLD			100
...
#define	ALPHA_N					29491		/* 0.9 in Q15 */
#define DTX_ETOT_TH				(10*32)		/* 10dB in Q5 */
#define	PITCH_TH				(50*32)  	/* in Q5 */
#define ALPHA_PITCH     		29491    	/* 0.9 in Q15 */
#define	ZC_LSB					16
#define	ZC_TH					50
#define	SLIDING_WINDOW_FRAME	150//250
#define	SLIDING_WINDOW_UPDATE	25
#define	SLIDING_WINDOW_LEN		(SLIDING_WINDOW_FRAME/SLIDING_WINDOW_UPDATE)
#define	NOISE_WINDOW_TH			(15*32)  	/* in Q5 */
#define	DTX_DELAY_BUFF_SIZE		4
#define MAX_SLIDING_WINDOW_LEN	16	// 25*10ms*16=>4s
#define	PITCH_TH_LONG_VOWEL		(40*32)
#define	ZC_TH_LONG_VOWEL		20//40
#define	SLIDING_WINDOW_FRAME_VOWEL	50
#define	SLIDING_WINDOW_LEN_VOWEL 	(SLIDING_WINDOW_FRAME_VOWEL/SLIDING_WINDOW_UPDATE)
#define	LONG_VOWEL_HANGOVER		100
#define ONSET_HANG_TIME			80
#define	LEV_TD_FD_TH_DB16		(5*16)
#define	FLAG_SH_TH				640
...
typedef struct
{
	...
	Int32	min_lin;
	Int32	min_lin0;
	Int16	second_tracker_max;
	Int16	second_tracker_min;
	Int32	second_tracker_min_lin;
	Int16	max_buff[MAX_SLIDING_WINDOW_LEN];
	Int16	min_buff[MAX_SLIDING_WINDOW_LEN];
	Int32	min_lin_buff[MAX_SLIDING_WINDOW_LEN];
	Int32	min_lin0_buff[MAX_SLIDING_WINDOW_LEN];
}sliding_window_min_energy_t;

typedef struct
{
	Int16	noise_window_th;
	Int16	pitch_th;
	Int16	dtx_etot_th;
	Int16	dev_thld;
	Int16	update_cnt_thld;
	Int16	zc_th;
}dynamic_noise_tuning_parm_t;
...
typedef struct 
{
	...
	Int16 E_avg[Nc];
	UInt32  En_Q8[Nc];
	UInt32  second_tracker_En_Q8[Nc];						// Alpha voice
	Int16 update_flag;
	Int16 second_tracker_update_flag;						// Alpha voice
	Int16 update_cnt;
	Int16 last_update_cnt;
	Int16 hyster_cnt;
	Int16 Delta_E_Q5;
	Int16 Etot_dB_Q5_buff[DTX_DELAY_BUFF_SIZE];
	Int16 pitch_dtx_state;
  	Int16 pitch_avg_Q5;
  	Int16 pitch_std_avg_Q5;
	Int16 zc;
	Int16 update_flag_min;
 	sliding_window_min_energy_t sliding_window_min_energy[Nc];
	Int16 update_flag_min_fast;
	Int16 noise_est_state;
	Int16 fast_tracker_update_flag_min_fast;
	Int16 second_tracker_update_flag_min_fast;							// Alpha voice	
	Int16 fast_tracker_noise_est_state;
	Int16 second_tracker_noise_est_state;								// Alpha voice
	Int16 dsp_noise_est_state;											// Alpha voice	
	dynamic_noise_tuning_parm_t parms;
	Int16 Etot_avg;
	Int16 onset_flag;
	Int16 onset_hangCnt;
	Int16 onset_gap_in_blk;
	Int16 onset_gap_in_window;
	Int16 update_flag_blk_sum[MAX_SLIDING_WINDOW_LEN];
	Int16 update_flag_window_sum;
	Int16 second_tracker_update_flag_blk_sum[MAX_SLIDING_WINDOW_LEN];	// Alpha voice
	Int16 second_tracker_update_flag_window_sum;						// Alpha voice	
	Int16 pitch_zc_blk_sum[MAX_SLIDING_WINDOW_LEN];
	Int16 pitch_zc_window_sum;
	Int16 long_vowel_hangover_cnt;
	Int16 mode;
	Int16 td_min_buff[MAX_SLIDING_WINDOW_LEN];
	Int16 td_min;
	Int16 fd_min;
	Int16 td_lev;
	Int16 vm_hangover;
...
{    
	...
	union
	{
		...
		UInt16	frame_g711[80];		//voip mode 4: G.711 encoded samples
	}voip_frame;
} VOIP_Buffer_t;        

// VOIP vocoder struct and defines, need to swap with xram
typedef struct 		 				
{                    
	union
	{
		UInt16	amr_enc[0x0380];	//voip amr encoder state, size hard-coded
		UInt16	fr_enc[0x01b8];		//voip fr encoder state, size hard-coded
	}voip_encoder_state;

	union
	{
		UInt16	amr_dec[0x0320];	//voip amr decoder state, size hard-coded
		UInt16	fr_dec[0x00fc];		//voip fr decoder state, size hard-coded
	}voip_decoder_state;

	UInt16 voip_ecan_state[0x0300];

} VOIP_VOCODER_STATE_t;        
...
{// Command						       		Code		Arguments (** = unused command)
 // ========================	       		====		====================================
    ...
    *              
    *              @param  NA; Only VP_SPEECH_MODE_LINEAR_PCM supported.
    *              @param  UInt16 nr_frame;  Number of 20ms frames to record per speech buffer.
...
{// Status                                       Code        Arguments (** = unused status)
 // ========================                   ====        ====================================
    ...
    *              @param  AMR_Mode vp_amr_mode or VP_SPEECH_MODE_LINEAR_PCM;
    *               
    *  \see  VP_Record_Mode_t, VP_Speech_Mode_t, AMR_Mode, COMMAND_AUDIO_ENABLE, COMMAND_AUDIO_CONNECT, 
