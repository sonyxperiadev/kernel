				    ...
				    int volume, AUDIO_GAIN_FORMAT_t gain_format)
{
	...
	if (gain_format == AUDIO_GAIN_FORMAT_mB) {
		...
		if (telephony_dl_gain_dB <
		    -(AUDIO_GetParmAccessPtr()[AUDDRV_GetAudioMode()].
		      voice_volume_max))
...
{
	Log_DebugPrintf(LOGID_AUDIO,
			"AUDCTRL_SaveAudioModeFlag: mode = %d audio_app = %d\n",
			mode, audio_app);
...
Result_t AUDCTRL_StartCapture(unsigned int streamID)
{
	...
	_DBG_(Log_DebugPrintf
	      (LOGID_SOC_AUDIO, "AUDCTRL_StartCapture::streamID=0x%x\n",
	       streamID));
			    ...
			    int arg1, int arg2, int arg3)
{
	...
	case AUDCTRL_HW_WRITE_GAIN:
		...
		case AUDCTRL_PMU_IHF_GAIN:
			pmu_gain = map2pmu_ihf_gain(arg2);
...
*/
	...
	if ((telephonyUseHS == FALSE) && (audioUseHS == FALSE)
		...
		hs_gain =
		    (short)AUDIO_GetParmAccessPtr()[AUDDRV_GetAudioMode()].
		    ext_speaker_pga_l;
	...
	if ((telephonyUseIHF == FALSE) && (audioUseIHF == FALSE)
		...
		ihf_gain =
		    (int)AUDIO_GetParmAccessPtr()[AUDDRV_GetAudioMode()].
