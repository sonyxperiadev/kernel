...
#define	CSL_AMR_WB_FRAME_SIZE				32		// Maximum size of any WB Amr speech recording/playback frame	
...
{ 
    ...
    CSL_AMR_WB_MODE_7k  = 0x10,		//6.60, bit4=1 means WB AMR, [bit3..0] is the actually rate defined in Spec
    ...
    CSL_VOIP_FR			= 0x2000,	   	//VOIP FR (GSM 06.10), for FR/AMR, bit0=[0/1]=[voip_dtx_enable=0/1]
...
typedef struct       // linear PCM speech Recording/Playback data for a 20ms frame
{
    ...
    UInt16 param[CSL_AMR_WB_FRAME_SIZE];	///< MSB first 16 bits per word packing format    
...
{    
	...
	union
	{
		...
		CSL_VR_Frame_G711_t	frame_g711[2];		//voip mode 4: G.711 encoded samples
		CSL_VR_Frame_AMR_WB_t frame_amr_wb;		//voip mode 6: AMR-WB
	} voip_frame;
} CSL_VOIP_Buffer_t;        
...
/**
*
*   CSL_ReadULVoIPData reads VoIP from DSP shared memory
*
*   @param    codec_type	(in)		codec type
