...
//        memcpy(dest, source2, step);
    ...
    if(bitPerSample > AUDIO_16_BIT_PER_SAMPLE && bitPerSample <= AUDIO_32_BIT_PER_SAMPLE)
    {
...
AUDQUE_Queue_t* AUDQUE_Create(UInt8 *baseAddr, 
							  ...
							  UInt32 blockSize)
{
	AUDQUE_Queue_t *que = (AUDQUE_Queue_t *)OSHEAP_Alloc (sizeof(AUDQUE_Queue_t));
...
void AUDQUE_UpdateReadPtr (AUDQUE_Queue_t *aq)
{
...
#ifdef DEBUG_RING_BUFFER
	{
		...
		for (j = 0;j < k; j++)
		{
			if ( *(buf+ j) != i_hack++)
			{
				Log_DebugPrintf(LOGID_SOC_AUDIO, "richlu AUDQUE_Write found driver error! i_hack = %d, j = %d, buf = %d\n", i_hack, j, *(buf+ j));
			}
	...
	if (aq->readPtr <= aq->writePtr && nextReadPtr > aq->writePtr)
	{
		...
		{
			Log_DebugPrintf(LOGID_SOC_AUDIO, "AUDQUE_UpdateReadPtr flush the queue due to underflow.\n");
...
UInt32 AUDQUE_GetLoad (AUDQUE_Queue_t *aq)
{
	...
	//_DBG_(Log_DebugPrintf(LOGID_AUDIO, "AUDQUE_GetLoad:: aq->writePtr 0x%x, aq->readPtr 0x%x, aq->base 0x%x, aq->bottom = 0x%x, load %d \n",
