...
UInt32 csl_audio_capture_init( CSL_AUDIO_DEVICE_e source, CSL_AUDIO_DEVICE_e sink )
{
	...
	_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_init:: \n"));
    ...
    {
        _DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_init:: Error, Obtain channel failed.\n"));
		return CSL_AUDVOC_STREAM_NONE;
    }

	_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_init:: DMA channel for audio path capture 0x%x\n", dmaChAudio));
...
{
	CSL_AUDVOC_Drv_t	*audDrv = NULL;
	_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_deinit::release DMA channel, streamID=%d.\n", streamID));	
						...
						)
{
	...
	_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_configure:: DMA streamID = 0x%x, sampleRate =0x%x, numChannels = 0x%x, bits/sample = 0x%x numbBuffers = 0x%x, blockSize = 0x%x\n", 
...
{
	CSL_AUDVOC_Drv_t	*audDrv = NULL;
	_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_start:: DMA streamID = %d\n", streamID));
	...
	{
		_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "csl_audvoc_capture_start::Error, Start transfer failed.\n"));
	}

	if (streamID == CSL_AUDVOC_STREAM_BTW)
	{
		_DBG_(Log_DebugPrintf(LOGID_AUDIO, "csl_audio_capture_start BT wb tap \n"));
...
Result_t csl_audio_capture_pause( UInt32 streamID )
{
	...
	_DBG_(Log_DebugPrintf(LOGID_AUDIO, " csl_audvoc_capture_pause:: streamID = 0x%x.\n", streamID));
...
{
	_DBG_(Log_DebugPrintf(LOGID_AUDIO, " csl_audvoc_capture_resume:: streamID = 0x%x.\n", streamID));
...
Result_t csl_audio_capture_stop( UInt32 streamID )
{
	...
	_DBG_(Log_DebugPrintf(LOGID_AUDIO, " csl_audvoc_capture_stop:: DMA streamID = %d\n", streamID));
	...
	{
		_DBG_(Log_DebugPrintf(LOGID_AUDIO, "csl_audvoc_capture_stop::Error, Stop transfer failed.\n"));
	}
			...
			OSDAL_DMA_CALLBACK dmaCB)
{
    ...
    _DBG_(Log_DebugPrintf(LOGID_AUDIO, "ConfigDMA:: channel: 0x%x, numBlocks = 0x%x, blockSize = 0x%x\n", channel, numBlocks, blockSize));
    ...
    {
        _DBG_(Log_DebugPrintf(LOGID_AUDIO, "ConfigDMA:: Error, Configure channel failed.\n"));
        return RESULT_ERROR;
    }
	...
	switch (srcClient)
	{
		...
		default:
			_DBG_(Log_DebugPrintf(LOGID_AUDIO, "ConfigDMA:: Error, Wrong source client, srcClient = 0x%x.\n", srcClient));
			break;
	...
	for (i = 0; i < numBlocks; i++)
	{
		...
		cirBufList[i].buffers[0].destAddr = (UInt32)current;
    ...
    {
        _DBG_(Log_DebugPrintf(LOGID_AUDIO, "ConfigDMA:: Error, Add buffer failed.\n"));
        return RESULT_ERROR;
    }
...
static void BTW_DMA_CB(OSDAL_DMA_CALLBACK_STATUS status)
{
	...
	_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "BTW_DMA_CB:: DMA callback.\n"));

	if (status != OSDAL_ERR_OK)
	{
		_DBG_(Log_DebugPrintf(LOGID_SOC_AUDIO, "BTW_DMA_CB:: Fatal error! DMA transfer failure.\n"));
		return;
	}
