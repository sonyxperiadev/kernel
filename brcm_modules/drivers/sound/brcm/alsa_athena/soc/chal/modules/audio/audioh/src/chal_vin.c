...
cVoid chal_audio_vinpath_int_enable(CHAL_HANDLE handle, cUInt16 thr_int_enable, cUInt16 err_int_enable )
{
    ...
    {
        reg_val |= AUDIOH_AUDIO_INTC_VIN_INTEN_MASK;
    }
    else
    {
        reg_val &= ~AUDIOH_AUDIO_INTC_VIN_INTEN_MASK;
    ...
    {
        reg_val |= AUDIOH_AUDIO_INTC_VIN_FIFO_ERRINT_EN_MASK;
    }
    else
    {
        reg_val &= ~AUDIOH_AUDIO_INTC_VIN_FIFO_ERRINT_EN_MASK;
...
cVoid chal_audio_vinpath_dma_enable(CHAL_HANDLE handle, Boolean dma_enable )
{
    ...
    {
        reg_val |= AUDIOH_DMA_CTL_VIN_DMA_EN_MASK;
    }
    else
    {
        reg_val &= ~AUDIOH_DMA_CTL_VIN_DMA_EN_MASK;
                                                                                        ...
                                                                                        UInt32 *dma_addr )
{
    ...
    *dma_addr = (UInt32)(base) + AUDIOH_VIN_FIFO_DATA0_OFFSET;
...
cUInt32 chal_audio_vinpath_read_fifo_status(CHAL_HANDLE handle)
{
    ...
    status = BRCM_READ_REG(base, AUDIOH_VIN_FIFO_STATUS);
...
cUInt32 chal_audio_vinpath_read_fifo(CHAL_HANDLE handle, cUInt32 *src, cUInt32 length, Boolean ign_udf)
{
    ...
    if(ign_udf == CHAL_AUDIO_STOP_ON_FIFO_UNDERFLOW)
    {
        for(n = 0; n < length; n++)
        {
            ...
            *src++ = BRCM_READ_REG(base, AUDIOH_VIN_FIFO_DATA0);
        }
    }
    else
    {
...
cUInt32 chal_audio_vinpath_read_int_status(CHAL_HANDLE handle)
{
    ...
    if(reg_val & AUDIOH_AUDIO_INT_STATUS_VIN_FIFO_ERR_MASK)
    {
        status |= CHAL_AUDIO_FIFO_STATUS_ERR_INT;
    }

    //Check VIN path FIFO thereshold interrupt
    if(reg_val & AUDIOH_AUDIO_INT_STATUS_VIN_INT_MASK)
    {
...
cVoid chal_audio_vinpath_int_clear(CHAL_HANDLE handle, Boolean thr_int, Boolean err_int)
{
    ...
    if(thr_int == TRUE)
    {
        reg_val |= AUDIOH_AUDIO_INT_STATUS_VIN_INT_MASK;
    }

    if(err_int == TRUE)
    {
        reg_val |= AUDIOH_AUDIO_INT_STATUS_VIN_FIFO_ERR_MASK;
    }
