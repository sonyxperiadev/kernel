...
typedef struct
{
	NOT_USE_Dsp_SharedMem_t	NOT_USE_CP_shared_memory;
	AP_SharedMem_t			AP_shared_mememory;						// Define global structure variable and put them at the dsp-arm shared memory
} Common_Shared_memory_t;

#pragma arm section zidata = "shared_rip_mem_sect"
static Common_Shared_memory_t shared_mem_region;
      
#pragma arm section


#ifdef UNDER_LINUX
static AP_SharedMem_t 			*global_shared_mem = NULL;		// Define pointers to global strcuture of the shared memory
static AP_SharedMem_t 			*dsp_shared_mem = NULL;		// Define pointers to global strcuture of the shared memory
...
#else
static AP_SharedMem_t 			*dsp_shared_mem = &shared_mem_region.AP_shared_mememory;		// Define pointers to global strcuture of the shared memory
static AP_SharedMem_t			*shared_mem = &shared_mem_region.AP_shared_mememory; 
static AP_SharedMem_t 			*pg27_shared_mem = &shared_mem_region.AP_shared_mememory;
static AP_SharedMem_t 			*pg28_shared_mem = &shared_mem_region.AP_shared_mememory;
...
extern int IPC_AudioControlApSend(char *buff, UInt32 len);
//******************************************************************************
//
...
{
	IPC_AudioControlApSend((char *)cmd_msg, sizeof(CmdQ_t));
}
...
AP_SharedMem_t *SHAREDMEM_GetDsp_SharedMemPtr()		// Return pointer to 32-bit aligned shared memory
{
    ...
    {
        global_shared_mem = ioremap_nocache(DSP_SH_BASE, DSP_SH_SIZE);
...
AP_SharedMem_t *SHAREDMEM_GetSharedMemPtr()// Return pointer to shared memory
{
   ...
   shared_mem = ((UInt8*)global_shared_mem + sizeof(NOT_USE_Dsp_SharedMem_t));
#endif
	return shared_mem;
}	
...
AP_SharedMem_t *SHAREDMEM_GetPage27SharedMemPtr()// Return pointer to page 27 shared memory
{
   ...
   pg27_shared_mem = ((UInt8*)global_shared_mem + sizeof(NOT_USE_Dsp_SharedMem_t));
#endif
...
AP_SharedMem_t *SHAREDMEM_GetPage28SharedMemPtr()// Return pointer to page 28 shared memory
{
   ...
   pg28_shared_mem = ((UInt8*)global_shared_mem + sizeof(NOT_USE_Dsp_SharedMem_t));
#endif
...
static const UInt16 sVoIPDataLen[5] = {0, 322, 160, 38, 166};
...
void SHAREDMEM_WriteDL_VoIP_Data(UInt16 codec_type, UInt16 *pSrc)
{
	...
	if (index > 4)
  		TRACE_Printf_Sio( "=====SHAREDMEM_WriteDL_VoIP_Data, invalid codec type!!!\r\n");
	else
...
UInt8 SHAREDMEM_ReadUL_VoIP_Data(UInt16 codec_type, UInt16 *pDst)
{
	...
	index = (codec_type & 0xf000) >> 12;
	if (index > 4)
  		TRACE_Printf_Sio( "=====SHAREDMEM_ReadUL_VoIP_Data, invalid codec type!!!\r\n");
	else
		data_len = sVoIPDataLen[index];
