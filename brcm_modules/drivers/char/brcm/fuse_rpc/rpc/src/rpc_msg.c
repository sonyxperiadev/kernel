...
static RPC_XdrInfo_t ACK_dscrm[] = 
{
	...
	{ (int)MSG_CAPI2_ACK_RSP,_T("MSG_CAPI2_ACK_RSP"), (xdrproc_t)xdr_RPC_Ack_t, sizeof(RPC_Ack_t) },
	/* Add other modules message to serialize/deserialize routine map */
	{ __dontcare__, "",NULL_xdrproc_t, 0,0 } 
};
...
{
	if( xdr_u_int32_t(xdrs, &data->ackUsrData) &&
