...
Int8 eps[IPC_EndpointId_Count]={-1, INTERFACE_CAPI2, INTERFACE_CAPI2, INTERFACE_PACKET, INTERFACE_PACKET, INTERFACE_LOGGING, INTERFACE_LOGGING, -1,-1,INTERFACE_CSD, INTERFACE_CSD, INTERFACE_SS_IPC40, INTERFACE_SS_IPC40};
...
static Int8 GetInterfaceType(IPC_EndpointId_T epId)
{
	...
	else if(epId == IPC_EP_SSIPC40Ap || epId == IPC_EP_SSIPC40Cp)
		return INTERFACE_SS_IPC40;
...
RPC_Result_t RPC_IPC_EndPointInit(RpcProcessorType_t rpcProcType)
{
	...
	if (rpcProcType == RPC_COMMS)
	{
		...
		IPC_EndpointRegister(IPC_EP_SSIPC40Cp,		RPC_FlowCntrl,	RPC_BufferDelivery,4);
	}
	else
	{
		...
		IPC_EndpointRegister(IPC_EP_SSIPC40Ap,			RPC_FlowCntrl,	RPC_BufferDelivery,4);
...
RPC_Result_t RPC_IPC_Init(RpcProcessorType_t rpcProcType)
{
	...
	for(itype = 0; itype < INTERFACE_TOTAL;itype++)
	{
		...
		else if(itype == INTERFACE_SS_IPC40)
		{

			ipcBufList[itype].max_pkts[0]=CFG_RPC_SSIPC40_MAX_PACKETS;
			ipcBufList[itype].max_pkts[1]=CFG_RPC_SSIPC40_MAX_PACKETS1;
			ipcBufList[itype].max_pkts[2]=CFG_RPC_SSIPC40_MAX_PACKETS2;
			ipcBufList[itype].max_pkts[3]=CFG_RPC_SSIPC40_MAX_PACKETS3;

			ipcBufList[itype].pkt_size[0]=CFG_RPC_SSIPC40_PKT_SIZE;
			ipcBufList[itype].pkt_size[1]=CFG_RPC_SSIPC40_PKT_SIZE1;
			ipcBufList[itype].pkt_size[2]=CFG_RPC_SSIPC40_PKT_SIZE2;
			ipcBufList[itype].pkt_size[3]=CFG_RPC_SSIPC40_PKT_SIZE3;

			ipcBufList[itype].start_threshold=CFG_RPC_SSIPC40_START_THRESHOLD;
			ipcBufList[itype].end_threshold=CFG_RPC_SSIPC40_END_THRESHOLD;
			
			ipcBufList[itype].srcEpId = (rpcProcType == RPC_COMMS)?IPC_EP_SSIPC40Cp:IPC_EP_SSIPC40Ap;
			ipcBufList[itype].destEpId = (rpcProcType == RPC_COMMS)?IPC_EP_SSIPC40Ap:IPC_EP_SSIPC40Cp;
		}
		else
			xassert(0,itype);
