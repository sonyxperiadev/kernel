...
typedef struct 
{
	...
	BCMLOG_Fifo_t		output_fifo ;			///< output FIFO
}	BCMLOG_Module_t ;
...
static int __init BCMLOG_ModuleInit(void)
{
	...
	if( !fifobuf )
	{
		BCMLOG_PRINTF( BCMLOG_CONSOLE_MSG_ERROR, "allocate FIFO buffer failed\n" ) ;
        return -1 ;
	}

	BCMLOG_FifoInit( &g_module.output_fifo, fifobuf, BCMLOG_OUTPUT_FIFO_MAX_BYTES ) ;
                        				...
                        				unsigned short inSender )
{
	...
	BCMLOG_Output( &g_module.output_fifo, kbuf_mtt, mttFrameSize ) ;
                                        ...
                                        LogSignalCompress_t inCompress )
{
    ...
    else
    {
        ...
        BCMLOG_Output( &g_module.output_fifo, frame_head, frame_head_size );
        // then payload...(located on stack; do not free when done)
        BCMLOG_Output( &g_module.output_fifo, mtt_payload, mtt_payload_size );
        // then signal if applicable...
        if ( bufToSend && (bufToSendSize > 0) )
        {
            BCMLOG_Output( &g_module.output_fifo, bufToSend, bufToSendSize );
        }
        // then frame end (located on stack; do not free when done)
        BCMLOG_Output( &g_module.output_fifo, frame_end, frame_end_size );
...
void BCMLOG_HandleCpLogMsg( const char *buf, int size )
{
	...
	{
		unsigned long irql = AcquireOutputLock( ) ;
		BCMLOG_Output( &g_module.output_fifo, buf, size ) ;
		ReleaseOutputLock( irql ) ;
	}
}
