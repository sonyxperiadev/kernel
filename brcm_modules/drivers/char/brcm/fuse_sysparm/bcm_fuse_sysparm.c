...
// **FIXME** MAG - should have more generic build flag 
// to specify building from CIB vs 2153SDB (eg CONFIG_BCM_CP_CIB)
...
static UInt32 SYSPARM_GePAtByIndex(char *name, unsigned int size, unsigned int flag)
{
    ...
    pr_err("[sysparm]: SYSPARM_GePAtByIndex: sysparm not found\n");
    return 0;
}
...
//******************************************************************************
//
// Function Name: SYSPARM_GetAudioParmAccessPtr
//
...
SysAudioParm_t* SYSPARM_GetAudioParmAccessPtr(UInt8 AudioMode)
{
	...
	if(loaded_audio_parm_table)
	    return &audio_parm_table[AudioMode];
    ...
    memcpy(&audio_parm_table[0], &audio_parm_ptr[0], sizeof(audio_parm_table));
...
//
// Description:   Get access pointer to mmaudio_parm structure 
//                based on the audio tuning mode.
//
//******************************************************************************
SysIndMultimediaAudioParm_t* APSYSPARM_GetMultimediaAudioParmAccessPtr(void)
{
    ...
    memcpy(&mmaudio_parm_table[0], &mmaudio_parm_ptr[0], sizeof(mmaudio_parm_table));
...
//******************************************************************************
//
// Function Name: SYSPARM_GetMultimediaAudioParmAccessPtr
//
// Description:   Get access pointer to multimedia audio sysparm structure 
//
//******************************************************************************
SysIndMultimediaAudioParm_t* SYSPARM_GetMultimediaAudioParmAccessPtr( UInt8 inAudioMode )
{
    ...
    if ( inAudioMode >= AUDIO_MODE_NUMBER )
    {
        pr_err("[sysparm]: invalid audio mode for mmaudio_parm\n");
        return 0;
    }
    ...
    memcpy(&mm_audio_parm_table[inAudioMode], &mm_audio_parm_ptr[inAudioMode], sizeof(SysIndMultimediaAudioParm_t));
    iounmap(mm_audio_parm_ptr);
    return &mm_audio_parm_table[inAudioMode];
}
...
#endif //CONFIG_BRCM_FUSE_IPC_CIB
