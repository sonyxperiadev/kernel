...
/**
	...
	@param		inClientInfoPtr (in) pointer to the client Info
	@param		inCallIndex (in) Index of call
	@param		outCnapNamePtr (out) pointer to the CNAP name string (UCS2 capable).
	@param		outNameLengthPtr (out) CNAP name string length
	@param		outNameCodingPtr (out) CNAP name coding format
	@return		None 
	@note 
...
void  CcApi_GetCNAPName(ClientInfo_t*	inClientInfoPtr,
						UInt8			inCallIndex,
						const UInt8**	outCnapNamePtr,
						UInt8*			outNameLengthPtr,
						ALPHA_CODING_t* outNameCodingPtr);
