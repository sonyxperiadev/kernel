...
typedef struct
{
	...
	PBK_EXT_Number_t		addi_num[MAX_NUM_OF_TYPE1_ANR_FILE];		///< Additional Number (EF-ANR) and Names (EF-AAS)
	...
	PBK_API_Name_t			email[MAX_NUM_OF_TYPE1_EMAIL_FILE];			///< Email alpha (EF-EMAIL)
...
{	
	...
	UInt8 sne_len[MAX_NUM_OF_ADN_SET];	///< Number of bytes allowed in Second Name EF-SNE (max only one EF-SNE per PBR record). 0 if Second Name not provisioned in USIM
