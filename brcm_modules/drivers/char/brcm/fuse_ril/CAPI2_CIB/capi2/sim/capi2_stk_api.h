...
/**
	...
	@param		tid (in) Unique exchange/transaction id which is passed back in the response
	@param		clientID (in) ID of the STK module (currently not used, set to 0)
	@param		dsReq (in) data service request data
	@note
	@n@b Responses 
...
/**
	Send an Browsing Status Event to the SIM.
	
	@param		tid (in) Unique exchange/transaction id which is passed back in the response
	@param		clientID (in) Client ID
	@param		status (in) pointer to browser status code received from network
    @param      len (in) length of browser status code
...
**/
void CAPI2_SATK_SendBrowsingStatusEvent(UInt32 tid, UInt8 clientID, UInt8 *status, UInt8 len);
