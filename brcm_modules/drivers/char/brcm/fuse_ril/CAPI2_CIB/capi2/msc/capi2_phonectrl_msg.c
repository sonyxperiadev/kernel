...
XDR_ENUM_FUNC(HAL_EM_BATTMGR_ErrorCode_en_t)
XDR_ENUM_FUNC(HAL_EM_BATTMGR_Events_en_t)
...
xdr_HAL_EM_BatteryLevel_t( XDR* xdrs, HAL_EM_BatteryLevel_t* data)
{
	...
	if(	XDR_ENUM(xdrs,  &data->eventType, HAL_EM_BATTMGR_Events_en_t) &&
...
static const struct xdr_discrim CAPI2_MSElement_t_dscrm[] = {
        ...
        { (int)MS_STACK_ELEM_NW_MEAS_RESULT,_T("MS_STACK_ELEM_NW_MEAS_RESULT"),	(xdrproc_t)xdr_default_proc, 0,		NULL_capi2_proc_t, (xdrproc_t)xdr_default_proc ,0},
...
bool_t xdr_ADC_StartRsp_t(XDR* xdrs, ADC_StartRsp_t *param)
{
	...
	if(	_xdr_u_int16_t(xdrs, &param->badr_value,"badr_value") &&
		xdr_u_int32_t(xdrs, (u_int32_t *) &param->Context)
		)
		return(TRUE);
	else
		return(FALSE);
