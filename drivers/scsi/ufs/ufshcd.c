...
#if defined(CONFIG_ARCH_SONY_YOSHINO) || defined(CONFIG_ARCH_SONY_TAMA) || 1
	UFS_FIX(UFS_VENDOR_SKHYNIX, UFS_ANY_MODEL,
			       ...
			       struct ufs_dev_desc *dev_desc)
{
	...
	str_desc_buf[QUERY_DESC_MAX_SIZE] = '\0';
	strlcpy(dev_desc->revision, (str_desc_buf + QUERY_DESC_HDR_SIZE),
		min_t(u8, str_desc_buf[QUERY_DESC_LENGTH_OFFSET],
		      MAX_REVISION_LEN));
	...
	dev_info(hba->dev, "%s : vid=%04x, model=%s, spec ver=%04x , "
		"fw ver=%s\n", __func__, hba->dev_info.w_manufacturer_id,
		 model, hba->dev_info.w_spec_version, dev_desc->revision);
