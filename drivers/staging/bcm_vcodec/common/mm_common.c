...
static DEFINE_MUTEX(mm_fmwk_mutex);
static LIST_HEAD(mm_dev_list);
static LIST_HEAD(mm_file_list);
static struct workqueue_struct *single_wq;
static char *single_wq_name = "mm_wq";
static wait_queue_head_t mm_queue;
...
static void mm_common_add_job(struct work_struct *work)
{
...
void mm_common_wait_job(struct work_struct *work)
{
	...
	if ((from == NULL) && list_is_singular(&to_filp->write_head)) {
		mm_interlock_completion(to);
		}
	else if (from != NULL) {
		struct file_private_data *from_filp = from->filp;
		if (list_is_singular(&from_filp->write_head))
			mm_interlock_completion(from);
		}
	mutex_unlock(&mm_common_mutex);
}
...
static void mm_common_read_job(struct work_struct *work)
{
	struct job_maint_work *maint_job = container_of(work, \
					struct job_maint_work,\
					work);
...
static void mm_common_release_jobs(struct work_struct *work)
{
	struct job_maint_work *maint_job = container_of(work, \
					struct job_maint_work, \
					work);
...
static void mm_common_add_file(struct work_struct *work)
{
	struct job_maint_work *maint_job = container_of(work, \
					struct job_maint_work, \
					work);
...
void mm_interlock_completion(struct dev_job_list *job)
{
	...
	if (job->successor)
		mm_interlock_completion(job->successor);
	...
	if (0 == list_empty(&filp->write_head)) {
		...
		if ((wait_job->job.type == INTERLOCK_WAITING_JOB) &&
			(wait_job->predecessor == NULL)) {
				mm_interlock_completion(wait_job);
				}
		if (wait_job->job.type != INTERLOCK_WAITING_JOB) {
...
static int mm_file_fsync(struct file *filp, loff_t p1, loff_t p2, int datasync)
{
	struct file_private_data *private = filp->private_data;
	struct mm_common *common = private->common;
	struct interlock il;
						...
						MM_PROF_HW_IFC *prof_param)
{
	...
	for (i = 0; i < count; i++) {
		common->mm_core[i] = mm_tmp_core_init(common, name, &core_param[i]);
...
//#else
	common->mm_dvfs = NULL;
//#endif
	common->mm_prof = mm_tmp_prof_init(common, name, prof_param);
...
void mm_tmp_fmwk_unregister(void *dev_name)
{
	...
	if (common->mm_prof)
		mm_tmp_prof_exit(common->mm_prof);
#ifdef CONFIG_KONA_PI_MGR
	if (common->mm_dvfs)
	...
	for (i = 0; \
		...
		i++)
		mm_tmp_core_exit(common->mm_core[i]);
	if (common->debugfs_dir)
