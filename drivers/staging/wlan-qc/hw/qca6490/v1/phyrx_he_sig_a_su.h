...
/* Description		PHYRX_HE_SIG_A_SU_0_PHYRX_HE_SIG_A_SU_INFO_DETAILS_CP_LTF_SIZE
			...
			<enum 1 TwoX_LTF_0_8CP> 2x LTF + 0.8 탎 CP (See note
			for proprietary mode)

			<enum 2 TwoX_LTF_1_6CP> 2x LTF + 1.6 탎 CP (See note
			for proprietary mode)
			...
			When DCM == 0 OR STBC == 0: 4x LTF + 3.2 탎 CP

			When DCM == 1 AND STBC == 1: 4x LTF + 0.8 탎 CP. Note:
