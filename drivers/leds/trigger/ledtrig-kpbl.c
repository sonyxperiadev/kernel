...
static struct delayed_work kpbl_work;
...
static void kpbl_trig_work(struct work_struct *work)
{
	...
	__led_set_brightness(data->led_cdev, LED_OFF);
}
...
static void kpbl_trig_late_resume(struct early_suspend *h)
{
	...
	data = container_of(h, struct kpbl_trig_data, suspend);
	__led_set_brightness(data->led_cdev, LED_FULL);
	schedule_delayed_work(&data->del_work,
				msecs_to_jiffies(data->duration));
}
#endif
...
static void kpbl_work_func(struct work_struct *work)
{
	if (tp_key_register) {
		(*tp_key_register)(&notify);
		printk(KERN_INFO "%s--kpbl registerred.\n", __func__);
	} else
		printk(KERN_INFO "%s--no kpbl registerred.\n", __func__);
...
static int __init kpbl_trig_init(void)
{
	...
	if (ret)
		return ret;
	INIT_DELAYED_WORK(&kpbl_work, kpbl_work_func);
	schedule_delayed_work(&kpbl_work, HZ*5);
}
