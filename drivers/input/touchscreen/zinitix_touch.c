...
const u8 *m_pFirmware[TSP_TYPE_COUNT]
		= {(u8 *)m_firmware_data_02, (u8 *)m_firmware_data_03,};
u8 m_FirmwareIdx;
...
static inline s32 ts_retry_send(struct i2c_client *client,
						u8 *buf, u16 length)
{
	s32 ret;
	int i;
...
static bool i2c_checksum(struct zinitix_touch_dev *touch_dev,
					s16 *pChecksum, u16 wlength)
{
	s16 checksum_result;
	s16 checksum_cur;
	int i;
...
static bool ts_read_coord(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev->cap_info.i2s_checksum)
		if (ts_write_reg(touch_dev->client,
				ZINITIX_I2C_CHECKSUM_WCNT,
				(sizeof(struct _ts_zinitix_point_info)/2))
				!= I2C_SUCCESS){
...
continue_check_point_data:
	...
	if (touch_dev->touch_info.status == 0x0) {
		/*zinitix_debug_msg("periodical esd
		repeated int occured\r\n");*/
	...
	if (ts_write_cmd(touch_dev->client, ZINITIX_CLEAR_INT_STATUS_CMD)
				!= I2C_SUCCESS)
		return false;
	udelay(DELAY_FOR_SIGNAL_DELAY);
/*zinitix_debug_msg("ts_read_coord-\r\n");*/
...
retry_power_sequence:
	if (ts_write_reg(touch_dev->client, 0xc000, 0x0001)
			!= I2C_SUCCESS){
		zinitix_printk("power sequence error (vendor cmd enable)\n");
		goto fail_power_sequence;
	}
	udelay(10);
	if (ts_read_data(touch_dev->client, 0xcc00,
				(u8 *)&chip_code, 2) < 0) {
	...
	udelay(10);
	if (ts_write_cmd(touch_dev->client, 0xc004)
			!= I2C_SUCCESS) {
		zinitix_printk("power sequence error (intn clear)\n");
		goto fail_power_sequence;
	}
	udelay(10);
	if (ts_write_reg(touch_dev->client, 0xc002, 0x0001)
			!= I2C_SUCCESS) {
		zinitix_printk("power sequence error (nvm init)\n");
		goto fail_power_sequence;
	}
	mdelay(2);
	if (ts_write_reg(touch_dev->client, 0xc001, 0x0001)
			!= I2C_SUCCESS) {
...
void touch_ctrl_regulator(unsigned char on_off)
{
	...
	if (tsp_regulator_3_3 == NULL){
		...
		ret = regulator_set_voltage(tsp_regulator_3_3,
			3000000,3000000);
		printk("--> regulator_set_voltage ret = %d\n", ret);
		if (ret) {
			pr_err("can not set voltage TSP AVDD 3.3V,
				ret=%d\n", ret);
	...
	if (ctl == POWER_OFF) {
		if (regulator_is_enabled(tsp_regulator_3_3)) {
			...
			printk(" --> 3.3v regulator_disable ret = %d\n",
				ret);
			if (ret) {
				pr_err("can not disable TSP AVDD 3.3V,
				ret=%d\n", ret);
				return ret;
			}
		}
		...
		if (!regulator_is_enabled(tsp_regulator_3_3)) {
			...
			printk(
				" --> 3.3v regulator_enable ret = %d\n",
				ret);
			if (ret) {
				pr_err("can not enable TSP AVDD 3.3V,
				ret=%d\n", ret);
				return ret;
			}
		}
...
void touch_ctrl_regulator(unsigned char on_off)
{
	int ret;
		...
		if (touch_regulator == NULL) {
			...
			(touch_regulator,3300000,3300000);*/
...
static bool ts_mini_init_touch(struct zinitix_touch_dev *touch_dev)
{
	...
	if (chip_check_sum != 0x55aa) {
		zinitix_printk(
			"firmware checksum error(0x%04x)\r\n",
			chip_check_sum);
		goto fail_mini_init;
	}
#endif
	...
	if (ts_write_reg(touch_dev->client,
		...
		(u16)(touch_dev->cap_info.x_resolution))
			!= I2C_SUCCESS)
		goto fail_mini_init;

	if (ts_write_reg(touch_dev->client,
		ZINITIX_Y_RESOLUTION,
		(u16)(touch_dev->cap_info.y_resolution))
			!= I2C_SUCCESS)
		goto fail_mini_init;
	...
	u16 curVersion, u16 curMinorVersion, u16 curRegVersion, u16 curHWID)
{
	...
	newVersion = (u16) (firmware_data[52] |	\
		(firmware_data[53]<<8));
	newVersion = newVersion&0xff;
	newMinorVersion = (u16) (firmware_data[56] |	\
		(firmware_data[57]<<8));
	newRegVersion = (u16) (firmware_data[60] |	\
		(firmware_data[61]<<8));
	newChipCode = (u16) (firmware_data[64] |	\
		(firmware_data[65]<<8));
	if (touch_dev->cap_info.is_zmt200 == 0)
		newHWID = (u16) (firmware_data[0x6b12] |	\
		(firmware_data[0x6b13]<<8));
	else
		newHWID = (u16) (firmware_data[0x57d2] |	\
		(firmware_data[0x57d3]<<8));
...
retry_upgrade:
	...
	if (ts_write_cmd(touch_dev->client,
	ZINITIX_INIT_FLASH) != I2C_SUCCESS) {
...
retry_init:
	...
	if (ts_check_need_upgrade(touch_dev,
	...
	touch_dev->cap_info.ic_revision
		= (u16)ic_revision;
	...
	if (ts_write_reg(touch_dev->client,
		ZINITIX_X_RESOLUTION, (u16)(touch_dev->cap_info.x_resolution))
			!= I2C_SUCCESS)
		goto fail_init;

	if (ts_write_reg(touch_dev->client,
		ZINITIX_Y_RESOLUTION, (u16)(touch_dev->cap_info.y_resolution))
			!= I2C_SUCCESS)
		goto fail_init;
...
fail_init:
	...
	if (++retry_cnt <= ZINITIX_INIT_RETRY_CNT) {
		...
		zinitix_debug_msg(
			"retry to initiallize(retry cnt = %d)\r\n",
			retry_cnt);
...
static void zinitix_parsing_data(struct zinitix_touch_dev *touch_dev)
{
	...
	if (touch_dev->work_proceedure != TS_NO_WORK) {
		zinitix_debug_msg(
			"zinitix_touch_thread : [warning] other process occupied..(%d)\r\n",
			touch_dev->work_proceedure);
		udelay(DELAY_FOR_SIGNAL_DELAY);
		if (!gpio_get_value(touch_dev->int_gpio_num)) {
			ts_write_cmd(touch_dev->client,
	...
	if (!read_result) {
		zinitix_debug_msg(
			"couldn't read touch_dev coord. maybe chip is dead\r\n");
		ts_power_control(touch_dev, POWER_OFF);
		ts_power_control(touch_dev, POWER_ON_SEQUENCE);
		...
		for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
			...
			if (zinitix_bit_test(prev_sub_status, SUB_BIT_EXIST)) {
				...
				input_report_abs(touch_dev->input_dev,
					ABS_MT_POSITION_X,
					touch_dev->
					reported_touch_info.coord[i].x);
				input_report_abs(touch_dev->input_dev,
					ABS_MT_POSITION_Y,
					touch_dev->
					reported_touch_info.coord[i].y);
				input_mt_sync(touch_dev->input_dev);
#endif
	...
	if (zinitix_bit_test(touch_dev->
			touch_info.status, BIT_PALM_REJECT)) {
	...
	for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			/*zinitix_debug_msg("finger [%02d] x = %d,
			y = %d\r\n", i, x, y);*/
...
static void zinitix_late_resume(struct early_suspend *h)
{
	...
	/*touch_dev = container_of(h,
	struct zinitix_touch_dev, early_suspend);*/
...
{
	struct zinitix_touch_dev *touch_dev = misc_touch_dev;
	/*touch_dev = container_of(h, struct zinitix_touch_dev,
	early_suspend);*/
...
static bool ts_set_touchmode(u16 value)
{
	...
	if (value == TOUCH_DND_MODE) {
		if (ts_write_reg(misc_touch_dev->client,
			ZINITIX_DND_N_COUNT, SEC_DND_N_COUNT)
			!= I2C_SUCCESS)
			printk(KERN_INFO
			"[zinitix_touch] TEST Mode : Fail to set ZINITIX_DND_N_COUNT %d.\r\n",
			SEC_DND_N_COUNT);
		if (ts_write_reg(misc_touch_dev->client,
			ZINITIX_AFE_FREQUENCY, SEC_DND_FREQUENCY)
			!= I2C_SUCCESS)
			printk(KERN_INFO
			"[zinitix_touch] TEST Mode : Fail to set ZINITIX_AFE_FREQUENCY %d.\r\n",
			SEC_DND_FREQUENCY);
	...
	printk(KERN_INFO
		"[zinitix_touch] tsp_set_testmode, touchkey_testmode = %d\r\n",
	...
	if (misc_touch_dev->touch_mode != TOUCH_POINT_MODE) {
		if (ts_write_reg(misc_touch_dev->client,
			ZINITIX_DELAY_RAW_FOR_HOST,
			RAWDATA_DELAY_FOR_HOST) != I2C_SUCCESS)
			zinitix_printk(KERN_ERR
				"Fail to set ZINITIX_DELAY_RAW_FOR_HOST.\r\n");
	...
	if (ts_write_reg(misc_touch_dev->client,
		...
		printk(KERN_INFO
			"[zinitix_touch] TEST Mode : Fail to set ZINITX_TOUCH_MODE %d.\r\n",
			misc_touch_dev->touch_mode);
...
static void not_support_cmd(void *device_data)
{
	struct zinitix_touch_dev *info
		= (struct zinitix_touch_dev *)device_data;
	char buff[16] = {0};
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s",
		info->cap_info.is_zmt200 ? "ZMT200" : "ZFT400");
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void get_threshold(void *device_data)
{
	...
	ret = ts_read_data(info->client,
		ZINITIX_SENSITIVITY, (u8 *)&threshold, 2);
...
static void get_key_threshold(void *device_data)
{
	...
	ret = ts_read_data(misc_touch_dev->client,
		ZINITIX_BUTTON_SENSITIVITY, (u8 *)&threshold, 2);
...
static bool get_raw_data(struct zinitix_touch_dev *touch_dev,
					u8 *buff, int skip_cnt)
{
	u32 total_node = touch_dev->cap_info.total_node_num;
	u32 sz;
	int i;
...
static void run_reference_read(void *device_data)
{
	...
	printk(KERN_INFO " raw_min= %u, raw_max= %u", min, max);
	sprintf(buff, "%u,%u\n", min, max);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
...
static void get_tkey_delta(void *device_data)
{
	...
	ret = ts_read_data(misc_touch_dev->client,
		ZINITIX_BTN_WIDTH + btn_node, (u8 *)&val, 2);
...
int Zinitix_ts_check(void)
{
	...
	printk(KERN_INFO "[TSP] Zinitix_ts_check start\n");

	ret = i2c_master_send(misc_touch_dev->client , &buf_firmware , 2);

	printk(KERN_INFO
		"[TSP] Zinitix_ts_check : i2c_master_send = %d\n",
		ret);

	/*i2c read retry*/
	if (ret <= 0) {
		for (i = 0; i < retry; i++) {
			ret = i2c_master_send(misc_touch_dev->client,
				&buf_firmware , 2);
			if (ret > 0)
				break;
		}
		printk(KERN_ERR
			"%s : i2c_master_send fail [%d]\n",
			__func__, ret);
		tsp_check = 1;
	}
	...
	if (ret <= 0) {
		...
		printk(KERN_INFO "[TSP][Zinitix][%s] %s\n",
			__func__, "Passed Zinitix i2c");

		Zinitix_Connected = 1;

		if (tsp_check == 0) {
			ret = i2c_master_recv(misc_touch_dev->client,
				&buf_firmware, 3);
			if (ret < 0) {
		...
		/*printk("[TSP][Silabs][%s][SlaveAddress : 0x%x][VendorID :
		0x%x] [HW : 0x%x] [SW : 0x%x]\n",
		...
		const struct i2c_device_id *i2c_id)
{
	...
	if (touch_dev->irq < 0)
		printk(KERN_INFO
			"error. gpio_to_irq(..) function is not supported? you should define GPIO_TOUCH_IRQ.\r\n"
		);
#endif
...
#if USE_THREADED_IRQ
	/*ret = request_threaded_irq(touch_dev->irq,
	ts_int_handler, zinitix_touch_work,*/
	ret = request_threaded_irq(touch_dev->irq, NULL, zinitix_touch_work,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT ,
		ZINITIX_DRIVER_NAME, touch_dev);
#else
	...
	for (i = 0; i < ARRAY_SIZE(zinitix_tsp_cmds); i++)
		list_add_tail(&zinitix_tsp_cmds[i].list,
	...
	sec_touchscreen_dev = device_create(sec_class, NULL,
			0, touch_dev, "tsp");
	...
	ret = sysfs_create_group(&sec_touchscreen_dev->kobj,
		&sec_touch_attributes_group);
	if (ret)
		dev_err(&client->dev, "Failed to create sysfs .\n");

	sec_touchkey_dev = device_create(sec_class, NULL, 0,
		touch_dev, "sec_touchkey");
