...
#include <linux/input/synaptics_tcm_tama.h>
/* SOMC_TOUCH_BRINGUP end */

#if defined(CONFIG_DRM_PANEL) || defined(CONFIG_FB)
...
STORE_PROTOTYPE(syna_tcm, irq_enable)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
static ssize_t syna_tcm_sysfs_irq_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
static ssize_t syna_tcm_sysfs_cover_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
static ssize_t syna_tcm_sysfs_glove_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
static ssize_t syna_tcm_sysfs_glove_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
static ssize_t syna_tcm_sysfs_stamina_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
static ssize_t syna_tcm_sysfs_stamina_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
#else
...
#if defined(SOMC_TOUCH_BRINGUP)
	dev_set_name(&tcm_hcd->virtdev, "%s", path_name);
	retval = device_register(&tcm_hcd->virtdev);
	if (retval)
		LOGE(tcm_hcd->pdev->dev.parent,
			"device virtdev register error\n");
	dev_set_drvdata(&tcm_hcd->virtdev, tcm_hcd);
	retval = sysfs_create_link(&tcm_hcd->virtdev.kobj, tcm_hcd->dynamnic_config_sysfs_dir, "touch");
	if (retval)
		LOGE(tcm_hcd->pdev->dev.parent,"sysfs link error\n");
#endif
#if defined(CONFIG_FB) && !defined(CONFIG_DRM_PANEL)
