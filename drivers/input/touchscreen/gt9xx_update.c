...
static u8 gup_enter_upadte_judge(struct st_fw_head *fw_head)
{
	...
	if (!memcmp
		...
		if ((!memcmp
		     (fw_head->pid, update_msg.ic_fw_msg.pid,
		      strlen(fw_head->pid)))
		...
		struct st_fw_head *fw_head, u8 *path)
{
	...
	if (path) {
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++) {
			...
			if (IS_ERR(update_msg.file)) {
				update_msg.file =
				    filp_open(UPDATE_FILE_PATH_2, O_RDWR, 0666);
				if (IS_ERR(update_msg.file)) {
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	ret =
	    gup_load_section_file(fw_ss51, FW_SECTION_LENGTH,
				  FW_SECTION_LENGTH);
	if (FAIL == ret) {
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	GTP_DEBUG("[burn_fw_dsp]step8:");
	GTP_DEBUG("send burn cmd to move data to flash from sram");
