		...
		bool enable)
{
	...
	if (enable) {
		if (rmi4_data->irq_enabled) {
			TP_LOGW("Interrupt already enabled\n");
			goto exit;
		}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		TP_LOGE("Out of register map limit\n");
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TP_LOGD("Attention interrupt disabled\n");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmidev->pid) {
		...
		if (!rmidev->task) {
			TP_LOGE("Failed to locate PID of data logging tool\n");
			return -EINVAL;
		}
	}
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (IS_ERR(dev_data)) {
		TP_LOGE("Pointer of char device data is invalid");
		return -EBADF;
	}
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		TP_LOGE("New position 0x%04x is invalid\n",
				(unsigned int)newpos);
		newpos = -EINVAL;
		goto clean_up;
	}
...
static int rmidev_create_device_class(void)
{
	...
	if (IS_ERR(rmidev_device_class)) {
		TP_LOGE("Failed to create /dev/%s\n", CHAR_DEVICE_NAME);
		return -ENODEV;
	}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!rmidev) {
		TP_LOGE("Failed to alloc mem for rmidev\n");
		retval = -ENOMEM;
		goto err_rmidev;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to create device class\n");
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			TP_LOGE("Failed to allocate char device region\n");
			goto err_device_region;
		}

		rmidev_major_num = MAJOR(dev_no);
		TP_LOGD("Major number of rmidev = %d\n", rmidev_major_num);
	}

	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		TP_LOGE("Failed to alloc mem for dev_data\n");
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to add rmi char device\n");
	...
	if (IS_ERR(device_ptr)) {
		TP_LOGE("Failed to create rmi char device\n");
		retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (retval < 0) {
		TP_LOGE("Failed to export attention gpio\n");
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", bdata->irq_gpio);
		if (retval < 0) {
			TP_LOGE("Failed to create gpio symlink\n");
		} else {
			TP_LOGD("Exported attention gpio %d\n", bdata->irq_gpio);
		}
	}
