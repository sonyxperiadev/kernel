...
static struct device_attribute attributes[] = {
	...
	__ATTR(hw_reset, S_IWUSR | S_IWGRP, NULL, cyttsp5_hw_reset_store),
	__ATTR(hw_irq_stat, S_IRUGO, cyttsp5_hw_irq_stat_show, NULL),
	__ATTR(drv_irq, S_IRUGO | S_IWUSR | S_IWGRP, cyttsp5_drv_irq_show, cyttsp5_drv_irq_store),
		...
		unsigned long event, void *data)
{
	...
	if (event == FB_EVENT_BLANK && evdata) {
		...
		if (*blank == FB_BLANK_UNBLANK || *blank == FB_BLANK_VSYNC_SUSPEND) {
			dev_info(cd->dev, "%s: UNBLANK(%d)!\n", __func__, *blank);
			if (cd->fb_state != FB_ON) {
				cyttsp5_core_wake(cd);
				cd->fb_state = FB_ON;
			}
		} else if (*blank == FB_BLANK_POWERDOWN) {
			dev_info(cd->dev, "%s: POWERDOWN(%d)!\n", __func__, *blank);
			...
			dev_err(cd->dev, "%s: Unknown *blank(%d)!\n", __func__, *blank);
	}
