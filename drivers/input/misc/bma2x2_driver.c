...
static const int bma2x2_direction_mapping[][3][3] = {
...
/* PERI-FG-REGULATOR_SET-00+[ */
static int bma2x2_set_power(struct bma2x2_data *bma2x2, int Mode)
{
    int rc = 0;
    if (IS_ERR(bma2x2->reg))
    {
        GSENSOR_DEBUG(LEVEL0, "Failed.");
        return -ENODEV;
    }

    if (Mode != BMA2X2_MODE_SUSPEND && Mode != BMA2X2_MODE_DEEP_SUSPEND)
    {
        rc = regulator_set_optimum_mode(bma2x2->reg, bma2x2->hpm_uA);
        if (rc < 0)
        {
            GSENSOR_DEBUG(LEVEL0, "enable failed.");
            return rc;
        }
        GSENSOR_DEBUG(LEVEL0, "Mode(%d), %duA", Mode, bma2x2->hpm_uA);
    }
    else
    {
        rc = regulator_set_optimum_mode(bma2x2->reg, bma2x2->lpm_uA);
        if (rc < 0)
        {
            GSENSOR_DEBUG(LEVEL0, "disable failed.");
            return rc;
        }
        GSENSOR_DEBUG(LEVEL0, "Mode(%d), %duA", Mode, bma2x2->lpm_uA);
    }
    return 0;
}
/* PERI-FG-REGULATOR_SET-00+] */
...
static int bma2x2_set_mode(struct i2c_client *client, unsigned char Mode)
{
	...
	if (Mode < 6) {
		...
		bma2x2_set_power(bma2x2, Mode);	/* PERI-FG-REGULATOR_SET-00+ */
	} else {
		comres = -1 ;
	}
		...
		signed char sensor_type, struct bma2x2acc *acc)
{
    ...
    {
        for (j=0; j<3; j++)
            temp[i] += raw[j] * bma2x2_direction_mapping[bma2x2->layout][i][j];
    }
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
    ...
    if(pre_enable == enable)
        return;
    GSENSOR_DEBUG(LEVEL1,"Enable(%d)",enable);
    ...
    {   /* Polling Mode */
        ...
        {   /* Interrupt Mode */
            ...
            bma2x2->polling_count = 0;	/* PERI-FG-FIX_INTERRUPT_SCOTOMA-00+ */
            schedule_delayed_work(&bma2x2->irq_work,msecs_to_jiffies(0));	/* PERI-FG-FIX_INTERRUPT_SCOTOMA-00+ */
        }
    }
    else
    {
...
/*		GSENSOR_DEBUG(LEVEL0, "wait 2ms cal ready flag is %d\n", tmp);
 */
		timeout++;
		if (timeout == 50) {
			GSENSOR_DEBUG(LEVEL0, "get fast calibration ready error\n");
			return -EINVAL;
		};

	} while (tmp == 0);

	GSENSOR_DEBUG(LEVEL0, "x axis fast calibration finished\n");
	return count;
}
 ...
 */
	...
	GSENSOR_DEBUG(LEVEL0, "z axis fast calibration finished\n");
	return count;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bma2x2_get_fifo_data_sel(bma2x2->bma2x2_client, &fifo_datasel) < 0)
		return snprintf(buf, PAGE_SIZE, "Read data sel error\n");
	...
	if (bma2x2_get_fifo_framecount(bma2x2->bma2x2_client, &f_count) < 0)
		return snprintf(buf, PAGE_SIZE, "Read frame count error\n");
...
static ssize_t bma2x2_layout_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
    unsigned long data;
...
static DEVICE_ATTR(layout, S_IRUGO|S_IWUSR|S_IWGRP,
		bma2x2_layout_show, bma2x2_layout_store);
static DEVICE_ATTR(interrupt, S_IRUGO|S_IWUSR|S_IWGRP,
		bma2x2_interrupt_show, bma2x2_interrupt_store);
static DEVICE_ATTR(dbg, S_IRUGO|S_IWUSR|S_IWGRP,
		NULL, bma2x2_debug_store);
				...
				struct bma2x2_data *pdata)
{
    ...
    pdata->reg = regulator_get(dev, "bmavdd");
    if (IS_ERR(pdata->reg))
    {
		...
		const struct i2c_device_id *id)
{
    ...
    if (client->dev.of_node)
    {
        ...
        if (!data)
        {
            GSENSOR_DEBUG(LEVEL0, "Failed to allocate memory\n");
            return -ENOMEM;
        }
...
//	}
    ...
    /* read chip id */
    while(tempvalue!=BMA2X2_RETRY_SCAN)
    {
        err = bma2x2_smbus_read_byte(data->bma2x2_client,0,&tmp_chip_id);
        if(err == 0)
        	break;    
        tempvalue++;
    }
    ...
    if (data->sensor_type != -1) {
        ...
        GSENSOR_DEBUG(LEVEL0, "Bosch Sensortec Device detected!\n"
            "%s registered I2C driver!\n",
            sensor_name[data->sensor_type]);
    } else{
        GSENSOR_DEBUG(LEVEL0, "Bosch Sensortec Device not found"
            " i2c error %d \n", tmp_chip_id);
            err = -ENODEV;
        goto kfree_exit;
    }

    /* PERI-FG-FIX_KERNEL_PANIC-01*[ */
...
#if defined(BMA2X2_ENABLE_INT1) || defined(BMA2X2_ENABLE_INT2)
    INIT_DELAYED_WORK(&data->irq_work, bma2x2_irq_work_func);	/* PERI-FG-FIX_KERNEL_PANIC-00* */
    ...
    err = request_irq(data->IRQ, bma2x2_irq_handler, IRQF_TRIGGER_RISING,
        "bma2x2", data);
    if (err)
        GSENSOR_DEBUG(LEVEL0, "could not request irq\n");
