...
#define CONFIG_OUTPUT_ENU  					1
#define CONFIG_OUTPUT_ENHANCED_STILL_MODE	1
...
static void _em718x_sensor_3axes_notify( struct em718x_sensor * sensor, DI_3AXIS_INT_DATA_T * d )
{
	...
	send_event( input, INPUT_EVENT_TIME, sensor->timestamp );
	input_sync( input );
				      			 ...
				      			 const struct i2c_device_id *id)
{
	...
	em718x_load_firmware(emdev, id->name);
	...
	for(i=DST_FIRST; i<DST_NUM_SENSOR_TYPES; i++) {
		...
		sensor->attr_rate.attr.mode = 0664;								
		...
		sensor->attr_enable.attr.mode = 0664;							
		...
		switch( i ) {
			...
			#define setup_msc_16()													\
					setup_event_type												\
					__set_bit( INPUT_EVENT_TIME, input->mscbit );					\
