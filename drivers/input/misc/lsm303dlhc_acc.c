...
struct lsm303dlhc_acc_data {
	...
	int (*power)(enum lsm303dlhc_acc_power_sate pwr_state);
};
...
static int lsm303dlhc_acc_set_sampling_rate(struct lsm303dlhc_acc_data *acc)
{
	...
	if (!err)
		dev_dbg(&acc->client->dev, "%s: sampling rate %d ms, "
			"polling rate %d ms\n", __func__,
			odr_table[i].poll_rate_ms, ms);
	return err;
}
...
static int lsm303dlhc_acc_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s\n", __func__);
	LOCK(acc);
	acc->power(LSM303DLHC_STANDBY);
	if (acc->input_dev->users) {
		int e;
		acc->power(LSM303DLHC_PWR_ON);
...
static int lsm303dlhc_acc_power_stub(enum lsm303dlhc_acc_power_sate pwr_state)
{
	pr_debug("%s: power state %d\n", __func__, pwr_state);
	return 0;
}

static int __devinit lsm303dlhc_acc_probe(struct i2c_client *client,
