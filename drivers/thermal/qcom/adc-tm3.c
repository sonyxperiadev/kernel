...
struct adc_tm_linear_graph cal_graph[ADC_ABS_CAL + 1];
...
static int adc_tm3_get_temp(struct adc_tm_sensor *sensor, int *temp)
{
...
static int32_t adc_tm3_read_reg(struct adc_tm_chip *chip,
...
static int32_t adc_tm3_write_reg(struct adc_tm_chip *chip,
					int16_t reg, u8 *data, int len)
{
	int ret;
...
static int32_t adc_tm3_reg_update(struct adc_tm_chip *chip,
...
static int32_t adc_tm3_get_btm_idx(struct adc_tm_chip *chip,
...
static int32_t adc_tm3_enable(struct adc_tm_chip *chip)
{
	int rc = 0;
	u8 data = 0;
...
static int32_t adc_tm3_manage_thresholds(struct adc_tm_sensor *sensor)
{
	...
	ret = adc_tm3_get_btm_idx(chip, sensor->btm_ch, &btm_chan_idx);
	if (ret < 0) {
...
static void notify_adc_tm3_fn(struct work_struct *work)
{
	...
	ret = adc_tm3_get_btm_idx(chip, btm_chan_num, &btm_chan_idx);
	if (ret < 0) {
		pr_err("Invalid btm channel idx\n");
		return;
	}
			...
			int trip, enum thermal_device_mode mode)
{
	...
	ret = adc_tm3_get_btm_idx(chip, btm_chan, &btm_chan_idx);
	if (ret < 0) {
		pr_err("Invalid btm channel idx\n");
		return ret;
	}
...
static int32_t qpnp_adc_tm_check_revision(struct adc_tm_chip *chip,
							uint32_t btm_chan_num)
{
	...
	if (perph_subtype == ADC_TM_PERPH_TYPE2) {
		if ((rev < ADC_TM_REV_EIGHT_CHAN_SUPPORT) &&
			(btm_chan_num > ADC_TM_Mn_ADC_CH_SEL_CTL(4))) {
	...
	if (perph_subtype == ADC_TM_PERPH_SUBTYPE_TWO_CHAN_SUPPORT) {
		if (btm_chan_num > ADC_TM_Mn_ADC_CH_SEL_CTL(1)) {
					...
					int low_temp, int high_temp)
{
	...
	if (qpnp_adc_tm_check_revision(chip, sensor->btm_ch))
		return -EINVAL;
