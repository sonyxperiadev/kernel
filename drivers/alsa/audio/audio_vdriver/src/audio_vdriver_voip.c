...
#define VOIP_MAX_FRAME_LEN	(322 + AUDQUE_MARGIN) // 160 words + 1 word for codecType
...
static void ProcessDlRequest(VOIP_Drv_t *audDrv, UInt8 *pDst, UInt32 nFrames)
{
	...
	else
	{
		...
		if (((readPtr->voip_vocoder & 0xf000) == VOIP_PCM) ||
		    ...
		    ((readPtr->voip_vocoder & 0xf000) == VOIP_AMR475) || ((readPtr->voip_vocoder & 0xf000) == VOIP_G711_U))
		{
...
static Boolean AP_VoIP_StopTelephony( void )
{
#if defined(tempINTERFACE_CLEAR_VOIPMODE)
