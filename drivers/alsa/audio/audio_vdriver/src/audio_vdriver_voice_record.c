...
static void ProcessSharedMemRequest (VOCAPTURE_Drv_t *audDrv, UInt16 bufIndex, UInt32 bufSize)
{
	...
	switch (audDrv->drvType)
	{
		case VOCAPTURE_TYPE_AMRNB:
			...
			{
				recvSize = dspif_VPU_record_read_AMRNB ( AUDQUE_GetWritePtr(aq), bufSize, bufIndex, audDrv->config.speechMode,audDrv->numFramesPerInterrupt);
			...
			else
			{
				...
				recvSize = dspif_VPU_record_read_AMRNB ( AUDQUE_GetWritePtr(aq), bottomSize, bufIndex, audDrv->config.speechMode,audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);

				recvSize = dspif_VPU_record_read_AMRNB ( AUDQUE_GetWritePtr(aq), bufSize - bottomSize, bufIndex, audDrv->config.speechMode,audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);
			}
			break;
		...
		case VOCAPTURE_TYPE_PCM:
			...
			if (bottomSize >= bufSize)
			{
				recvSize = dspif_VPU_record_read_PCM ( AUDQUE_GetWritePtr(aq), bufSize, bufIndex, audDrv->config.speechMode,audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);
			}
			else
			{
				...
				recvSize = dspif_VPU_record_read_PCM ( AUDQUE_GetWritePtr(aq), bottomSize, bufIndex, audDrv->config.speechMode,audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);

				recvSize = dspif_VPU_record_read_PCM ( AUDQUE_GetWritePtr(aq), bufSize - bottomSize, bufIndex, audDrv->config.speechMode,audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);
			}
			break;
		...
		case VOCAPTURE_TYPE_AMRWB:
			...
			if (bottomSize >= bufSize)
			{
				recvSize = dspif_AMRWB_record_read ( AUDQUE_GetWritePtr(aq), bufSize, bufIndex, audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);
			}
			else
			{
				recvSize = dspif_AMRWB_record_read ( AUDQUE_GetWritePtr(aq), bottomSize, bufIndex, audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);

                bufIndex = dspif_AMRWB_record_GetReadIndex();
				recvSize = dspif_AMRWB_record_read ( AUDQUE_GetWritePtr(aq), bufSize - bottomSize, bufIndex, audDrv->numFramesPerInterrupt);
				if (recvSize >0) AUDQUE_UpdateWritePtrWithSize (aq, recvSize);
			}
			break;
