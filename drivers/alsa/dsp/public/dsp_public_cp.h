...
void tl3_dsp_cleanup(void);
// this one is in phy,this violates arch spec, 
void AMRPROC_DSP_StartDlinkNewCodecSet(UInt16 arg0, UInt16 arg1, UInt16 arg2);
void  L1_Ftt_SetScellSlot( UInt16 cell_id, UInt16 slot);

// move these from sysparm.c to DSP public functions file, 


void UpdateBlueToothFilterEnableFlag(void);
void ConfigBlueToothFilter(void);
void UpdateCompBiquadGain(void);
void UpdateAndConfigCompFilter(void);

void SYSPARM_SetPaRampScaleVoltTemp(// Load a ramp profile to shared memory
	UInt8		paVolt,				// Power amplifier voltage, unit: pre-defined step
	UInt8		paTemp				// Power amplifier temperature, unit: pre-defined step
	);
