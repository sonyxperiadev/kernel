...
#include "hscodec.h"
#define MAX_ARM_HS_ENC_DELAY 3
#define MAX_ARM_HS_DEC_DELAY 1
#endif
			...
			case STATUS_FRAME_INT :
				...
				if(frame_int & FRAMEINT_EXTEND)
				{
					if( !PHYEvent_IsNonIdleMode())
					{
						...
						JTAG_FRIENDLY_L1_ASSERT( ENTRYDATA_GetLastExtendEntry() == (UInt8)status_msg.arg0, (status_msg.arg0|((ENTRYDATA_GetLastExtendEntry())<<8)));
...
// CQ MobC00103169, Andy Shen, 02/25/10
		...
		case STATUS_DSP_WAKEUP_RSP:
			...
			PHYFRAME_DSPSuspendAck(STATUS_DSP_WAKEUP_RSP, status_msg.arg0 ? status_msg.arg1 - 1 : 0);
			break;



#ifdef	ARM_HS_VOCODER
		case STATUS_RUN_HS_VOCODER:
			{
				ARM_HS_Msg_t	hs_req;

				hs_req.parm0 = status_msg.arg0;
				hs_req.parm1 = status_msg.arg1;
				hs_req.parm2 = status_msg.arg2;

				L1_LOGV4( "HS req = ", status_msg.arg0, status_msg.arg1, status_msg.arg2, 0 );
				// Argument 0 indicates if Encode/Decode/Vocoder init should be performed
				HSCODEC_Trigger( hs_req );
			}
			break;

		case STATUS_HS_VOCODER_LATE:
			L1_LOGV4( "HS late = ", status_msg.arg0, status_msg.arg1, status_msg.arg2, 0 );
			if ( status_msg.arg0 == ARM_HS_ENC && status_msg.arg1 >= MAX_ARM_HS_ENC_DELAY )
			{
				RIPCMDQ_HS_VOCODER_Done(ARM_HS_ENC);
			}
			else if ( status_msg.arg0 == ARM_HS_DEC && status_msg.arg1 >= MAX_ARM_HS_DEC_DELAY )
			{
				RIPCMDQ_HS_VOCODER_Done(ARM_HS_DEC);
			}
			break;
#endif	// #ifdef	ARM_HS_VOCODER
