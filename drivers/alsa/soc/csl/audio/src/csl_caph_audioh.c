...
//			chal_audio_earpath_set_dac_pwr(handle, CHAL_AUDIO_ENABLE);
		...
		case AUDDRV_PATH_VIN_INPUT:
			...
			chnl_enable = CHAL_AUDIO_CHANNEL_LEFT;
			if(path[path_id].mic_id & CHAL_AUDIO_CHANNEL_LEFT)	
                chnl_enable |= CHAL_AUDIO_CHANNEL_LEFT;
			if(path[path_id].mic_id & CHAL_AUDIO_CHANNEL_RIGHT)	
                chnl_enable |= CHAL_AUDIO_CHANNEL_RIGHT;
			chal_audio_vinpath_digi_mic_enable(handle, chnl_enable);	// Enable the digital microphone
		...
		case AUDDRV_PATH_NVIN_INPUT:
			...
			chal_audio_nvinpath_digi_mic_enable(handle, path[path_id].mic_id);	

			break;

		case AUDDRV_PATH_ANALOGMIC_INPUT:

//			chal_audio_mic_input_select(handle, path[path_id].mic_id);
...
//			PMU_DRV_AUDIO_IHF_PowerDown(NULL);
		...
		case AUDDRV_PATH_VIN_INPUT:
			...
			chal_audio_vinpath_digi_mic_enable(handle, 0);	// Disable the digital microphone
//			chal_audio_mic_pwrctrl(handle, 0);
		...
		case AUDDRV_PATH_NVIN_INPUT:
			...
			chal_audio_nvinpath_digi_mic_enable(handle, 0);	
...
void csl_caph_audioh_mute(int path_id, Boolean mute_ctrl)
{
	switch(path_id)
	{
		...
		case AUDDRV_PATH_VIN_INPUT:
			...
			chal_audio_vinpath_mute(handle,  mute_ctrl);

			break;

		case AUDDRV_PATH_NVIN_INPUT:

//			chal_audio_nvinpath_mute(handle,  mute_ctrl);
