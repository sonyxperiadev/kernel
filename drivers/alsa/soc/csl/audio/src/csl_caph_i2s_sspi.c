...
/****************************************************************************
*
*  Function Name: SSPI_hw_DMA_init(CSL_HANDLE handle, CSL_I2S_CONFIG_t *config)
*
*  Description: Initialize I2S DMA
*
****************************************************************************/
...
#if 0
static CSL_I2S_ERR_CODE  SSPI_hw_DMA_init(CSL_HANDLE handle, CSL_I2S_CONFIG_t *config)
{
    CHAL_SSPI_STATUS_t status = CHAL_SSPI_STATUS_SUCCESS;
    if(config->rx_ena) {
            status = chal_sspi_dma_set_burstsize(handle, SSPI_DMA_CHAN_SEL_CHAN_RX0,
                                    CHAL_SSPI_DMA_BURSTSIZE_16BYTES);
            status = chal_sspi_enable_dma(handle, SSPI_DMA_CHAN_SEL_CHAN_RX0,
                                SSPI_FIFO_ID_RX0, 1);
         	Log_DebugPrintf(LOGID_SOC_AUDIO, "RX enable \n");
        }
    if(config->tx_ena) {
            status = chal_sspi_dma_set_burstsize(handle, SSPI_DMA_CHAN_SEL_CHAN_TX0,
                                    CHAL_SSPI_DMA_BURSTSIZE_16BYTES);

            status = chal_sspi_enable_dma(handle, SSPI_DMA_CHAN_SEL_CHAN_TX0,
                                 SSPI_FIFO_ID_TX0, 1);
        }
	...
	if(!config->interleave) {
        ...
        if(config->rx_ena) {
            status = chal_sspi_dma_set_burstsize(handle, SSPI_DMA_CHAN_SEL_CHAN_RX1,
                                    CHAL_SSPI_DMA_BURSTSIZE_16BYTES);
            status = chal_sspi_enable_dma(handle, SSPI_DMA_CHAN_SEL_CHAN_RX1,
                                SSPI_FIFO_ID_RX1, 1);
        }
       if(config->tx_ena) {
           status = chal_sspi_dma_set_burstsize(handle, SSPI_DMA_CHAN_SEL_CHAN_TX1,
                                    CHAL_SSPI_DMA_BURSTSIZE_16BYTES);
           status = chal_sspi_enable_dma(handle, SSPI_DMA_CHAN_SEL_CHAN_TX1,
                                 SSPI_FIFO_ID_TX1, 1);
	   }
	}

    if(status == CHAL_SSPI_STATUS_SUCCESS)
        return CSL_I2S_SUCCESS;
    else if(status == CHAL_SSPI_STATUS_ILLEGAL_HANDLE)
        return CSL_I2S_ERROR;
    return CSL_I2S_SUCCESS;
}
#endif
/****************************************************************************
