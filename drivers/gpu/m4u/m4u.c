...
static int m4u_map_pages(struct m4u_device *mdev, u32 mma, u32 pa, int page_order, int n)
{
	...
	for(i=0; i<(n/M4U_TLB_LINE_SIZE); i++) {
		for(j=0; j<num_pte; j++) {
			pa = pa_group;
			for(k=0; k<M4U_TLB_LINE_SIZE; k++) {
				*pt++ = m4u_pte(pa, page_order_base4k, 1);
				pa += 1 << page_order;
			}
		}
		pa_group = pa;
...
static int m4u_map(struct m4u_device *mdev, u32 mma, u32 pa, u32 size)
{
	return 0;
}
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	/* TODO: m4u_regions are not copied */
#ifdef DEBUG_M4U
	dbg_mdev = mdev;
	dbg_page_size = mdev->page_size;
	pr_info("(Debug) mdev(%p) page_size(0x%x) \n", 
			dbg_mdev, dbg_page_size);
#endif
	...
	mdev->base = ioremap_nocache(mem_res->start, resource_size(mem_res));
	if (!mdev->base) {
	...
	pr_info("probe successfull - page_size(%d) irq(%d) reg-base(%p). \n",
			mdev->page_size, irq, mdev->base);
