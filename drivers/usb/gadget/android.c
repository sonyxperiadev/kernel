...
#ifdef CONFIG_USB_GADGET_GG
#define MAX_GG_INSTANCES 2
struct gg_function_config {
	int instances;
	int instances_on;
	struct usb_function *f_gg[MAX_GG_INSTANCES];
	struct usb_function_instance *f_gg_inst[MAX_GG_INSTANCES];
};

static int gg_function_init(struct android_usb_function *f,
		struct usb_composite_dev *cdev)
{
	int i;
	int ret;
	struct gg_function_config *config;

	config = kzalloc(sizeof(struct gg_function_config), GFP_KERNEL);
	if (!config)
		return -ENOMEM;
	f->config = config;

	for (i = 0; i < MAX_GG_INSTANCES; i++) {
		config->f_gg_inst[i] = usb_get_function_instance("gg");
		if (IS_ERR(config->f_gg_inst[i])) {
			ret = PTR_ERR(config->f_gg_inst[i]);
			goto err_usb_get_function_instance;
		}
		config->f_gg[i] = usb_get_function(config->f_gg_inst[i]);
		if (IS_ERR(config->f_gg[i])) {
			ret = PTR_ERR(config->f_gg[i]);
			goto err_usb_get_function;
		}
	}
	return 0;
err_usb_get_function_instance:
	while (i-- > 0) {
		usb_put_function(config->f_gg[i]);
err_usb_get_function:
		usb_put_function_instance(config->f_gg_inst[i]);
	}
	return ret;
}
...
{
	int i;
	struct gg_function_config *config = f->config;
		...
		struct usb_configuration *c)
{
	...
	config->instances_on = config->instances;
	for (i = 0; i < config->instances_on; i++) {
		ret = usb_add_function(c, config->f_gg[i]);
		if (ret) {
			pr_err("Could not bind gg%u config\n", i);
			goto err_usb_add_function;
		}
	}
...
err_usb_add_function:
	while (i-- > 0)
		usb_remove_function(c, config->f_gg[i]);
	return ret;
}

static ssize_t gg_instances_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct android_usb_function *f = dev_get_drvdata(dev);
	struct gg_function_config *config = f->config;
...
static ssize_t gg_instances_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct android_usb_function *f = dev_get_drvdata(dev);
	struct gg_function_config *config = f->config;
	...
	sscanf(buf, "%d", &value);
	if (value > MAX_GG_INSTANCES)
		value = MAX_GG_INSTANCES;
	config->instances = value;
	return size;
}

static DEVICE_ATTR(gg_instances, S_IRUGO | S_IWUSR, gg_instances_show,
						 gg_instances_store);
static struct device_attribute *gg_function_attributes[] = {
	&dev_attr_gg_instances,
	NULL
};

static struct android_usb_function gg_function = {
	.name		= "gg",
	.init		= gg_function_init,
	.cleanup	= gg_function_cleanup,
	.bind_config	= gg_function_bind_config,
	.attributes	= gg_function_attributes,
};
#endif
					...
					struct usb_configuration *c)
{
	...
	return ncm_bind_config(c, ncm->ethaddr, dev);
}
