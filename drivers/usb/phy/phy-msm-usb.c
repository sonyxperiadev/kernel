	...
	enum msm_otg_phy_reg_mode mode)
{
	...
	switch (mode) {
	case USB_PHY_REG_ON:
		ret = regulator_set_load(hsusb_1p8,
	...
	case USB_PHY_REG_3P3_ON:
		ret = regulator_set_load(hsusb_3p3,
...
static int msm_otg_resume(struct msm_otg *motg)
{
	...
	msm_otg_dbg_log_event(phy, "LPM EXIT START", motg->inputs, phy->otg->state);
	if (!atomic_read(&motg->in_lpm)) {
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	 ...
	 * kholk 19/06/2017: Should we remove this check?
	 */
	if (motg->chg_type == 0)
		return 0;
