...
dhdsdio_bussleep(dhd_bus_t *bus, bool sleep)
{
	...
	if (sleep) {
		...
		if (bus->in_dpc || bus->dpc_sched || bus->rxskip || pktq_len(&bus->txq)) {
        		/* Re-enable interrupts if previously enabled */
			if (bus->intr && !bus->intdis) {
...
//extern int bcm_sdiowl_register_hwakeup_pin(int);
//extern void bcm_sdiowl_unregister_hwakeup_pin(void);
/**
 ...
 */
int
dhdsdio_register_hwakeup(dhd_bus_t *bus)
{
	int ret = 0;
	//struct dhd_host_wake_info *wake_info = &bus->dhd->host_wake_info;

   // ret = bcm_sdiowl_register_hwakeup_pin(wake_info->irq);
		return ret;
}

void
dhdsdio_unregister_hwakeup(dhd_bus_t *bus)
{
  //  bcm_sdiowl_unregister_hwakeup_pin();
}

int 
dhdsdio_is_bus_sleeping_or_down(dhd_bus_t *bus)
{
	
	if ( bus->sleeping || (bus->dhd->busstate == DHD_BUS_DOWN))
		return TRUE;
	else
		return FALSE;
}
...
dhdsdio_readshared(dhd_bus_t *bus, sdpcm_shared_t *sh)
{
	...
	/*
	 * For the FALCON RC14, the dongle version is 3 while the driver is 1.
	 * Dennis Lo confirms that the sdpcm_shared_t stuctures are the same and
	 * suggested this patch. By returning OK here we will not lose any
	 * firmware crash info.
	*/
