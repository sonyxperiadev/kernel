...
)
{
	...
	if ((error = dev_wlc_ioctl(dev, WLC_SET_CHANNEL, &chan, sizeof(chan)))) {
		...
		/* Return zero anyway even though setting the frequency failed.
		 * FW will pick a valid frequency when joining 
		 */
	}
	...
	/*
	* Dennis Lo hack, this delay increases chances of 5Ghz association succes.
	* -don
	*/
	WL_ERROR(("%s: ASSOC hack - sleeping 1 sec\n", __FUNCTION__));
	msleep(1000);
...
struct {
	pmkid_list_t pmkids;
	pmkid_t foo[MAXPMKID-1];	/* This must come right after pmkids */
} pmkid_list;
