...
int dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if ((!op_mode && strstr(fw_path, "_apsta") != NULL)
		...
		if ((ret =
		     dhd_wl_ioctl_cmd(dhd, WLC_SET_BAND, (char *)band,
		...
		if (buf[0] == 0) {
			int wme = 0;
			bcm_mkiovar("wme", (char *)&wme, 4, iovbuf,
				    sizeof(iovbuf));
			if ((ret =
			     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
...
int dhd_os_wake_lock_timeout(dhd_pub_t *pub)
{
	...
	if (dhd) {
		...
		ret =
		    ...
		    dhd->wakelock_ctrl_timeout_enable ? dhd->
		    wakelock_rx_timeout_enable : dhd->
		    wakelock_ctrl_timeout_enable;
#ifdef CONFIG_HAS_WAKELOCK
		if (dhd->wakelock_rx_timeout_enable)
			wake_lock_timeout(&dhd->wl_rxwake,
					  msecs_to_jiffies(dhd->
