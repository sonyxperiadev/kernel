...
int dhd_bta_docmd(dhd_pub_t *pub, void *cmd_buf, uint cmd_len)
{
	...
	len = bcm_mkiovar("HCI_cmd",
			  (char *)cmd, (uint) cmd->plen + HCI_CMD_PREAMBLE_SIZE,
			  (char *)buf, len);
...
static void dhd_bta_flush_hcidata(dhd_pub_t *pub, uint16 llh)
{
	int prec;
	struct pktq *q;
	uint count = 0;
		...
		while (pktq_ppeek(q, prec) != head_pkt) {
			...
			if (PKTLEN(pub->osh, pkt) >= RFC1042_HDR_LEN) {
				struct ether_header *eh =
				    (struct ether_header *)PKTDATA(pub->osh,
								   pkt);
 ...
 *        > 0: stop, succuess
 */
static int _dhd_bta_docmd(dhd_pub_t *pub, amp_hci_cmd_t * cmd)
{
	int status = 0;
...
int dhd_bta_tx_hcidata(dhd_pub_t *pub, void *data_buf, uint data_len)
{
	...
	if ((len =
	     (uint) ltoh16(data->dlen) + HCI_ACL_DATA_PREAMBLE_SIZE) >
	    data_len) {
...
/* txcomplete callback */
void dhd_bta_tx_hcidata_complete(dhd_pub_t *dhdp, void *txp, bool success)
{
	...
	uint8 data[HCI_EVT_PREAMBLE_SIZE +
...
/* event callback */
void dhd_bta_doevt(dhd_pub_t *dhdp, void *data_buf, uint data_len)
{
