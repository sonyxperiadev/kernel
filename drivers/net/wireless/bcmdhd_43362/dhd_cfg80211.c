		       ...
		       uint reg, int *retval)
{
	...
	error =
	    wldev_ioctl(dev, WLC_GET_VAR, (char *)(&var), sizeof(var.buf),
			false);
...
dev_wlc_bufvar_set(struct net_device *dev, char *name, char *buf, int len)
{
	...
	return (wldev_ioctl
		(dev, WLC_SET_VAR, ioctlbuf_local, sizeof(ioctlbuf_local),
		 true));
}
...
*/
static int
dev_wlc_intvar_set_reg(struct net_device *dev, char *name, char *addr,
		       char *val)
{
	char reg_addr[8];
	...
	return (dev_wlc_bufvar_set
		(dev, name, (char *)&reg_addr[0], sizeof(reg_addr)));
}
...
static int set_btc_esco_params(struct net_device *dev, bool trump_sco)
{
	...
	if (trump_sco) {
		...
		if ((!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 50, &saved_reg50))
		    &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 51, &saved_reg51))
		    &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 64, &saved_reg64))
		    &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 65, &saved_reg65))
		    &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 71, &saved_reg71))) {
			saved_status = TRUE;
...
int wl_cfg80211_set_btcoex_dhcp(struct net_device *dev, char *command)
{
	...
	char buf_reg66va_dhcp_on[8] =
	    { 66, 00, 00, 00, 0x10, 0x27, 0x00, 0x00 };
	char buf_reg41va_dhcp_on[8] =
	    { 41, 00, 00, 00, 0x33, 0x00, 0x00, 0x00 };
	char buf_reg68va_dhcp_on[8] =
	    { 68, 00, 00, 00, 0x90, 0x01, 0x00, 0x00 };
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		if ((saved_status == FALSE) &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 66, &saved_reg66))
		    &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 41, &saved_reg41))
		    &&
		    (!dev_wlc_intvar_get_reg
		     (dev, "btc_params", 68, &saved_reg68))) {
			saved_status = TRUE;
			...
			if (btcoex_is_sco_active(dev)) {
				...
				dev_wlc_bufvar_set(dev, "btc_params",
						   (char *)
						   &buf_reg66va_dhcp_on[0],
						   sizeof(buf_reg66va_dhcp_on));
				/* btc_params 41 0x33 */
				dev_wlc_bufvar_set(dev, "btc_params",
						   (char *)
						   &buf_reg41va_dhcp_on[0],
						   sizeof(buf_reg41va_dhcp_on));
				/* btc_params 68 0x190 */
				dev_wlc_bufvar_set(dev, "btc_params",
						   (char *)
						   &buf_reg68va_dhcp_on[0],
