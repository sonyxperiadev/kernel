...
static int dhdsdio_htclk(dhd_bus_t * bus, bool on, bool pendok)
{
	...
	if (on) {
		...
		if (pendok &&
		    ((bus->sih->buscoretype == PCMCIA_CORE_ID)
...
static int dhdsdio_txpkt(dhd_bus_t * bus, void *pkt, uint chan, bool free_pkt)
{
	...
	*(((uint16 *)frame) + 1) = htol16(~len);
	...
	swheader =
	    ((chan << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK) | bus->
	    tx_seq | (((pad1 + SDPCM_HDRLEN) << SDPCM_DOFFSET_SHIFT) &
		      SDPCM_DOFFSET_MASK);
	...
	*(((uint16 *)frame) + 1) = htol16(~msglen);
	...
	swheader =
	    ((SDPCM_CONTROL_CHANNEL << SDPCM_CHANNEL_SHIFT) &
	     SDPCM_CHANNEL_MASK)
	    | bus->
	    tx_seq | ((doff << SDPCM_DOFFSET_SHIFT) & SDPCM_DOFFSET_MASK);
	htol32_ua_store(swheader, frame + SDPCM_FRAMETAG_LEN);
	htol32_ua_store(0, frame + SDPCM_FRAMETAG_LEN + sizeof(swheader));
	...
	if (TXCTLOK(bus) && bus->ctrl_frame_stat
		...
		ret =
		    dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2,
					...
					(uint32)bus->ctrl_frame_len, NULL, NULL,
					NULL);
		ASSERT(ret != BCME_PENDING);
