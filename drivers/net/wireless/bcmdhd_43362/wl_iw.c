...
/* jiffies to ms timestamp macro */
#define JF2MS ((((jiffies / HZ) * 1000) + ((jiffies % HZ) * 1000) / HZ))

#ifdef COEX_DBG     /* 1- en coex trace */
...
#ifdef SCAN_DBG      /* 1- en scan trace */
...
bool btcoex_is_sco_active(struct net_device *dev);  /* BTCOEX fwd decl */
...
/*  ----- compile time Tunable  BT + WIFI DHCP coexistance parameters ---  */
#define  COEX_DHCP 1	/* enable bt coex during dhcp */
#ifdef COEX_DHCP

#define BT_DHCP_eSCO_FIX /* use New SCO/eSCO smart YG suppression  */
#define BT_DHCP_USE_FLAGS  /* this flag boost wifi pkt priority to max, caution: -not fair to sco */
#define BT_DHCP_OPPORTUNITY_WINDOW_TIME	 2500 /* T1 start SCO/ESCo priority suppression  */
#define BT_DHCP_FLAG_FORCE_TIME 5500 /* T2 turn off SCO/SCO supperesion is (timeout) */
...
typedef struct bt_info {
	...
	bool	dhcp_done; /* flag, indicates that host done with dhcp before t1/t2 expiration */
	int	bt_state;
...
/* 	Setting custome tx power in qdBm
	Note: firmware will NOT allowed to change power avove regulatory limited
*/
static int
wl_iw_set_txpower(
	struct net_device *dev,
...
)
{
	...
	if (country_offset != 0) {
		...
		/* Force disasoc for any new locale setting */
		...
		Locale rev maybe platform specific
		*/
...
)
{
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		/* ------ Framework is done with dhcp (ok or failed) ------ */
...
static int set_btc_esco_params(struct net_device *dev, bool trump_sco)
{
	...
	if (trump_sco) { /* this should reduce eSCO agressive retransmit w/o breaking it */
...
/*
 *   Setting PNO from private wpa_cli
 */
static int
...
)
{
	...
	/* change the cmd name to PNOSETUP */
	...
	tmp_size = size;
	/* translate SSID length from ASCII charater to number */
...
init_ap_profile_from_string(char *param_str, struct ap_profile *ap_cfg)
{
	...
	/* optional param, don't ret error if not found */
        ...
        char *extra)
{
	...
	for (i = 0; i < 8; i++) { /* wr a list of connected STA to z-string */
	...
	p_iwrq->data.length = strlen(mac_lst)+1; /* + Z term */
        ...
        char *ext)
{
	...
	struct mac_list_set mflist_set; /* mac filter structure */
...
#ifdef SOFTAP
/*
 * Disassociate particular or all station from AP
 * input: "STA_MAC=XXXXXXXXXXXXX", if empty or arbitrary str,
 * will disassociate all stations
 */
...
#if defined(CONFIG_FIRST_SCAN)
	/* only for firts scan reduce passive scan time */
...
)
{
	...
	memset(ssids_local, 0, sizeof(ssids_local));
	/* CSCAN TLV command must started with predifined
	* prefixes and first paramter should be for SSID
	*/
		...
		(cscan_tlv_temp->subver == CSCAN_TLV_SUBVERSION))
	{
		...
		if (g_first_broadcast_scan < BROADCAST_SCAN_FIRST_RESULT_CONSUMED) {
			if (++g_first_counter_scans == MAX_ALLOWED_BLOCK_SCAN_FROM_FIRST_SCAN) {
				/* OK, somehow Driver gets too many Scan request but somehow
				*  Supplicant didn't consume yet First Scan results
				*  So just clean up that flag and start to accept regular scans req
				*/
				WL_ERROR(("%s Clean up First scan flag which is %d\n",
					__FUNCTION__, g_first_broadcast_scan));
				g_first_broadcast_scan = BROADCAST_SCAN_FIRST_RESULT_CONSUMED;
...
#ifdef AP_ONLY
	if (ap_cfg_running) {
		wl_iw_softap_deassoc_stations(dev, NULL); /* 2nd argument -NULL: deassoc all */
		ap_cfg_running = FALSE;
	}
#endif	/* AP_ONLY */
...
static int wl_iw_softap_deassoc_stations(struct net_device *dev, u8 *mac)
{
	...
	/* if mac NULL , deauthenticate all current stations  */
	if (mac == NULL) {
		deauth_all = TRUE;
		sta_mac = z_mac;  /* dummy mac */
	} else {
		sta_mac = mac;  /* real one to deauth */
	}

	memset(assoc_maclist, 0, sizeof(mac_buf));
	assoc_maclist->count = 8; /* softap No of supported SCBs  */
	...
	if (assoc_maclist->count)
		...
		if (deauth_all || (memcmp(&scbval.ea, sta_mac, ETHER_ADDR_LEN) == 0))  {
			/* deauthenticate all stations on the list */
...
set_ap_mac_list(struct net_device *dev, void *buf)
{
	...
	int ioc_res = 0;
	ap_macmode = mac_list_set->mode;  /* save in a global var */

	/* 1st clear our global mac filtering list ist  */
	...
	if (mac_mode == MACLIST_MODE_DISABLED) {
		...
		/* save a copy for use from the event */
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				for (j = 0; j < maclist->count; j++)
					if (!bcmp(&assoc_maclist->ea[i], &maclist->ea[j],
						ETHER_ADDR_LEN)) {
						/* assoc client matches list entry[j] */
						assoc_mac_matched = TRUE;
						break;
					}

				/* now do conditional deauth dpending on the w/b list mode  */
...
static const iw_handler wl_iw_priv_handler[] = {
	...
	*/
	...
	NULL, /* disassoc ALL or one STA from AP */
...
static const struct iw_priv_args wl_iw_priv_args[] =
{
	...
	{
		WL_AP_STA_LIST,
		IW_PRIV_TYPE_CHAR | 0,	/* set params */
		IW_PRIV_TYPE_CHAR | 1024,  /*  get var size  */
		"AP_GET_STA_LIST"
	},
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
...
#if defined(WL_IW_USE_ISCAN)
		if (!g_iscan) {
			/* may occur if wifi turned off while scan is in progress */
	...
	if (cmd && dhd_bcm_whdmi_enable && strncmp(dev->name,
		...
		/*
		WL_TRACE(("wl_iw_event: not sending %s wlevent cmd=%u\n",
	...
	while (down_interruptible(&tsk_ctl->sema) == 0) {
		...
		switch (g_bt->bt_state) {
			...
			case BT_DHCP_OPPORTUNITY_WINDOW:
				...
				/*  enorce btc_params + flags if necessary  */
