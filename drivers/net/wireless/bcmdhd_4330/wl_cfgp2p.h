...
#define wl_get_p2p_status(wl, stat) ((!(wl)->p2p_supported) ? 0 : test_bit(WLP2P_STATUS_ ## stat, \
									&(wl)->p2p->status))
#define wl_set_p2p_status(wl, stat)						\
	do {									\
		if ((wl)->p2p_supported) {					\
			set_bit(WLP2P_STATUS_ ## stat, &(wl)->p2p->status);	\
		}								\
	} while (0)
#define wl_clr_p2p_status(wl, stat)						\
	do {									\
		if ((wl)->p2p_supported) {					\
			clear_bit(WLP2P_STATUS_ ## stat, &(wl)->p2p->status);	\
		}								\
	} while (0)
#define wl_chg_p2p_status(wl, stat)						\
	do {									\
		if ((wl)->p2p_supported) {					\
			change_bit(WLP2P_STATUS_ ## stat, &(wl)->p2p->status);	\
		}								\
	} while (0)
