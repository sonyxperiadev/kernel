...
SDIOH_API_RC sdioh_enable_hw_oob_intr(sdioh_info_t *sd, bool enable)
{
	...
	status =
	    sdioh_request_byte(sd, SDIOH_WRITE, 0, SDIOD_CCCR_BRCM_SEPINT,
			       &data);
	return status;
}
#endif /* defined(OOB_INTR_ONLY) && defined(HW_OOB) */
				...
				if (gInstance->func[2]) {
					...
					if (*byte & SDIO_FUNC_ENABLE_2) {
						...
						err_ret =
						    ...
						    (gInstance->func[2]);
						if (err_ret) {
							sd_err(("bcmsdh_sdmmc: enable F2 failed:%d", err_ret));
						}
					} else {
						/* Disable Function 2 */
						err_ret =
						    ...
						    (gInstance->func[2]);
						if (err_ret) {
							sd_err(("bcmsdh_sdmmc: Disab F2 failed:%d", err_ret));
						}
					}
					sdio_release_host(gInstance->func[2]);
				}
		     ...
		     uint addr, void *pkt)
{
	...
	if (0 != dma_len) {
		...
		pprev = pkt;
		for (pnext = pkt;
		     pnext && dma_len; pnext = PKTNEXT(sd->osh, pnext)) {
			...
			sg_set_buf(&sd->sg_list[SGCount++],
				   (uint8 *)PKTDATA(sd->osh, pnext), pkt_len);
	...
	if (0 != lft_len) {
		...
		for (pnext = pkt; pnext; pnext = PKTNEXT(sd->osh, pnext)) {
			...
			if ((write) && (!fifo))
				err_ret =
				    sdio_memcpy_toio(gInstance->func[func],
						     addr, buf, pkt_len);
			else if (write)
				err_ret =
				    sdio_memcpy_toio(gInstance->func[func],
				...
				err_ret =
				    sdio_memcpy_fromio(gInstance->func[func],
						       buf, addr, pkt_len);

			if (err_ret)
				sd_err(("%s: %s FAILED %p[%d], addr=0x%05x, pkt_len=%d, ERR=%d\n", __FUNCTION__, (write) ? "TX" : "RX", pnext, SGCount, addr, pkt_len, err_ret));
			else
				sd_trace(("%s: %s xfr'd %p[%d], addr=0x%05x, len=%d\n", __FUNCTION__, (write) ? "TX" : "RX", pnext, SGCount, addr, pkt_len));
		     ...
		     uint8 *buffer, void *pkt)
{
	...
	if (pkt == NULL) {
		...
		sd_data(("%s: Creating aligned %s Packet, len=%d\n",
			 __FUNCTION__, write ? "TX" : "RX", PKTLEN(sd->osh,
								   pkt)));
#ifdef CONFIG_DHD_USE_STATIC_BUF
		if (!
		    (mypkt =
		     PKTGET_STATIC(sd->osh, PKTLEN(sd->osh, pkt),
				   write ? TRUE : FALSE))) {
#else
		if (!
		    (mypkt =
		     PKTGET(sd->osh, PKTLEN(sd->osh, pkt),
			    write ? TRUE : FALSE))) {
#endif /* CONFIG_DHD_USE_STATIC_BUF */
			sd_err(("%s: PKTGET failed: len %d\n",
		...
		if (write) {
			bcopy(PKTDATA(sd->osh, pkt),
			      PKTDATA(sd->osh, mypkt), PKTLEN(sd->osh, pkt));
		}
		...
		if (!write) {
			bcopy(PKTDATA(sd->osh, mypkt),
			      PKTDATA(sd->osh, pkt), PKTLEN(sd->osh, mypkt));
		}
#ifdef CONFIG_DHD_USE_STATIC_BUF
		PKTFREE_STATIC(sd->osh, mypkt, write ? TRUE : FALSE);
#else
...
static void IRQHandler(struct sdio_func *func)
{
	...
	if (sd->use_client_ints) {
		...
		sd_err(("%s: Not ready for intr: enabled %d, handler %p\n",
			__FUNCTION__, sd->client_intr_enabled,
			sd->intr_handler));
	}
...
SDIOH_API_RC sdioh_gpioouten(sdioh_info_t *sd, uint32 gpio)
{
	return SDIOH_API_RC_FAIL;
}

SDIOH_API_RC sdioh_gpioout(sdioh_info_t *sd, uint32 gpio, bool enab)
{
	return SDIOH_API_RC_FAIL;
}
...
SDIOH_API_RC sdioh_gpio_init(sdioh_info_t *sd)
{
	return SDIOH_API_RC_FAIL;
}
