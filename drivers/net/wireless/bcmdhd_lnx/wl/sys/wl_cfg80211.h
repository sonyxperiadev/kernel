...

{
	...
	list_for_each_entry_safe(_net_info, next, &wl->net_list, list) {
		if (_net_info->ndev &&
			test_bit(status, &_net_info->status))
			cnt++;
	}
	...
	struct net_device *ndev, u32 op)
{
	...
	list_for_each_entry_safe(_net_info, next, &wl->net_list, list) {
		if (ndev && (_net_info->ndev == ndev)) {
			switch (op) {
				case 1:
					set_bit(status, &_net_info->status);
					break;
				case 2:
					clear_bit(status, &_net_info->status);
					break;
				case 4:
					change_bit(status, &_net_info->status);
					break;
			}
		}
	...
	struct net_device *ndev)
{
	...
	list_for_each_entry_safe(_net_info, next, &wl->net_list, list) {
				if (ndev && (_net_info->ndev == ndev))
					return test_bit(status, &_net_info->status);
	}
	return 0;
}
