...
osl_t *osl_attach(void *pdev, uint bustype, bool pkttag)
{
	...
	if (!bcm_static_buf) {
		if (!
		    (bcm_static_buf =
		     (bcm_static_buf_t *) dhd_os_prealloc(osh, 3,
							  STATIC_BUF_SIZE +
...
void *osl_ctfpool_add(osl_t *osh)
{
	struct sk_buff *skb;
...
void osl_ctfpool_replenish(osl_t *osh, uint thresh)
{
	if ((osh == NULL) || (osh->ctfpool == NULL))
		return;
...
int32 osl_ctfpool_init(osl_t *osh, uint numobj, uint size)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25))
	gfp_t flags;
...
void osl_ctfpool_cleanup(osl_t *osh)
{
...
void osl_ctfpool_stats(osl_t *osh, void *b)
{
	struct bcmstrbuf *bb;
...
static inline struct sk_buff *osl_pktfastget(osl_t *osh, uint len)
{
	struct sk_buff *skb;
...
#ifdef CTFPOOL
static inline void osl_pktfastfree(osl_t *osh, struct sk_buff *skb)
{
	ctfpool_t *ctfpool;
