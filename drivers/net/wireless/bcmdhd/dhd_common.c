 ...
 *
 * $Id: dhd_common.c 701290 2017-05-24 10:46:57Z $
 */
#include <typedefs.h>
#include <osl.h>
...
dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifidx, wl_ioctl_t *ioc, void *buf, int len)
{
	...
	if (dhd_os_proto_block(dhd_pub))
	{
		...
		if (!dhdpcie_runtime_bus_wake(dhd_pub, TRUE, dhd_wl_ioctl) && (dhd_pub->dhd_bus_busy_state & DHD_BUS_BUSY_RPM_ALL)) {
			DHD_ERROR(("%s: bus_wake returns FALSE on bus_busy_state %d\n",
				__FUNCTION__, dhd_pub->dhd_bus_busy_state));
		}
...
dhd_ndo_add_ip(dhd_pub_t *dhd, char* ipv6addr, int idx)
{
	...
	retcode = dhd_iovar(dhd, idx, "nd_hostip", ipv6addr,
		IPV6_ADDR_LEN, NULL, 0, TRUE);
...
dhd_ndo_remove_ip(dhd_pub_t *dhd, int idx)
{
	...
	retcode = dhd_iovar(dhd, idx, "nd_hostip_clear", NULL, 0,
		NULL, 0, TRUE);
