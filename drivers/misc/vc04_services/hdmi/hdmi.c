...
static int __devinit hdmi_pltfm_probe(struct platform_device *pdev)
{
   ...
   if ( ch->hw_cfg.gpio_hdmi_det >= 0 )
   {
      ...
      if (ret < 0)
      {
         dev_err( &pdev->dev, "Unable to set GPIO pin %d as input, reason %d\n",
                  ch->hw_cfg.gpio_hdmi_det, ret );
         //return ret;
      }
      
      ret = gpio_set_debounce(ch->hw_cfg.gpio_hdmi_det,128000);
      if (ret < 0)
      {
         dev_err( &pdev->dev, "Unable to set GPIO pin %d debounce, reason %d\n",
                  ch->hw_cfg.gpio_hdmi_det, ret );
         //return ret;
      }
...
static struct platform_device gPlatform_device = 
{
	.name		= "hdmi0",
	.dev		= 
	{
		.platform_data	= &gHDMI[1],
	},
	.id		= -1,
};
...
#endif
	 rc = platform_device_register(&gPlatform_device);
   if ( rc < 0 )
   {
