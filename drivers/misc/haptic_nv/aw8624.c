			   ...
			   unsigned char *buf, unsigned int len)
{
	...
	ret = i2c_master_send(aw8624->i2c, &reg_addr, AW_I2C_BYTE_ONE);
	if (ret < 0) {
		aw_dev_err("%s: couldn't send addr:0x%02x, ret=%d\n",
			   __func__, reg_addr, ret);
		return ret;
	}
	ret = i2c_master_recv(aw8624->i2c, buf, len);
	if (ret != len) {
...
static int aw8624_rtp_osc_calibration(struct aw8624 *aw8624)
{
	...
	/* haptic start */
	aw8624_haptic_start(aw8624);
	cpu_latency_qos_add_request(&aw8624_pm_qos_req_vb, AW8624_PM_QOS_VALUE_VB);
	while (1) {
