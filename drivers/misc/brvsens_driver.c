...
/* Utility: Search for first active node with same name, but different handle
   Helps us identify sensor registrations where single driver supports multiple Sensors */
...
/* Helper to deactivate existing node. We terminate workqueue, but leave
   work structure for next time around. This is always initiated when last sensor
   reference in User Land has been cleared, as part of user app termination
...
static int __init brvsens_init(void)
{
    ...
    dev = device_create( brvsens_data.class, NULL, brvsens_data.devnum, NULL, DRIVER_NAME );
    if ( IS_ERR( dev ))
    {
        rc = PTR_ERR( dev );
...
/* Sensor Registration: Export Where specific drivers register with BRVSENS device */
int brvsens_register
(
    u32                handle,   // Sensor UID, must be provided and unique
    const char*        name,     // Name: Must be provided, duplicates OK (i.e. different sensors from same driver)
...
// which takes one-time snapshot at SensorService initialization & 
