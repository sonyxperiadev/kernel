...
static int mpu_device_setup(struct device *dev,
				struct mpu3050_platform_data *pdata, int on)
{
	int result = ML_SUCCESS;

	if (on) {
		result = pdata->hw_config(dev, 1);
		if (result)
			goto exit;
		usleep_range(4000, 5000); /* by Spec */
		result = pdata->setup(dev, 1);
		if (result)
			goto exit;
	} else {
		result = pdata->setup(dev, 0);
		if (result)
			goto exit;
		result = pdata->hw_config(dev, 0);
		if (result)
			goto exit;
	}
exit:
...
void mpu_shutdown(struct i2c_client *client)
{
	...
	(void) mpu3050_suspend(mldl_cfg, this_client->adapter,
			       ...
			       TRUE, TRUE, TRUE, TRUE);
	(void) mpu_device_setup(&client->dev, mldl_cfg->pdata, 0);
	mutex_unlock(&mpu->mutex);
	dev_dbg(&this_client->adapter->dev, "%s\n", __func__);
}
...
int mpu_resume(struct i2c_client *client)
{
	...
	if (mpu->pid && !mldl_cfg->ignore_system_suspend) {
		unsigned long sensors = mldl_cfg->requested_sensors;
		if (mpu_device_setup(&client->dev, mldl_cfg->pdata, 1))
			return -ENODEV;
		  ...
		  const struct i2c_device_id *devid)
{
	...
	if (!pdata->setup || !pdata->hw_config) {
		dev_err(&this_client->adapter->dev,
			"No HW configuration setup functions for mpu3050.\n");
		res = -ENODEV;
		goto out_whoami_failed;
	}

	res =  mpu_device_setup(&client->dev, mldl_cfg->pdata, 1);
	if (res) {
		res = -ENODEV;
		goto out_whoami_failed;
	}
