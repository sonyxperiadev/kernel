		    ...
		    struct ext_slave_platform_data *pdata, unsigned char *data)
{
	...
	if (stat & 0x01) {
		...
		/* NOTE/IMPORTANT: lsm303dlm compass axis definition
		   does not respect the right hand rule.
		   We invert the sign of the Z axis to
		   fix that. */
		axisFixed = (short)(-1 * axisFixed * 36);
		data[4] = (unsigned short)axisFixed >> 8;
		data[5] = (unsigned short)axisFixed & 0xFF;
...
EXPORT_SYMBOL(lsm303dlm_m_get_slave_descr);
