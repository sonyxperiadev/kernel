 ...
 *  @{
 *      @file   ak89xx.c
 *      @brief  Magnetometer setup and handling methods for the AKM AK89xx compass device.
 */
		       ...
		       struct ext_slave_platform_data *pdata)
{
	...
	private_data = (struct ak89xx_private_data *)
	    MLOSMalloc(sizeof(struct ak89xx_private_data));
		...
		struct ext_slave_platform_data *pdata, unsigned char *data)
{
	...
	result =
	    MLSLSerialRead(mlsl_handle, pdata->address, AK89XX_REG_ST1,
			    8, regs);
	ERROR_CHECK(result);
...
EXPORT_SYMBOL(ak89xx_get_slave_descr);
