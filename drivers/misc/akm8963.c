...
#define AKM_DRDY_TIMEOUT_MS		200
...
static int AKECS_Set_CNTL(struct akm_compass_data *akm, uint8_t mode)
{
	...
	if (akm->is_busy > 0) {
		...
		if (err < 0) {
			...
			dev_vdbg(&akm->i2c->dev, "Mode is set to (%d).", mode);
			/* Set flag */
			akm->is_busy = 1;
...
static int AKECS_Set_PowerDown(struct akm_compass_data *akm)
{
	uint8_t buffer[2];
	int err;
...
static int AKECS_SetMode(struct akm_compass_data *akm, uint8_t mode)
{
	int err;
...
static void AKECS_SetYPR(struct akm_compass_data *akm, int *rbuf)
{
	uint32_t ready;
 ...
 * data is available.
 */
static int AKECS_GetData(struct akm_compass_data *akm, uint8_t *rbuf, int size)
{
	int err;
...
static int AKECS_GetData_Poll(struct akm_compass_data *akm,
			      uint8_t *rbuf, int size)
{
...
static int AKECS_GetOpenStatus(struct akm_compass_data *akm)
{
	return wait_event_interruptible(akm->open_wq,
					(atomic_read(&akm->active) > 0));
}

static int AKECS_GetCloseStatus(struct akm_compass_data *akm)
{
	return wait_event_interruptible(akm->open_wq,
					(atomic_read(&akm->active) <= 0));
}
...
static long AKECS_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_GET_DATA:
		...
		if (akm->irq)
			...
			ret =
			    AKECS_GetData_Poll(akm, dat_buf,
...
/***** sysfs enable *************************************************/
static void akm_compass_sysfs_update_status(struct akm_compass_data *akm)
{
	uint32_t en;
	mutex_lock(&akm->val_mutex);
	en = akm->enable_flag;
...
static ssize_t akm_compass_sysfs_enable_show(struct akm_compass_data *akm,
					     char *buf, int pos)
{
	int flag;
...
static ssize_t akm_compass_sysfs_enable_store(struct akm_compass_data *akm,
...
/***** Acceleration ***/
static ssize_t akm_enable_acc_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return akm_compass_sysfs_enable_show(dev_get_drvdata(dev), buf,
					     ACC_DATA_FLAG);
}
static ssize_t akm_enable_acc_store(struct device *dev,
				    struct device_attribute *attr,
				    char const *buf, size_t count)
{
	return akm_compass_sysfs_enable_store(dev_get_drvdata(dev), buf, count,
					      ACC_DATA_FLAG);
}

/***** Magnetic field ***/
static ssize_t akm_enable_mag_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return akm_compass_sysfs_enable_show(dev_get_drvdata(dev), buf,
					     MAG_DATA_FLAG);
}
static ssize_t akm_enable_mag_store(struct device *dev,
				    struct device_attribute *attr,
				    char const *buf, size_t count)
{
	return akm_compass_sysfs_enable_store(dev_get_drvdata(dev), buf, count,
...
/***** Fusion ***/
static ssize_t akm_enable_fusion_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	return akm_compass_sysfs_enable_show(dev_get_drvdata(dev), buf,
					     FUSION_DATA_FLAG);
}
static ssize_t akm_enable_fusion_store(struct device *dev,
				       struct device_attribute *attr,
				       char const *buf, size_t count)
{
	return akm_compass_sysfs_enable_store(dev_get_drvdata(dev), buf, count,
...
/***** sysfs delay **************************************************/
static ssize_t akm_compass_sysfs_delay_show(struct akm_compass_data *akm,
					    char *buf, int pos)
{
	int64_t val;
...
static ssize_t akm_compass_sysfs_delay_store(struct akm_compass_data *akm,
					     char const *buf, size_t count,
					     int pos)
{
	long long val = 0;
...
/***** Accelerometer ***/
static ssize_t akm_delay_acc_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return akm_compass_sysfs_delay_show(dev_get_drvdata(dev), buf,
					    ACC_DATA_FLAG);
}
static ssize_t akm_delay_acc_store(struct device *dev,
				   struct device_attribute *attr,
				   char const *buf, size_t count)
{
	return akm_compass_sysfs_delay_store(dev_get_drvdata(dev), buf, count,
					     ACC_DATA_FLAG);
}

/***** Magnetic field ***/
static ssize_t akm_delay_mag_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return akm_compass_sysfs_delay_show(dev_get_drvdata(dev), buf,
					    MAG_DATA_FLAG);
}
static ssize_t akm_delay_mag_store(struct device *dev,
				   struct device_attribute *attr,
				   char const *buf, size_t count)
{
	return akm_compass_sysfs_delay_store(dev_get_drvdata(dev), buf, count,
...
/***** Fusion ***/
static ssize_t akm_delay_fusion_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return akm_compass_sysfs_delay_show(dev_get_drvdata(dev), buf,
					    FUSION_DATA_FLAG);
}
static ssize_t akm_delay_fusion_store(struct device *dev,
				      struct device_attribute *attr,
				      char const *buf, size_t count)
{
	return akm_compass_sysfs_delay_store(dev_get_drvdata(dev), buf, count,
...
/***** accel (binary) ***/
static ssize_t akm_bin_accel_write(struct file *file,
				   struct kobject *kobj,
...
#if AKM_DEBUG_IF
static ssize_t akm_sysfs_mode_store(struct device *dev,
				    struct device_attribute *attr,
				    char const *buf, size_t count)
{
	struct akm_compass_data *akm = dev_get_drvdata(dev);
	...
	if (strtol(buf, AKM_BASE_NUM, &mode))
		return -EINVAL;
...
static ssize_t akm8963_sysfs_bdata_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
...
static ssize_t akm8963_sysfs_asa_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	/* AK8963 specific function */
	struct akm_compass_data *akm = dev_get_drvdata(dev);
...
static ssize_t akm8963_sysfs_regs_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	/* AK8963 specific function */
...
static int create_sysfs_interfaces(struct akm_compass_data *akm)
{
	...
	err = create_device_attributes(akm->class_dev, akm_compass_attributes);
	if (0 > err)
...
static void remove_sysfs_interfaces(struct akm_compass_data *akm)
{
	...
	if (NULL != akm->class_dev) {
		remove_device_binary_attributes(&akm->class_dev->kobj,
						akm_compass_bin_attributes);
		remove_device_attributes(akm->class_dev,
...
static int akm_compass_input_init(struct input_dev **input)
{
	...
	*input = input_allocate_device();
	...
	input_set_abs_params(*input, ABS_HAT1Y, -5760, 5760, 0, 0);
	/* Rotation Vector [-1,+1] in Q14 format */
	input_set_abs_params(*input, ABS_TILT_X, -16384, 16384, 0, 0);
	input_set_abs_params(*input, ABS_TILT_Y, -16384, 16384, 0, 0);
	input_set_abs_params(*input, ABS_TOOL_WIDTH, -16384, 16384, 0, 0);
	input_set_abs_params(*input, ABS_VOLUME, -16384, 16384, 0, 0);
...
static void akm8963_delayed_work(struct work_struct *work)
{
	struct akm_compass_data *akm =
		container_of(work, struct akm_compass_data, work);
...
static int akm8963_i2c_check_device(struct i2c_client *client)
{
	...
	for (i = 0; i < 20; i++) {
		err = akm_i2c_rxdata(client, akm->sense_info,
			AKM_SENSOR_INFO_SIZE);
		if (err < 0) {
			dev_err(&client->dev, "Can not read WIA.\n");
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		if (client->dev.of_node) {
			...
			pr_err("[akm8963]Set layout fail!!!\n");
	}
	...
	if (s_akm->irq == 0) {
		...
		err = gpio_request(s_akm->irq_gpio, "akm_irq");
		if (err != 0)
			dev_err(&client->dev, "gpio request irq fail\n");

		err = gpio_direction_input(s_akm->irq_gpio);
		if (err != 0)
