			   ...
			   struct ext_slave_platform_data *pdata)
{
	...
	/* NOTE :
	   we don't put the device to sleep as the gyros and/or DMP
	   might still be requested. To do so, we rely on the
	   mpu60xx_pwr_mgmt function called from mpu_resume in mldl_cfg.c.
	   */
			  ...
			  struct ext_slave_platform_data *pdata)
{
	...
	if (regs[0] & BIT_SLEEP) {
		...
		/* clear the device suspended flag to keep track of the state */
		private_data->mldl_cfg_ref->inv_mpu_state->status &=
			~MPU_DEVICE_IS_SUSPENDED;
	}
	msleep(2);

	if ((regs[1] & (BIT_STBY_XA | BIT_STBY_YA | BIT_STBY_ZA)) != 0) {
		regs[1] &= ~(BIT_STBY_XA | BIT_STBY_YA | BIT_STBY_ZA);
		result = inv_serial_single_write(mlsl_handle, pdata->address,
						 MPUREG_PWR_MGMT_2, regs[1]);
		if (result) {
			LOG_RESULT_LOCATION(result);
			return result;
		}
	...
	regs[0] = (unsigned char)private_data->suspend.ths / ACCEL_MOT_THR_LSB;
	result = inv_serial_single_write(mlsl_handle, pdata->address,
					 MPUREG_ACCEL_MOT_THR, regs[0]);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
	}
	regs[0] = (unsigned char)
	    ACCEL_ZRMOT_THR_LSB_CONVERSION(private_data->resume.ths);
	result = inv_serial_single_write(mlsl_handle, pdata->address,
					 MPUREG_ACCEL_ZRMOT_THR, regs[0]);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
	}
	regs[0] = (unsigned char)private_data->suspend.ths / ACCEL_MOT_DUR_LSB;
	result = inv_serial_single_write(mlsl_handle, pdata->address,
					 MPUREG_ACCEL_MOT_DUR, regs[0]);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
	}
	regs[0] = (unsigned char)private_data->resume.ths / ACCEL_ZRMOT_DUR_LSB;
	result = inv_serial_single_write(mlsl_handle, pdata->address,
					 MPUREG_ACCEL_ZRMOT_DUR, regs[0]);
	if (result) {
		LOG_RESULT_LOCATION(result);
		return result;
	}
