...
/*0x*/	2302,1631,1326,1137,1038,964,905,856,815,779,747,718,693,669,647,627,
...
static ssize_t kona_thermal_get_name( struct device *dev, struct device_attribute *devattr, char *buf)
{
...
static ssize_t kona_thermal_get_threshold( struct device *dev, struct device_attribute *devattr, char *buf)
{
        int long raw = 0;
        struct platform_drvdata *pdata = dev_get_drvdata(dev);
        struct sensor_device_attribute_2 *attr = to_sensor_dev_attr_2(devattr);
...
static ssize_t kona_thermal_set_threshold( struct device* dev, struct device_attribute *devattr, const char *buf, size_t count)
{
        unsigned long raw;
        struct platform_drvdata *pdata = dev_get_drvdata(dev);
        struct sensor_device_attribute_2 *attr = to_sensor_dev_attr_2(devattr);
...
static ssize_t kona_thermal_get_alarm( struct device* dev, struct device_attribute *devattr, char *buf)
{
        struct platform_drvdata *pdata = dev_get_drvdata(dev);
...
void thermal_interval_wake(unsigned long var)
{
    ...
    mod_timer(&thermal_timer,jiffies+(board->thermal_update_interval)*HZ);
}

static ssize_t kona_thermal_set_interval( struct device* dev, struct device_attribute *devattr, const char *buf, size_t count)
{
    unsigned long raw;
    struct platform_drvdata *pdata = dev_get_drvdata(dev);
    struct therm_data               *board;
...
static ssize_t kona_thermal_get_interval( struct device* dev, struct device_attribute *devattr, char *buf)
{
    unsigned raw;
    struct platform_drvdata *pdata = dev_get_drvdata(dev);
    struct therm_data               *board;
...
#define SENSOR_TEMP(ix) \
	...
	SENSOR_ATTR_2(temp##ix##_max, S_IRUGO | S_IWUSR, \
		kona_thermal_get_threshold, kona_thermal_set_threshold, 1, ix-1), \
	SENSOR_ATTR_2(temp##ix##_crit, S_IRUGO | S_IWUSR, \
		kona_thermal_get_threshold, kona_thermal_set_threshold, 2, ix-1), \
	SENSOR_ATTR_2(temp##ix##_alarm, S_IRUGO, \
		kona_thermal_get_alarm, NULL, 0, ix-1)

static struct sensor_device_attribute_2 kona_attrs[] = {
    SENSOR_ATTR_2(update_interval, S_IWUSR | S_IRUGO, kona_thermal_get_interval, kona_thermal_set_interval, 0, 0),
	SENSOR_TEMP(1),
	SENSOR_TEMP(2),
	SENSOR_TEMP(3),
	SENSOR_TEMP(4)
...
void kona_thermal_register_notify( unsigned therm_id, struct notifier_block* thermal_block)
{
    therm_debug("thermal_register_notify (therm_id=%d)\n", therm_id);
    atomic_notifier_chain_register(&therm_notifier[therm_id-1], thermal_block);
}
...
static int kona_thermal_probe(struct platform_device *pdev)
{
		...
		if (board->thermal_update_interval != 0)
        {
            ...
            init_timer(&thermal_timer);
            thermal_timer.function = thermal_interval_wake;
            thermal_timer.expires = jiffies+(board->thermal_update_interval)*HZ;
            add_timer(&thermal_timer);
        }
#endif
...
DEFINE_SIMPLE_ATTRIBUTE(set_thermal_test_notify, NULL, set_thermal_notification, "%llu\n");
