	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (egis) {
		dev_dbg(&egis->pd->dev, "%s pd_show\n", __func__);
		if (egis->pd)
			dev_dbg(&egis->pd->dev, "%s pd show\n", __func__);
	} else
...
static DEVICE_ATTR(navigation_enable, S_IRUSR|S_IWUSR, property_navigation_enable_get, property_navigation_enable_set);
...
void sysfs_egis_init(struct egis_data *egis)
{
	...
	egis->pd = platform_device_alloc("egis_input", -1);
	if (!egis->pd) {
		pr_err("Egis navigation driver, platform_device_alloc fail\n");
		return;
	}
	status = platform_device_add(egis->pd);
	if (status != 0) {
		pr_err("Egis navigation driver, platform_device_add fail\n");
		platform_device_put(egis->pd);
		return;
	}
	dev_set_drvdata(&egis->pd->dev, egis);
	status = sysfs_create_group(&egis->pd->dev.kobj, &attribute_group);
	if (status) {
		pr_err("Egis navigation driver, could not create sysfs\n");
		platform_device_del(egis->pd);
		platform_device_put(egis->pd);
		return;
	}
}
