...
{
    printk( KERN_INFO "vchiq_memdrv_kona: firmware downloaded - connecting\n" );
...
static int __devinit vchiq_memdrv_kona_interface_probe( struct platform_device *pdev )
{
    VCHIQ_PLATFORM_DATA_MEMDRV_KONA_T *platform_data = pdev->dev.platform_data;
    ...
    */
    ...
    {
        printk( KERN_ERR "vchiq_memdrv_kona: Failed to create vchiq instance for '%s'\n",
                name );
...
static int vchiq_memdrv_kona_interface_suspend( struct platform_device *pdev, pm_message_t state )
{
    VCHIQ_PLATFORM_DATA_MEMDRV_KONA_T *platform_data = pdev->dev.platform_data;
