...
static int vc_mem_mmap( struct file *filp, struct vm_area_struct *vma )
{
    ...
    rc = remap_pfn_range( vma, vma->vm_start,
                         (mm_vc_mem_phys_addr >> PAGE_SHIFT) + vma->vm_pgoff,
...
static int vc_mem_proc_read( char *buf, char **start, off_t offset, int count, int *eof, void *data )
{
    char *p = buf;
...
static int __init vc_mem_init( void )
{
    ...
    printk( KERN_INFO "vc-mem: mm_vc_mem_phys_addr = 0x%08lx\n", mm_vc_mem_phys_addr );

    vc_mem_get_size();
    if ( mm_vc_mem_size == 0 )
    {
       printk( KERN_INFO "vc-mem: mm_vc_mem_size      = not available until videocore is booted\n" );
    }
    else
    {
       printk( KERN_INFO "vc-mem: mm_vc_mem_size      = 0x%08x (%u MiB)\n",
               mm_vc_mem_size, mm_vc_mem_size / (1024 * 1024));
    }
