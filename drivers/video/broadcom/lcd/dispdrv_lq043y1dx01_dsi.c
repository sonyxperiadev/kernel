...
// LOCAL FUNCTIONs
static void lq043y1dx01_WrCmndP0(DISPDRV_HANDLE_T drvH, UInt32 reg);

// DRV INTERFACE FUNCTIONs
Int32 LQ043Y1DX01_Init(struct dispdrv_init_parms *parms,
...
Int32 LQ043Y1DX01_GetDispDrvFeatures(DISPDRV_HANDLE_T drvH,
...
Int32 LQ043Y1DX01_Start(DISPDRV_HANDLE_T drvH,
			struct pi_mgr_dfs_node *dfs_node);
...
Int32 LQ043Y1DX01_Update(DISPDRV_HANDLE_T drvH,
...
static int lq043y1dx01_send_cmd(struct spi_device *spi,
				const u16 *cmd_buff, u32 byte_count)
{
	if (spi == NULL) {
...
//
//*****************************************************************************
Int32 lq043y1dx01_WinSet(DISPDRV_HANDLE_T drvH,
...
Int32 LQ043Y1DX01_Open(DISPDRV_HANDLE_T drvH)
{
	...
	if (brcm_enable_dsi_pll_clocks(pPanel->busNo,
				       pPanel->dsi_cfg->hsBitClk.clkIn_MHz *
				       1000000,
				       pPanel->dsi_cfg->hsBitClk.clkInDiv,
				       pPanel->dsi_cfg->escClk.clkIn_MHz *
				       1000000 /
...
//
//*****************************************************************************
static int lq043y1dx01_WrSendCmnd(DISPDRV_HANDLE_T drvH,
...
//                   
//*****************************************************************************
static void lq043y1dx01_ExecCmndList(DISPDRV_HANDLE_T drvH,
...
//
//*****************************************************************************
Int32 LQ043Y1DX01_Start(DISPDRV_HANDLE_T drvH, struct pi_mgr_dfs_node *dfs_node)
{
	LQ043Y1DX01_PANEL_t *pPanel = (LQ043Y1DX01_PANEL_t *)drvH;

	if (brcm_enable_dsi_lcd_clocks(dfs_node, pPanel->busNo,
				       lq043y1dx01_dsiCfg.hsBitClk.clkIn_MHz *
				       1000000,
				       lq043y1dx01_dsiCfg.hsBitClk.clkInDiv,
				       lq043y1dx01_dsiCfg.escClk.clkIn_MHz *
				       1000000 /
