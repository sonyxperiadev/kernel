...
*
* @file drivers/video/broadcom/dispdrv_otm1281a_dsi.c
*
...
#define OTM1281A_VC		(0)
#define OTM1281A_CMND_IS_LP TRUE  /* display init comm LP or HS mode */


#if 0
#define OTM1281A_LOG(id, fmt, args...) printk(KERN_ERR fmt, ##args)
#else
#define OTM1281A_LOG(id, fmt, args...)
#endif
...
#if 0
/* LOCAL FUNCTIONs */
static void OTM1281A_WrCmndP0(
	DISPDRV_HANDLE_T drvH,
	UInt32 reg);
#endif

/* DRV INTERFACE FUNCTIONs */
static Int32 OTM1281A_Init(
	struct dispdrv_init_parms	*parms,
	DISPDRV_HANDLE_T		*handle);

static Int32 OTM1281A_Exit(DISPDRV_HANDLE_T drvH);

static Int32 OTM1281A_Open(DISPDRV_HANDLE_T drvH);

static Int32 OTM1281A_Close(DISPDRV_HANDLE_T drvH);

static void OTM1281A_ExecCmndList(
	DISPDRV_HANDLE_T     drvH,
	pDISPCTRL_REC_T cmnd_lst);

static Int32 OTM1281A_GetDispDrvFeatures(
	DISPDRV_HANDLE_T drvH,
...
const DISPDRV_INFO_T *OTM1281A_GetDispDrvData(DISPDRV_HANDLE_T drvH);

static Int32 OTM1281A_Start(
	DISPDRV_HANDLE_T drvH,
	struct pi_mgr_dfs_node *dfs_node);

static Int32 OTM1281A_Stop(
	DISPDRV_HANDLE_T drvH,
	struct pi_mgr_dfs_node *dfs_node);

static Int32 OTM1281A_PowerControl(
	DISPDRV_HANDLE_T drvH,
	DISPLAY_POWER_STATE_T state);

static Int32 OTM1281A_Atomic_Update(
	DISPDRV_HANDLE_T drvH,
	void			*buff,
	DISPDRV_WIN_t		*p_win);

static Int32 OTM1281A_Update(
	 DISPDRV_HANDLE_T drvH,
	 void			*buff,
	 DISPDRV_WIN_t		*p_win,
	 DISPDRV_CB_T apiCb);

static Int32 OTM1281A_WinReset(DISPDRV_HANDLE_T drvH);


static DISPDRV_T OTM1281A_Drv = {
	&OTM1281A_Init,		/* init	*/
	&OTM1281A_Exit,		/* exit	*/
	&OTM1281A_GetDispDrvFeatures,/* info	*/
	&OTM1281A_Open,		/* open	*/
	&OTM1281A_Close,		/* close */
	&OTM1281A_GetDispDrvData,	/* get_info */
	&OTM1281A_Start,		/* start */
	&OTM1281A_Stop,		/* stop	*/
	&OTM1281A_PowerControl,	/* power_control */
	&OTM1281A_Atomic_Update,	/* update_no_os	*/
	&OTM1281A_Update,		/* update */
	NULL,				/* set_brightness */
	&OTM1281A_WinReset,		/* reset_win */
};
...
static DISPDRV_INFO_T OTM1281A_Info = {
...
CSL_DSI_CM_VC_t OTM1281A_VCCmCfg = {
	OTM1281A_VC,			/* VC */
...
static OTM1281A_PANEL_t panel[1];

/*###########################################################################*/

static void OTM1281A_panel_on(OTM1281A_PANEL_t *pPanel)
{
	DISPCTRL_REC_T cmd_list[] = {
		{DISPCTRL_WR_CMND, OTM1281A_CMD_DISPON, 0},
		{DISPCTRL_LIST_END, 0, 0},
	};

	OTM1281A_ExecCmndList(pPanel, cmd_list);
	return;
}

static void OTM1281A_panel_off(OTM1281A_PANEL_t *pPanel)
{
	DISPCTRL_REC_T cmd_list[] = {
		{DISPCTRL_WR_CMND, OTM1281A_CMD_DISPOFF, 0},
		{DISPCTRL_LIST_END, 0, 0},
	};
...
static void OTM1281A_panel_sleep_in(OTM1281A_PANEL_t *pPanel)
{
	DISPCTRL_REC_T cmd_list[] = {
		{DISPCTRL_WR_CMND, OTM1281A_CMD_SLPIN, 0},
		{DISPCTRL_LIST_END, 0, 0},
	};
...
static void OTM1281A_panel_sleep_out(OTM1281A_PANEL_t *pPanel)
{
	DISPCTRL_REC_T cmd_list[] = {
		{DISPCTRL_WR_CMND, OTM1281A_CMD_SLPOUT, 0},
		{DISPCTRL_LIST_END, 0, 0},
	};
...
static void OTM1281A_panel_init(OTM1281A_PANEL_t *pPanel)
{

	DISPCTRL_REC_T cmd_list[] = {

		{DISPCTRL_WR_CMND,	0xff, 0},
		{DISPCTRL_WR_DATA,	0, 0x12 },
		{DISPCTRL_WR_DATA,	0, 0x80 },
		{DISPCTRL_WR_DATA,	0, 0x01 },
		
		{DISPCTRL_WR_CMND_DATA, 0x00, 0x80},
		{DISPCTRL_WR_CMND,	0xff, 0},
		{DISPCTRL_WR_DATA,	0, 0x12 },
		{DISPCTRL_WR_DATA,	0, 0x80 },
		
		{DISPCTRL_WR_CMND_DATA, 0x00, 0x84},		
		{DISPCTRL_WR_CMND,	0xff, 0},
		{DISPCTRL_WR_DATA,	0, 0x10 },
		{DISPCTRL_WR_DATA,	0, 0x02 },
		
		{DISPCTRL_WR_CMND_DATA, 0x00, 0x90},
		...
		{DISPCTRL_WR_CMND_DATA, 0x00, 0xa0},
		{DISPCTRL_WR_CMND,	0xc5, 0},
		{DISPCTRL_WR_DATA,	0, 0x20 },
		{DISPCTRL_WR_DATA,	0, 0x6F },
		{DISPCTRL_WR_DATA,	0, 0x02 },		
		...
		{DISPCTRL_WR_CMND_DATA, 0x00, 0x00},
		{DISPCTRL_WR_CMND,	0xd8, 0},
		{DISPCTRL_WR_DATA,	0, 0x58 },
		{DISPCTRL_WR_DATA,	0, 0x00 },
		{DISPCTRL_WR_DATA,	0, 0x58 },		
		{DISPCTRL_WR_DATA,	0, 0x00 },
		
		{DISPCTRL_WR_CMND_DATA, 0x00, 0xb8},
		...
		{DISPCTRL_WR_CMND_DATA, 0x36, 0xD0},
...
/*
 *
 *   Function Name: OTM1281A_TeOn
 *
 *   Description:   Configure TE Input Pin & Route it to DSI Controller Input
 *
 */
static int OTM1281A_TeOn(OTM1281A_PANEL_t	*pPanel)
{
	Int32 res	= 0;
...
/*
 *
 *   Function Name: OTM1281A_TeOff
 *
 *   Description:   'Release' TE Input Pin Used
 *
 */
static int OTM1281A_TeOff(OTM1281A_PANEL_t *pPanel)
{
	Int32  res = 0;
...
/*
 *
 *   Function Name: OTM1281A_WrCmndP0
 *
 *   Description:
 *
 */
...
/*
 *
 *   Function Name: OTM1281A_ReadReg
 *
 *   Description:   DSI Read Reg
 *
 */
static int OTM1281A_ReadReg(DISPDRV_HANDLE_T drvH, UInt8 reg)
{
	OTM1281A_PANEL_t		*pPanel	= (OTM1281A_PANEL_t *)drvH;
	CSL_DSI_CMND_t msg;
	...
	if ((cslRes != CSL_LCD_OK) ||
		...
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	ERR"
			"Reading From Reg[0x%08X]\n\r",
			__func__, reg);
		res = -1;
	} else {
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	 OK"
...
/*
 *
 *   Function Name: OTM1281A_WinSet
 *
 *   Description:   Set Window
 *
 */
Int32 OTM1281A_WinSet(
	DISPDRV_HANDLE_T drvH,
	Boolean update,
	DISPDRV_WIN_t		*p_win)
{
 ...
 */
Int32 OTM1281A_WinReset(DISPDRV_HANDLE_T drvH)
{
	Int32 res;
	OTM1281A_PANEL_t *pPanel = (OTM1281A_PANEL_t *) drvH;

	printk("%s\n", __func__);
	res = OTM1281A_WinSet(drvH, TRUE, &pPanel->win_dim);
	return res;
}

/*
 *
 *   Function Name: OTM1281A_reset
 *
 *   Description:   (De)Assert display reset
 *
 */
static void OTM1281A_reset(DISPDRV_HANDLE_T drvH, Boolean on)
{
	OTM1281A_PANEL_t *pPanel = (OTM1281A_PANEL_t *) drvH;
	...
	if (!on) {
		OTM1281A_LOG(LCD_DBG_ERR_ID, "Resetting the panel\n");
		...
		OTM1281A_LOG(LCD_DBG_ERR_ID, "Powering off the panel\n");
		gpio_set_value_cansleep(pPanel->rst_panel_reset, reset_active);
		udelay(15);
		gpio_set_value_cansleep(pPanel->rst_panel_reset, !reset_active);
		msleep(5);
...
/*
 *
 *   Function Name: LCD_DRV_OTM1281A_GetDrvInfo
 *
 *   Description:   Get Driver Funtion Table
 *
 */
DISPDRV_T *DISP_DRV_OTM1281A_GetFuncTable(void)
{
	return &OTM1281A_Drv;
}
...
{
	Int32 res = 0;
	OTM1281A_PANEL_t	*pPanel;

	pPanel = &panel[0];

	if (pPanel->drvState ==	DRV_STATE_OFF)	{

		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: Bus %d\n",
			__func__, parms->w0.bits.bus_type);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: BootMode %d\n",
			__func__, parms->w0.bits.boot_mode);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: BusNo %d\n",
			__func__, parms->w0.bits.bus_no);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: col_mode_i %d\n",
			__func__, parms->w0.bits.col_mode_i);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: col_mode_o %d\n",
			__func__, parms->w0.bits.col_mode_o);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: te_input %d\n",
			__func__, parms->w0.bits.te_input);

		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: API Rev %d\n",
			__func__, parms->w1.bits.api_rev);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: Rst 0 %d\n",
			__func__, parms->w1.bits.lcd_rst0);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: Rst 1 %d\n",
			__func__, parms->w1.bits.lcd_rst1);
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: Rst 2 %d\n",
			__func__, parms->w1.bits.lcd_rst2);

		if ((u8)parms->w1.bits.api_rev != KONA_LCD_BOOT_API_REV) {
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s:"
		...
		pPanel->cmnd_mode = &OTM1281A_VCCmCfg;
		pPanel->dsi_cfg	  = &OTM1281A_dsiCfg;
		pPanel->disp_info = &OTM1281A_Info;

		pPanel->busNo =	dispdrv2busNo(parms->w0.bits.bus_no);

		/* check for valid DSI bus no */
		if (pPanel->busNo & 0xFFFFFFFE)	{
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	Invlid DSI "
		...
		case LCD_IF_CM_I_RGB888U:
			...
			break;
		default:
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	Unsupported"
				" Color Mode\n", __func__);
			return -1;
		}
		...
		*handle	= (DISPDRV_HANDLE_T)pPanel;
		...
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s: OK\n", __func__);
	} else {
		OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s: Not in OFF state\n",
			__func__);
		res = -1;
	}
...
/*
 *
 *   Function Name: OTM1281A_Exit
 *
 *   Description:
 *
 */
Int32 OTM1281A_Exit(DISPDRV_HANDLE_T drvH)
{
	OTM1281A_PANEL_t *pPanel;

	pPanel = (OTM1281A_PANEL_t *) drvH;
...
/*
 *
 *  Function Name: OTM1281A_Open
 *
 *  Description:   disp bus ON
 *
 */
Int32 OTM1281A_Open(DISPDRV_HANDLE_T drvH)
{
	Int32 res = 0;
	OTM1281A_PANEL_t	*pPanel;
	...
	OTM1281A_LOG(LCD_DBG_ERR_ID, "OTM1281A_Open\n");

	if (pPanel->drvState !=	DRV_STATE_INIT)	{
		OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s: ERROR State != Init\n",
			__func__);
		return -1;
	}
	...
	if (brcm_enable_dsi_pll_clocks(pPanel->busNo,
		...
		OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s:ERROR to enable the "
			"clock\n", __func__);
	}

	if (pPanel->isTE && OTM1281A_TeOn(pPanel) == -1) {
		OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s:"
	...
	if (CSL_DSI_Init(pPanel->dsi_cfg) != CSL_LCD_OK) {
		OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s:ERROR, DSI CSL Init "
			"Failed\n", __func__);
		goto err_dsi_init;
	}
	...
	if (CSL_DSI_OpenCmVc(pPanel->clientH,
		...
		OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV] %s:CSL_DSI_OpenCmVc "
			"Failed\n", __func__);
		goto err_dsi_open_cm;
	}

	if (!pPanel->dsi_cfg->dispEngine && pPanel->dsi_cfg->pixTxporter) {
		OTM1281A_LOG(LCD_DBG_ERR_ID, "Cannot transfer pixels via MMDMA"
...
/*
 *
 *  Function Name: OTM1281A_Close
 *
 *  Description:   disp bus OFF
 *
 */
Int32 OTM1281A_Close(DISPDRV_HANDLE_T drvH)
{
	Int32 res = 0;
	OTM1281A_PANEL_t	*pPanel	= (OTM1281A_PANEL_t *)drvH;
	...
	if (CSL_DSI_CloseClient(pPanel->clientH) != CSL_LCD_OK)	{
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	ERROR, Closing "
			"DSI Client\n",	__func__);
		return -1;
	}

	if (CSL_DSI_Close(pPanel->busNo) != CSL_LCD_OK)	{
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s: ERR Closing DSI "
			"Controller\n",	__func__);
		return -1;
	}
	...
	if (brcm_disable_dsi_pll_clocks(pPanel->busNo))	{
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s: ERROR to disable	"
			"the pll clock\n", __func__);
		return -1;
	}
...
/*
 *
 * Function Name:  OTM1281A_WrSendCmnd
 *
 ...
 */
static int OTM1281A_WrSendCmnd(
	DISPDRV_HANDLE_T drvH,
	UInt8			*msg_buff,
	UInt32 msg_size)
{
	...
	if (res	== 0) {
		...
		OTM1281A_LOG(LCD_DBG_INIT_ID, "[DISPDRV] %s:	DT[0x%02lX] "
...
/*
 *
 *   Function Name:   OTM1281A_ExecCmndList
 *
 *   Description:
 *
 */
	...
	pDISPCTRL_REC_T cmnd_lst)
{
	...
	while (cmnd_lst[i].type	!= DISPCTRL_LIST_END) {
		if (cmnd_lst[i].type ==	DISPCTRL_WR_CMND) {
			...
			OTM1281A_WrSendCmnd(drvH, tx_buff, tx_size);
			tx_size	= 0;
...
/*
 *
 *   Function Name: OTM1281A_PowerControl
 *
 *   Description:	  Display Module Control
 *
 */
Int32 OTM1281A_PowerControl(
	DISPDRV_HANDLE_T drvH,
	DISPLAY_POWER_STATE_T state)
{
	Int32  res = 0;
	...
	case CTRL_PWR_ON:
		...
		case STATE_PWR_OFF:
			...
			OTM1281A_panel_init(pPanel);
			OSTASK_Sleep(TICKS_IN_MILLISECONDS(120));

			OTM1281A_WinSet(drvH, TRUE, &pPanel->win_dim);

			pPanel->pwrState = STATE_SCREEN_OFF;
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV]	%s: INIT-SEQ\n",
				__func__);
			break;
		default:
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	POWER ON "
	...
	case CTRL_PWR_OFF:
		if (pPanel->pwrState !=	STATE_PWR_OFF) {
			OTM1281A_panel_off(pPanel);
			OTM1281A_panel_sleep_in(pPanel);
			OSTASK_Sleep(TICKS_IN_MILLISECONDS(20));
			OTM1281A_reset(drvH, TRUE);

			memset(&pPanel->win_cur, 0, sizeof(DISPDRV_WIN_t));
			pPanel->pwrState = STATE_PWR_OFF;
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV]	%s: PWR DOWN\n\r",
				__func__);
		}
		break;
	...
	case CTRL_SLEEP_IN:
		...
		case STATE_SCREEN_OFF:
			...
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV]	%s: SLEEP\n\r",
			    __func__);
			break;
		default:
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	"
	...
	case CTRL_SLEEP_OUT:
		switch (pPanel->pwrState) {
		case STATE_SLEEP:
			OTM1281A_panel_sleep_out(pPanel);
			OSTASK_Sleep(TICKS_IN_MILLISECONDS(120));

			pPanel->pwrState = STATE_SCREEN_OFF;
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV]	%s: SLEEP-OUT\n\r",
				__func__);
			break;
		default:
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	"
	...
	case CTRL_SCREEN_ON:
		switch (pPanel->pwrState) {
		case STATE_SCREEN_OFF:
			OTM1281A_panel_on(pPanel);

			pPanel->pwrState = STATE_SCREEN_ON;
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV]	%s: SCREEN ON\n",
				__func__);
			break;
		default:
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	"
	...
	case CTRL_SCREEN_OFF:
		...
		case STATE_SCREEN_ON:
			...
			pPanel->pwrState = STATE_SCREEN_OFF;
			OTM1281A_LOG(LCD_DBG_ERR_ID, "[DISPDRV]	%s: SCREEN OFF\n",
				__func__);
			break;
		default:
			OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	"
		...
		break;
	default:
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s: Invalid Power "
			"State[%d] Requested\n", __func__, state);
		res = -1;
		break;
	}
...
/*
 *
 *  Function Name: OTM1281A_Start
 *
 *  Description:
 *
 */
Int32 OTM1281A_Start(
	DISPDRV_HANDLE_T drvH,
	struct pi_mgr_dfs_node *dfs_node)
{
	OTM1281A_PANEL_t *pPanel = (OTM1281A_PANEL_t *)drvH;
	...
	if (brcm_enable_dsi_lcd_clocks(dfs_node, pPanel->busNo,
		OTM1281A_dsiCfg.hsBitClk.clkIn_MHz * 1000000,
		OTM1281A_dsiCfg.hsBitClk.clkInDiv,
		OTM1281A_dsiCfg.escClk.clkIn_MHz * 1000000
		/ OTM1281A_dsiCfg.escClk.clkInDiv)) {

		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s: ERROR to enable "
			"the clock\n", __func__);
		return -1;
	}
...
/*
 *
 *   Function Name: OTM1281A_Stop
 *
 *   Description:
 *
 */
Int32 OTM1281A_Stop(DISPDRV_HANDLE_T drvH, struct pi_mgr_dfs_node *dfs_node)
{
	OTM1281A_PANEL_t *pPanel = (OTM1281A_PANEL_t *)drvH;
...
/*
 *
 *   Function Name: OTM1281A_GetInfo
 *
 *   Description:
 *
 */
const DISPDRV_INFO_T *OTM1281A_GetDispDrvData(DISPDRV_HANDLE_T drvH)
{
	return &OTM1281A_Info;
}

/*
 *
 *  Function Name: OTM1281A_GetDispDrvFeatures
 *
 *  Description:
 *
 */
Int32 OTM1281A_GetDispDrvFeatures(
	DISPDRV_HANDLE_T drvH,
	const char			**driver_name,
	UInt32				*version_major,
	UInt32				*version_minor,
...
/*
 *
 *  Function Name: OTM1281A_Cb
 *
 *  Description:   CSL callback
 *
 */
static void OTM1281A_Cb(CSL_LCD_RES_T cslRes, pCSL_LCD_CB_REC pCbRec)
{
	DISPDRV_CB_RES_T apiRes;

	OTM1281A_LOG(LCD_DBG_ID, "[DISPDRV]	+%s\r\n", __func__);
...
/*
 *
 *  Function Name: OTM1281A_Update
 *
 *  Description:
 *
 */
Int32 OTM1281A_Update(
	DISPDRV_HANDLE_T drvH,
	void *buff,
	DISPDRV_WIN_t *p_win,
	DISPDRV_CB_T apiCb)
{
	...
	OTM1281A_LOG(LCD_DBG_ID, "[DISPDRV] +%s\r\n", __func__);
	if (pPanel->pwrState ==	STATE_PWR_OFF) {
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s: Skip Due To "
			"Power State\n", __func__);
		return -1;
	}
	...
	OTM1281A_LOG(LCD_DBG_ID, "%s %d %d %d %d\n", __func__, p_win->l, p_win->r, p_win->t,
		p_win->b);

	OTM1281A_WinSet(drvH, TRUE, p_win);
	...
	OTM1281A_LOG(LCD_DBG_ID, "%s: buf=%08x, linelenp = %lu, linecnt =%lu\n",
	...
	if (CSL_DSI_UpdateCmVc(pPanel->dsiCmVcHandle, &req, pPanel->isTE)
		...
		OTM1281A_LOG(LCD_DBG_ERR_ID,	"[DISPDRV] %s:	ERROR ret by "
			"CSL_DSI_UpdateCmVc\n\r", __func__);
		res = -1;
		OTM1281A_Cb(res, &req.cslLcdCbRec);
	}
...
/*
 *
 *  Function Name: OTM1281A_Atomic_Update
 *
 *  Description:
 *
 */
Int32 OTM1281A_Atomic_Update(
	DISPDRV_HANDLE_T drvH,
	void			*buff,
	DISPDRV_WIN_t		*p_win)
{
