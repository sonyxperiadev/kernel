...
//   { 0,  15, 10, 15 , },    //  CSL_SMI_TIMIMG_T  wrTiming_m;     //  1-4-3-4 => BB 36ns
 // Original   { 0, 8, 4, 8, },
	{ 0, 30, 10, 30 , }, //@HW, this value needs to be tuned
...
//
//*****************************************************************************
DISPDRV_T* DISPDRV_ili9341_GetFuncTable ( void )
{
    return ( &DISP_SMI_Drv );
}
...
//
//*****************************************************************************
#if defined(CONFIG_MACH_RHEA_SS_ZANIN) || defined(CONFIG_MACH_RHEA_SS_ZANIN_04) || defined(CONFIG_MACH_RHEA_SS_IVORY)
#define TE_VC4L_IN_PIN_SELECT TE_VC4L_IN_0_LCD
#else
#define TE_VC4L_IN_PIN_SELECT TE_VC4L_IN_1_DSI0
#endif
...
switch (state) {
	case CTRL_PWR_ON:
		...
		case STATE_PWR_OFF:
			...
			DispSmi_ExecCmndList ( dispH, &ili9341_Init[0]);
		...
		switch (pPanel->pwrState) {
                case STATE_SLEEP:
			DispSmi_ExecCmndList ( dispH, &ili9341_Sleep[0]);
			OSTASK_Sleep( TICKS_IN_MILLISECONDS(120) );
