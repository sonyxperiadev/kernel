...
/* LOCAL FUNCTIONs */
static void bcm91008_alex_WrCmndP0(DISPDRV_HANDLE_T drvH, UInt32 reg);

static void bcm91008_alex_WrCmndP1(DISPDRV_HANDLE_T drvH,
				   UInt32 reg, UInt32 val);

/* DRV INTERFACE FUNCTIONs */
static Int32 BCM91008_ALEX_Init(struct dispdrv_init_parms *parms,
...
static Int32 BCM91008_ALEX_GetDispDrvFeatures(DISPDRV_HANDLE_T drvH,
...
static const DISPDRV_INFO_T *BCM91008_ALEX_GetDispDrvData(DISPDRV_HANDLE_T
							  drvH);

static Int32 BCM91008_ALEX_Start(DISPDRV_HANDLE_T drvH,
				 struct pi_mgr_dfs_node *dfs_node);

static Int32 BCM91008_ALEX_Stop(DISPDRV_HANDLE_T drvH,
				struct pi_mgr_dfs_node *dfs_node);

static Int32 BCM91008_ALEX_PowerControl(DISPDRV_HANDLE_T drvH,
					DISPLAY_POWER_STATE_T state);

static Int32 BCM91008_ALEX_Update(DISPDRV_HANDLE_T drvH,
				  void *buff,
...
#define   RDID3	(0xDC)
	...
	/* 0xFE - OLED modules manufacturer */
	txData[0] = RDID1;
 ...
 *
 */
Int32 bcm91008_alex_WinSet(DISPDRV_HANDLE_T dispH,
...
static Int32 BCM91008_ALEX_Open(DISPDRV_HANDLE_T drvH)
{
	...
	if (brcm_enable_dsi_pll_clocks(pPanel->busNo,
				       ...
				       dsiCfg.escClk.clkIn_MHz * 1000000 /
...
#endif
			/* MADCTL - Invert, disp is mounted so RB is at
			   UL position */
			if (pPanel->cmnd_mode->cm_in == LCD_IF_CM_I_RGB888U)
				bcm91008_alex_WrCmndP1(drvH,
