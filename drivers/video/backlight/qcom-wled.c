...
static int wled_ovp_fault_status(struct wled *wled, bool *fault_set)
{
	...
	*fault_set = false;
	...
	if ((wled->version == WLED_VERSION_4_0 || wled->version == WLED_VERSION_4_1) &&
                (fault_sts & WLED3_CTRL_REG_OVP_FAULT_BIT))
		*fault_set = true;

	if (wled->version == WLED_VERSION_5 && (fault_sts & (WLED3_CTRL_REG_OVP_FAULT_BIT |
...
static int wled_configure(struct wled *wled)
{
	...
	case WLED_VERSION_4_1:
		if (wled->version == WLED_VERSION_4_0) {
			u32_opts = wled4_0_opts;
			size = ARRAY_SIZE(wled4_0_opts);
		} else if (wled->version == WLED_VERSION_4_1) {
...
static const struct of_device_id wled_match_table[] = {
	...
	{ .compatible = "qcom,pm8150l-wled", .data = (void *)WLED_VERSION_5 },
	{}
};
