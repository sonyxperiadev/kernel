...
static struct poll_ctrl *polling;
...
/* WARNING: This function assumes that the display is ON!! */
static void poll_worker_schedule(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (likely(pinfo->dsi_master == pinfo->pdest) &&
	    polling->enable) {
...
static void poll_worker_cancel(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_info *pinfo = NULL;
			...
			bool cont_splash_on, bool is_dsi_master)
{
	...
	polling->schedule = poll_worker_schedule;
	polling->cancel = poll_worker_cancel;

	if (polling->enable && cont_splash_on && is_dsi_master) {
		pr_info("%s: Continuous splash polling started.\n",
			__func__);
		rc = polling->intervals;
		polling->intervals = FIRST_POLL_REG_INTERVAL;
		polling->schedule(ctrl_pdata);
