...
#define SENSOR_ID 2
#define SENSOR_JPEG_SNAPSHOT_MEMSIZE	0x4b0000
/* 4915200 = 2560*1920     //0x33F000
//3403776 //2216 * 1536*/
...
#define S5K4ECGX_DEFAULT_PIX_FMT	V4L2_PIX_FMT_UYVY
/* YUV422 */
...
static bool camdrv_ss_s5k4ecgx_check_flash_needed(
	struct v4l2_subdev *sd);
static int camdrv_ss_s5k4ecgx_AAT_flash_control(
	struct v4l2_subdev *sd, int control_mode);
static int camdrv_ss_s5k4ecgx_MIC2871_flash_control(
	struct v4l2_subdev *sd, int control_mode);
static  camdrv_ss_s5k4ecgx_set_ae_stable_status(
	struct v4l2_subdev *sd, unsigned short value);/*Nikhil*/
static int camdrv_ss_s5k4ecgx_get_ae_stable_status_value(
	struct v4l2_subdev *sd);/*Nikihl*/
int camdrv_ss_s5k4ecgx_set_preflash_sequence(
	struct v4l2_subdev *sd, struct v4l2_control *ctrl);
#define S5K4ECGX_DELAY_DURATION 0xFFFF
static bool first_af_status;
...
*/
...

	{
		...
		.name = "ISO",
		.minimum = ISO_AUTO ,
		.maximum = (1 << ISO_AUTO | 1 << ISO_50
			| 1 << ISO_100 | 1 << ISO_200
			| 1 << ISO_400 | 1 << ISO_800
			| 1 << ISO_1200 | 1 << ISO_1600
			| 1 << ISO_2400 | 1 << ISO_3200
			| 1 << ISO_SPORTS | 1 << ISO_NIGHT
			| 1 << ISO_MOVIE),
		.step = 1,
...
*/
...

	{
		...
		.maximum = (1 << ANTI_BANDING_AUTO | 1 << ANTI_BANDING_50HZ
			| 1 << ANTI_BANDING_60HZ | 1 << ANTI_BANDING_OFF),
		.step = 1,
	...
	struct v4l2_subdev *sd, struct v4l2_frmivalenum *fival)
{
	...
	if (fival->index >= 1) {
		CAM_ERROR_PRINTK(
			"%s: returning ERROR because index = %d !!!\n",
	...
	for (i = 0;
	i < ARRAY_SIZE(s5k4ecgx_supported_preview_framesize_list);
	i++) {
		if ((s5k4ecgx_supported_preview_framesize_list[i].width
			== fival->width) &&
			(s5k4ecgx_supported_preview_framesize_list[i].height
			== fival->height)) {
			size =
				s5k4ecgx_supported_preview_framesize_list[i]
				.index;
			break;
		}
	}
...
int camdrv_ss_s5k4ecgx_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if (state->mode_switch == PICTURE_CAPTURE_TO_CAMERA_PREVIEW_RETURN) {
		...
		err = camdrv_ss_i2c_set_config_register(client,
			...
			/*err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_awb_unlock_regs,
				ARRAY_SIZE(s5k4ecgx_awb_unlock_regs),
				"awb_unlock_regs");*/

		if (err < 0) {
			CAM_ERROR_PRINTK(
				"%s :s5k4ecgx_ae_unlock_regs IS FAILED\n",
				__func__);
			return -EIO;
		}
		...
		if (err < 0) {
			CAM_ERROR_PRINTK(
				"%s :s5k4ecgx_preview_camera_regs IS FAILED\n",
				__func__);
			return -EIO;
		}
	...
	if (state->mode_switch == CAMERA_PREVIEW_TO_CAMCORDER_PREVIEW) {
		/* CSP614861: temporary added
		fixed FPS for 720p performance test */
		if (
			state->mode_switch
			...
			if (s5k4ecgx_fps_30_regs != NULL)
				...
				CAM_ERROR_PRINTK(
					"%s : Fixed FPS setting is not supported for 30,25,20,15 fps !!\n",
					__func__);

			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : Fixed FPS setting is FAILED !!\n",
					__func__);
				return -EIO;
			}
		}
		...
		if (state->currentScene != SCENE_MODE_NONE) {
			...
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s :s5k4ecgx_scene_none_regs IS FAILED\n",
					__func__);
				return -EIO;
			}
		}
...
static int camdrv_ss_s5k4ecgx_set_capture_start(
	struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camdrv_ss_state *state = to_state(sd);
	...
	if (err < 0) {
		CAM_ERROR_PRINTK(
			"%s: camdrv_ss_set_capture_size not supported !!!\n",
			__func__);
		return -EIO;
	}
...
#if 1
int camdrv_ss_s5k4ecgx_set_iso(
	struct v4l2_subdev *sd, int mode)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err = 0;
	...
	CAM_INFO_PRINTK(
		"%s :  value = %d wb_auto = %d, iso_auto = %d\n",
	...
	case ISO_AUTO:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_wb_iso_auto_regs,
				ARRAY_SIZE(s5k4ecgx_wb_iso_auto_regs),
				"wb_iso_auto_regs");
		} else if (wb_auto == 0 && iso_auto == 1) {
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_wb_manual_iso_auto_regs,
				ARRAY_SIZE(s5k4ecgx_wb_manual_iso_auto_regs),
				"wb_manual_iso_auto_regs");
		} else if (wb_auto == 1 && iso_auto == 0) {
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_wb_auto_iso_manual_regs,
				ARRAY_SIZE(s5k4ecgx_wb_auto_iso_manual_regs),
				"wb_auto_iso_manual_regs");
		} else {
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_wb_manual_iso_manual_regs,
				ARRAY_SIZE(s5k4ecgx_wb_manual_iso_manual_regs),
				"wb_manual_iso_manual_regs");
		}
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_iso_auto_regs,
				ARRAY_SIZE(s5k4ecgx_iso_auto_regs),
				"iso_auto_regs");
	...
	case ISO_50:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_iso_50_regs,
				ARRAY_SIZE(s5k4ecgx_iso_50_regs),
				"iso_50_regs");
	...
	case ISO_100:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_iso_100_regs,
				ARRAY_SIZE(s5k4ecgx_iso_100_regs),
				"iso_100_regs");
	...
	case ISO_200:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_iso_200_regs,
				ARRAY_SIZE(s5k4ecgx_iso_200_regs),
				"iso_200_regs");
	...
	case ISO_400:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_iso_400_regs,
				ARRAY_SIZE(s5k4ecgx_iso_400_regs),
				"iso_400_regs");
...
int camdrv_ss_s5k4ecgx_set_white_balance(
	struct v4l2_subdev *sd, int mode)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camdrv_ss_state *state = to_state(sd);
	...
	case WHITE_BALANCE_DAYLIGHT:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_wb_daylight_regs,
				ARRAY_SIZE(s5k4ecgx_wb_daylight_regs),
				"wb_daylight_regs");
	...
	case WHITE_BALANCE_INCANDESCENT:
	{
		...
		if (wb_auto == 1 && iso_auto == 1) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_wb_incandescent_regs,
				ARRAY_SIZE(s5k4ecgx_wb_incandescent_regs),
				"wb_incandescent_regs");
...
#endif
/*denis*/
static int camdrv_ss_s5k4ecgx_set_scene_mode(
	struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camdrv_ss_state *state = to_state(sd);
...
*/
	...
	{
		/* Metering-Center, EV0, WB-Auto, Sharp-1,
		Sat0, AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Spot, EV0, WB-Auto, Sharp0, Sat0,
		AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Matrix, EV0, WB-Auto, Sharp+1,
		Sat+1, AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Center, EV+1, WB-Auto, Sharp0,
		Sat+1, AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Center, EV0, WB-daylight, Sharp0,
		Sat0, AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Center, EV0, WB-fluorescent, Sharp0,
		Sat0, AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Center, EV0, WB-Auto, Sharp0, Sat+2,
		AF-Auto will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	{
		/* Metering-Center, EV0, WB-Auto, Sharp+2,
		Sat0, AF-Macro will be set in HAL layer */
		err = camdrv_ss_i2c_set_config_register(client,
	...
	regs_t reg_buffer[], int num_of_regs, char *name)
{
	...
	for (i = 0; i < num_of_regs; i++) {
		...
		case START_BURST_MODE:
			{
				...
				/* If next subaddr is different
				from the current subaddr
				In other words, if burst mode ends,
				write the all of
				the burst datas which were gathered until now*/
		...
		default:
			{
				err = camdrv_ss_i2c_write_4_bytes(client,
					subaddr, data_value);
				if (err < 0) {
					CAM_ERROR_PRINTK(
...
{
	CAM_INFO_PRINTK(
		"%s %d :camdrv_ss_s5k4ecgx_AAT_flash_write_data  E\n",
		__func__, count);
	unsigned long flags;
...
static float camdrv_ss_s5k4ecgx_get_exposureTime(
	struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int camdrv_ss_s5k4ecgx_get_iso_speed_rate(struct v4l2_subdev *sd)
{
	...
	CAM_INFO_PRINTK(
		"camdrv_ss_s5k4ecgx_get_iso_speed_rate read_value1 = 0x%x, read_value2 = 0x%x, iso_auto = %d\n",
		read_value1, read_value2 , iso_auto);
	if (iso_auto) {
		GainValue = ((read_value1 * read_value2) / 256)/2 ;
		CAM_INFO_PRINTK(
			"camdrv_ss_s5k4ecgx_get_iso_speed_rate ISO Auto GainValue = 0x%x\n",
			GainValue);
		...
		CAM_INFO_PRINTK(
			"camdrv_ss_s5k4ecgx_get_iso_speed_rate ISO Manual GainValue = 0x%x\n",
			GainValue);
	...
	/*denis : set iso value for the specific scene mode*/
	if (state->currentScene == SCENE_MODE_FIREWORKS)
		isospeedrating = 50;
...
/*NIKHIL*/
static  camdrv_ss_s5k4ecgx_set_ae_stable_status(
	struct v4l2_subdev *sd, unsigned short value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int err = 0;
	CAM_INFO_PRINTK(
	...
	struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
...
#endif
		CAM_ERROR_PRINTK(
			"%s: I2C failed during s5k4ecgx_get_1st_af_search_status\n",
			__func__);
		ctrl->value = CAMERA_AF_STATUS_FAILED;
		return -EFAULT;
	}
	err = camdrv_ss_i2c_read_2_bytes(client,
		0x0F12, &usReadData);
	if (err < 0) {
#ifdef CONFIG_FLASH_MIC2871
		camdrv_ss_s5k4ecgx_MIC2871_flash_control(sd,
			FLASH_CONTROL_OFF);
#else
		camdrv_ss_s5k4ecgx_AAT_flash_control(sd,
			FLASH_CONTROL_OFF);
#endif
		CAM_ERROR_PRINTK(
	...
	default:
		...
		if (state->camera_flash_fire == 1) {
			...
			#endif
		/*state->camera_flash_fire = 0 ;*/
		/*CAM_INFO_PRINTK("%d, camera af flash check",
			state->camera_flash_fire);*/
		}
		/*CAM_INFO_PRINTK("%s , ae unlock\n",__func__);
		//to maintain AE lock in case of af fail*/
		/* err = camdrv_ss_i2c_set_config_register(client,
			s5k4ecgx_ae_unlock_regs,
			ARRAY_SIZE(s5k4ecgx_ae_unlock_regs),
			"ae_unlock_regs");*/
	...
	if (first_af_status) {
		...
		if (err < 0) {
			...
			CAM_ERROR_PRINTK(
				"%s: I2C failed during s5k4ecgx_get_2nd_af_search_status\n",
				__func__);
			ctrl->value = CAMERA_AF_STATUS_FAILED;
		...
		if (!(usReadData & 0xff00)) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				...
				"ae_unlock_regs");
			if (state->camera_flash_fire == 1) {
				camdrv_ss_s5k4ecgx_set_ae_stable_status(sd,
					0x0002);
				if (s5k4ecgx_Pre_Flash_End_EVT1 == NULL)
					CAM_ERROR_PRINTK(
						"%s : pre flash start not supported!!!\n",
						__func__);
				...
				if (err < 0)
					CAM_ERROR_PRINTK(
						"[%s : %d] ERROR! Couldn't Set Pre Flash start\n",
					__FILE__, __LINE__);
	...
	struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	CAM_INFO_PRINTK(
		"camdrv_ss_s5k4ecgx_get_touch_focus_status E\n");
	if (!first_af_status) {
		err = camdrv_ss_i2c_set_config_register(client,
			s5k4ecgx_get_1st_af_search_status,
			ARRAY_SIZE(s5k4ecgx_get_1st_af_search_status),
	...
	struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (ctrl->value == AUTO_FOCUS_ON) {
		CAM_INFO_PRINTK(
			"camdrv_ss_s5k4ecgx_set_auto_focus E  %d\n",
...
/*  if (state->camera_af_flash_checked == 0) {
		...
		if (state->current_flash_mode == FLASH_MODE_ON) {
			...
			if (camdrv_ss_s5k4ecgx_check_flash_needed(sd))
				bflash_needed
				= camdrv_ss_s5k4ecgx_check_flash_needed(sd);
			else
				CAM_ERROR_PRINTK(
					"%s : check_flash_needed is NULL !!!\n",
					__func__);
			if (bflash_needed)
				state->camera_flash_fire = 1;
		}
...
/*Nikhil*/
		...
		err = camdrv_ss_i2c_set_config_register(client,
			s5k4ecgx_ae_lock_regs,
			ARRAY_SIZE(s5k4ecgx_ae_lock_regs),
			"ae_lock_regs");
		/*CAM_INFO_PRINTK("%s , awb lock\n",__func__);*/
		/*err = camdrv_ss_i2c_set_config_register(client,
			s5k4ecgx_awb_lock_regs,
			ARRAY_SIZE(s5k4ecgx_awb_lock_regs),
			"awb_lock_regs");*/
		...
		*/
		if (state->af_mode == FOCUS_MODE_AUTO) {
			/*err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_auto_regs,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_auto_regs),
				"focus_mode_auto_regs");*/
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_auto_regs_cancel3,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_auto_regs_cancel3),
				"focus_mode_auto_regs_cancel3");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
			...
			/*err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_macro_regs,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_macro_regs),
				"focus_mode_macro_regs");*/
				err = camdrv_ss_i2c_set_config_register(client,
					s5k4ecgx_focus_mode_macro_regs_cancel3,
					ARRAY_SIZE(
					s5k4ecgx_focus_mode_macro_regs_cancel3),
					"focus_mode_macro_regs_cancel3");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
	...
	/*	msleep(50);
	// need to add more delay in case of
	night or fireworks mode : 300ms*/
	} else if (ctrl->value == AUTO_FOCUS_1ST_CANCEL) {
		if (state->af_mode == FOCUS_MODE_AUTO) {
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_auto_regs_cancel1,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_auto_regs_cancel1),
				"focus_mode_auto_regs_cancel1");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_macro_regs_cancel1,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_macro_regs_cancel1),
				"focus_mode_macro_regs_cancel1");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
		...
		if (state->af_mode == FOCUS_MODE_AUTO) {
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_auto_regs_cancel2,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_auto_regs_cancel2),
				"focus_mode_auto_regs_cancel2");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
			...
			err = camdrv_ss_i2c_set_config_register(client,
				s5k4ecgx_focus_mode_macro_regs_cancel2,
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_macro_regs_cancel2),
				"focus_mode_macro_regs_cancel2");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
...
static int camdrv_ss_s5k4ecgx_set_touch_focus_area(
	struct v4l2_subdev *sd, enum v4l2_touch_af touch_af,
	v4l2_touch_area *touch_area)
{
	...
	unsigned short FirstWinEndX, FirstWinEndY,
		ConvFirstWinStartX, ConvFirstWinStartY;
	unsigned short SecondWinEndX, SecondWinEndY,
		ConvSecondWinStartX, ConvSecondWinStartY;
	unsigned short DefaultFirstWinStartX,
		DefaultFirstWinStartY, DefaultSecondWinStartX,
		DefaultSecondWinStartY;
	...
	DefaultFirstWinStartX
		= preview_width-((preview_width-AF_OUTER_WINDOW_WIDTH)/2);
	/* 640x480 = > 160, 800x480 = > 240 */
	DefaultFirstWinStartY
		= preview_height-((preview_height-AF_OUTER_WINDOW_HEIGHT)/2);
	/* 640x480 = > 107, 800x480 = > 107 */
	DefaultSecondWinStartX
		= preview_width-((preview_width-AF_INNER_WINDOW_WIDTH)/2);
	/* 640x480 = > 248, 800x480 = > 160 */
	DefaultSecondWinStartY
		= preview_height-((preview_height-AF_INNER_WINDOW_HEIGHT)/2);
	/* 640x480 = > 328, 800x480 = > 160 */

	CAM_INFO_PRINTK(
		"[%s:%d] preview_width %d, preview_height %d\n",
		__func__, __LINE__, preview_width, preview_height);
	CAM_INFO_PRINTK(
		"[%s:%d] DefaultFirstWinStartX %d, DefaultFirstWinStartY %d\n",
		__func__, __LINE__,
		DefaultFirstWinStartX, DefaultFirstWinStartY);
	CAM_INFO_PRINTK(
		"[%s:%d] DefaultSecondWinStartX %d, DefaultSecondWinStartY %d\n",
		__func__, __LINE__,
		DefaultSecondWinStartX, DefaultSecondWinStartY);
	...
	if (touch_af == TOUCH_AF_STOP)	{ /* AF stop */
		...
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(unsigned short)
			((DefaultFirstWinStartX << 10)/preview_width));
			/* FstWinStartX */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(unsigned short)
			((DefaultFirstWinStartY << 10)/preview_height));
			/* FstWinStartY */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(AF_OUTER_WINDOW_WIDTH << 10) / preview_width);
			/* FstWinSizeX : 320 */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(AF_OUTER_WINDOW_HEIGHT << 10) / preview_height);
			/* FstWinSizeY : 266 */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(unsigned short)
			((DefaultSecondWinStartX << 10)/preview_width));
			/* ScndWinStartX */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(unsigned short)
			((DefaultSecondWinStartY << 10)/preview_height));
			/* ScndWinStartY */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(AF_INNER_WINDOW_WIDTH << 10) / preview_width);
			/* ScndWinSizeX : 143 */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(AF_INNER_WINDOW_HEIGHT << 10) / preview_height);
			/* ScndWinSizeY : 143  */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12, 0x0001);
			/* WinSizesUpdated */
	...
	/*Prevent divided-by-zero.*/
		...
		SecondWinStartX
			= touch_area->leftTopX - ((
			AF_INNER_WINDOW_WIDTH - touch_area->rightBottomX)/2);
		if (SecondWinStartX < 0)
			SecondWinStartX = 0;
		if (SecondWinStartX+AF_INNER_WINDOW_WIDTH > preview_width)
			SecondWinStartX =
				preview_width - AF_INNER_WINDOW_WIDTH-1;

		SecondWinStartY
			= touch_area->leftTopY - ((
			AF_INNER_WINDOW_HEIGHT - touch_area->rightBottomY)/2);
		...
		FirstWinStartX
			= touch_area->leftTopX - ((
			AF_OUTER_WINDOW_WIDTH - touch_area->rightBottomX)/2);
		if (FirstWinStartX < 0)
			FirstWinStartX = 0;
		if (FirstWinStartX+AF_OUTER_WINDOW_WIDTH > preview_width)
			FirstWinStartX =
				preview_width - AF_OUTER_WINDOW_WIDTH-1;

		FirstWinStartY
			= touch_area->leftTopY - ((
			AF_OUTER_WINDOW_HEIGHT - touch_area->rightBottomY)/2);
		if (FirstWinStartY < 0)
			FirstWinStartY = 0;
		if (FirstWinStartY+AF_OUTER_WINDOW_HEIGHT > preview_height)
			FirstWinStartY
				= preview_height - AF_OUTER_WINDOW_HEIGHT-1;


		CAM_INFO_PRINTK(
			"[%s:%d]leftTopX %d, leftTopY %d, rightBottomX %d, rightBottomY %d\n",
			__func__, __LINE__,
			touch_area->leftTopX,
			touch_area->leftTopY,
			touch_area->rightBottomX,
			touch_area->rightBottomY);
		CAM_INFO_PRINTK(
			"[%s:%d]SecondWinStartX %d, SecondWinStartY %d\n",
			__func__, __LINE__,
			SecondWinStartX, SecondWinStartY);
		CAM_INFO_PRINTK(
			"[%s:%d]FirstWinStartX %d, FirstWinStartY %d\n",
			__func__, __LINE__,
		...
		ConvFirstWinStartX
			= (unsigned short)
			((FirstWinStartX << 10)/preview_width);
		ConvFirstWinStartY
			= (unsigned short)
			((FirstWinStartY << 10)/preview_height);

		ConvSecondWinStartX
			= (unsigned short)
			((SecondWinStartX << 10)/preview_width);
		ConvSecondWinStartY
			= (unsigned short)
			((SecondWinStartY << 10)/preview_height);

		CAM_INFO_PRINTK(
			"[%s:%d]ConvSecondWinStartX %d, ConvSecondWinStartY %d\n",
			__func__, __LINE__,
			ConvSecondWinStartX, ConvSecondWinStartY);
		CAM_INFO_PRINTK(
			"[%s:%d]ConvFirstWinStartX %d, ConvFirstWinStartY %d\n",
			__func__, __LINE__,
			ConvFirstWinStartX, ConvFirstWinStartY);
		...
		camdrv_ss_i2c_write_4_bytes(client, 0x002A, 0x0294);
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			ConvFirstWinStartX); /* FstWinStartX */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			ConvFirstWinStartY); /* FstWinStartY */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(AF_OUTER_WINDOW_WIDTH << 10) / preview_width);
			/* FstWinSizeX : 320 */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			...
			/* FstWinSizeY : 266 */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			ConvSecondWinStartX); /* ScndWinStartX */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			ConvSecondWinStartY); /* ScndWinStartY */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
			(AF_INNER_WINDOW_WIDTH << 10) / preview_width);
			/* ScndWinSizeX : 72 */
		camdrv_ss_i2c_write_4_bytes(client, 0x0F12,
	...
	v4l2_touch_area *touch_area)
{
	...
	mutex_lock(&af_cancel_op);
	CAM_INFO_PRINTK(
		"camdrv_ss_s5k4ecgx_set_touch_focus E\n");
	...
	if (touch_af == TOUCH_AF_START) {
		...
		if (state->af_mode == FOCUS_MODE_AUTO) {
			...
			err = camdrv_ss_i2c_set_config_register(client,
				...
				ARRAY_SIZE(
				s5k4ecgx_focus_mode_macro_regs_cancel3),
				"focus_mode_macro_regs_cancel3s");
			if (err < 0) {
				CAM_ERROR_PRINTK(
					"%s : i2c failed in OFF !!!\n",
					__func__);
...
static int camdrv_ss_s5k4ecgx_get_light_condition(
	struct v4l2_subdev *sd, int *Result)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
/*	camdrv_ss_i2c_write_4_bytes(client, 0x002E, 0x29F0);
	...
	CAM_INFO_PRINTK(
		"%s [Read Value 1: %X]  [Read Value 2: %X]\n",
	...
	struct v4l2_subdev *sd, int control_mode)
{
	...
	case FLASH_CONTROL_MAX_LEVEL:
	{
		...
		gpio_set_value(GPIO_CAM_FLASH_SET, 1);
		camdrv_ss_MIC2871_flash_write_data(1, 0);
...
static int camdrv_ss_s5k4ecgx_sensor_power(int on)
{
	...
	if (IS_ERR_OR_NULL(axi_clk)) {
		CAM_ERROR_PRINTK(
			"%s:unable to get clock csi0_axi_clk\n", __func__);
		goto e_clk_get;
	}
	...
	value = ioread32(padctl_base + PADCTRLREG_DCLK1_OFFSET)
	...
	if (on) {
		...
		value = regulator_set_voltage(VCAM_A_2_8_V,
			VCAM_A_2_8V_REGULATOR_uV,
			VCAM_A_2_8V_REGULATOR_uV);
		if (value)
			CAM_ERROR_PRINTK(
				"%s:regulator_set_voltage VCAM_A_2_8_V failed\n",
				__func__);

		value = regulator_set_voltage(VCAM_IO_1_8_V,
			VCAM_IO_1_8V_REGULATOR_uV,
			VCAM_IO_1_8V_REGULATOR_uV);
		if (value)
			CAM_ERROR_PRINTK(
				"%s:regulator_set_voltage VCAM_IO_1_8_V failed\n",
				__func__);

		value = regulator_set_voltage(VCAM_CORE_1_2_V,
			VCAM_CORE_1_2V_REGULATOR_uV,
			VCAM_CORE_1_2V_REGULATOR_uV);
		if (value)
			CAM_ERROR_PRINTK(
				"%s:regulator_set_voltage VCAM_CORE_1_2_V failed\n",
				__func__);

		value = regulator_set_voltage(VCAM_AF_2_8V,
			VCAM_AF_2_8V_REGULATOR_uV,
			VCAM_AF_2_8V_REGULATOR_uV);
		if (value)
			CAM_ERROR_PRINTK(
				"%s:regulator_set_voltage VCAM_AF_2_8V failed\n",
				__func__);
		...
		if (value) {
			CAM_ERROR_PRINTK(
				"%s:failed to enable VCAM_A_2_8_V\n", __func__);
			return -1;
		}
		...
		if (value) {
			CAM_ERROR_PRINTK(
				"%s:failed to enable VCAM_IO_1_8_V\n",
				__func__);
			return -1;
		}
		...
		if (value) {
			CAM_ERROR_PRINTK(
				"%s:failed to enable VCAM_AF_2_8V\n",
				__func__);
			return -1;
		}
	...
	/*msleep(1);*/
		...
		if (value) {
			CAM_ERROR_PRINTK(
				"%s:failed to enable VCAM_CORE_1_2_V\n",
				__func__);
			return -1;
		} else
			CAM_ERROR_PRINTK(
				"%s: enabled VCAM_CORE_1_2V\n",
				__func__);
		msleep(1);
...
//#ifdef CONFIG_FLASH_ENABLE*/
	...
	CAM_INFO_PRINTK(
		...
		if (HWREV > = 0x03)
		{
			ret = gpio_request(GPIO_CAM_FLASH_SET_NEW,
				"GPJ0[5]");
			if (ret)
			{
				CAM_ERROR_PRINTK(
		...
		{
			ret = gpio_request(GPIO_CAM_FLASH_SET,
				"GPJ0[2]");
			if (ret) {
				CAM_ERROR_PRINTK(
		...
		ret = gpio_request(GPIO_CAM_FLASH_EN,
			"GPJ0[7]");
		if (ret) {
			CAM_ERROR_PRINTK(
...
bool camdrv_ss_s5k4ecgx_get_esd_status(struct v4l2_subdev *sd)
{
	...
	if (read_value != 0xAAAA) {
		...
		CAM_ERROR_PRINTK(
			"%s :: ESD detected. 1st read value is 0x%x\n",
			__func__, read_value);
	}

/*	CAM_INFO_PRINTK("%s first detection is ok.
readed value is 0x%x\n", __func__,read_value);*/
	...
	if (read_value != 0x0000) {
		bEsdStatus = true;		/*ESD detected*/
		CAM_ERROR_PRINTK(
			"%s :: ESD detected. 2nd read value is 0x%x\n",
			__func__, read_value);
	}

/*	CAM_INFO_PRINTK(
		"%s 2nd detection is ok. readed value is 0x%x\n",
		__func__,read_value);*/
	...
	camdrv_ss_s5k4ecgx_get_mode_change(struct v4l2_subdev *sd)
{
	...
	/* CAM_INFO_PRINTK("read mode change value <7000.215Fh>
		= 0x%X\n", read_value); */
...
struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	int n = 0;/*Nikhil*/
	CAM_INFO_PRINTK(
		"camdrv_ss_s5k4ecgx_set_preflash_sequence E\n");
	camdrv_ss_s5k4ecgx_set_ae_stable_status(sd, 0x0000);
...
/*optional*/
	sensor->get_nightmode
		= camdrv_ss_s5k4ecgx_get_nightmode; /*aska add*/
	sensor->set_preview_start
		= camdrv_ss_s5k4ecgx_set_preview_start;/*aska add*/
	sensor->set_capture_start
		= camdrv_ss_s5k4ecgx_set_capture_start;/*aska add*/

	sensor->set_iso
		= camdrv_ss_s5k4ecgx_set_iso;/*aska add*/
	sensor->set_white_balance
		= camdrv_ss_s5k4ecgx_set_white_balance;/*aska add*/
	...
	sensor->set_touch_focus_area
		= camdrv_ss_s5k4ecgx_set_touch_focus_area;
...
/*	sensor->get_light_condition
	...
	sensor->sensor_power
		= camdrv_ss_s5k4ecgx_sensor_power;

	sensor->get_exif_sensor_info
		= camdrv_ss_s5k4ecgx_get_sensor_param_for_exif;
	sensor->getEsdStatus
		= camdrv_ss_s5k4ecgx_get_esd_status;
	/* use 150ms delay or frame skip instead of register read */
	/* sensor->get_mode_change_reg
			= camdrv_ss_s5k4ecgx_get_mode_change; */

	sensor->set_scene_mode
		= camdrv_ss_s5k4ecgx_set_scene_mode;   /*denis*/
	sensor->get_prefalsh_on
		= camdrv_ss_s5k4ecgx_set_preflash_sequence; /*Nikhil*/
	...
	/*ISO*/
	sensor->iso_auto_regs
		= s5k4ecgx_iso_auto_regs;
	sensor->rows_num_iso_auto_regs
		= ARRAY_SIZE(s5k4ecgx_iso_auto_regs);

	sensor->iso_50_regs
		= s5k4ecgx_iso_50_regs;
	sensor->rows_num_iso_50_regs
		= ARRAY_SIZE(s5k4ecgx_iso_50_regs);

	sensor->iso_100_regs
		= s5k4ecgx_iso_100_regs;
	sensor->rows_num_iso_100_regs
		= ARRAY_SIZE(s5k4ecgx_iso_100_regs);

	sensor->iso_200_regs
		= s5k4ecgx_iso_200_regs;
	sensor->rows_num_iso_200_regs
		= ARRAY_SIZE(s5k4ecgx_iso_200_regs);

	sensor->iso_400_regs
		= s5k4ecgx_iso_400_regs;
	sensor->rows_num_iso_400_regs
		= ARRAY_SIZE(s5k4ecgx_iso_400_regs);

	/* WDR */
	sensor->wdr_on_regs
		= s5k4ecgx_wdr_on_regs;
	sensor->rows_num_wdr_on_regs
		= ARRAY_SIZE(s5k4ecgx_wdr_on_regs);

	sensor->wdr_off_regs
		= s5k4ecgx_wdr_off_regs;
	sensor->rows_num_wdr_off_regs
		= ARRAY_SIZE(s5k4ecgx_wdr_off_regs);

	/* CCD EV */
	sensor->ev_camcorder_minus_4_regs
		= s5k4ecgx_ev_camcorder_minus_4_regs;
	sensor->rows_num_ev_camcorder_minus_4_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_minus_4_regs);

	sensor->ev_camcorder_minus_3_regs
		= s5k4ecgx_ev_camcorder_minus_3_regs;
	sensor->rows_num_ev_camcorder_minus_3_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_minus_3_regs);

	sensor->ev_camcorder_minus_2_regs
		= s5k4ecgx_ev_camcorder_minus_2_regs;
	sensor->rows_num_ev_camcorder_minus_2_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_minus_2_regs);

	sensor->ev_camcorder_minus_1_regs
		= s5k4ecgx_ev_camcorder_minus_1_regs;
	sensor->rows_num_ev_camcorder_minus_1_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_minus_1_regs);

	sensor->ev_camcorder_default_regs
		= s5k4ecgx_ev_camcorder_default_regs;
	sensor->rows_num_ev_camcorder_default_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_default_regs);

	sensor->ev_camcorder_plus_1_regs
		= s5k4ecgx_ev_camcorder_plus_1_regs;
	sensor->rows_num_ev_camcorder_plus_1_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_plus_1_regs);

	sensor->ev_camcorder_plus_2_regs
		= s5k4ecgx_ev_camcorder_plus_2_regs;
	sensor->rows_num_ev_camcorder_plus_2_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_plus_2_regs);

	sensor->ev_camcorder_plus_3_regs
		= s5k4ecgx_ev_camcorder_plus_3_regs;
	sensor->rows_num_ev_camcorder_plus_3_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_plus_3_regs);

	sensor->ev_camcorder_plus_4_regs
		= s5k4ecgx_ev_camcorder_plus_4_regs;
	sensor->rows_num_ev_camcorder_plus_4_regs
		= ARRAY_SIZE(s5k4ecgx_ev_camcorder_plus_4_regs);


	/* auto contrast */
	sensor->auto_contrast_on_regs
		= s5k4ecgx_auto_contrast_on_regs;
	sensor->rows_num_auto_contrast_on_regs
		= ARRAY_SIZE(s5k4ecgx_auto_contrast_on_regs);

	sensor->auto_contrast_off_regs
		= s5k4ecgx_auto_contrast_off_regs;
	sensor->rows_num_auto_contrast_off_regs
		= ARRAY_SIZE(s5k4ecgx_auto_contrast_off_regs);


	/* af return & focus mode */
	sensor->af_return_inf_pos
		= s5k4ecgx_af_return_inf_pos;
	sensor->rows_num_af_return_inf_pos
		= ARRAY_SIZE(s5k4ecgx_af_return_inf_pos);

	sensor->af_return_macro_pos
		= s5k4ecgx_af_return_macro_pos;
	sensor->rows_num_af_return_macro_pos
		= ARRAY_SIZE(s5k4ecgx_af_return_macro_pos);

	sensor->focus_mode_auto_regs
		= s5k4ecgx_focus_mode_auto_regs;
	sensor->rows_num_focus_mode_auto_regs
		= ARRAY_SIZE(s5k4ecgx_focus_mode_auto_regs);

	sensor->focus_mode_macro_regs
		= s5k4ecgx_focus_mode_macro_regs;
	sensor->rows_num_focus_mode_macro_regs
		= ARRAY_SIZE(s5k4ecgx_focus_mode_macro_regs);
	sensor->focus_mode_infinity_regs
		= s5k4ecgx_focus_mode_auto_regs;
	sensor->rows_num_focus_mode_infinity_regs
		= ARRAY_SIZE(s5k4ecgx_focus_mode_auto_regs);

	sensor->vt_mode_regs
		= s5k4ecgx_vt_mode_regs;
	sensor->rows_num_vt_mode_regs
		= ARRAY_SIZE(s5k4ecgx_vt_mode_regs);
	sensor->main_flash_off_regs
		= s5k4ecgx_Main_Flash_End_EVT1;
	sensor->rows_num_main_flash_off_regs
		= ARRAY_SIZE(s5k4ecgx_Main_Flash_End_EVT1);
