...
static const struct camdrv_ss_framesize
	sr030pc50_supported_preview_framesize_list[] = {
	{ PREVIEW_SIZE_VGA,	640,  480 },
};

static const struct camdrv_ss_framesize
	sr030pc50_supported_capture_framesize_list[] = {
...
static const struct v4l2_queryctrl sr030pc50_controls[] = {
...

	{
		...
		.maximum = (1 << WHITE_BALANCE_AUTO
			| 1 << WHITE_BALANCE_DAYLIGHT
			/*WHITE_BALANCE_SUNNY*/
			| 1 << WHITE_BALANCE_CLOUDY
			| 1 << WHITE_BALANCE_INCANDESCENT
...

	{
		...
		.maximum = (1 << IMAGE_EFFECT_NONE
			| 1 << IMAGE_EFFECT_MONO/*IMAGE_EFFECT_BNW*/
			| 1 << IMAGE_EFFECT_SEPIA
			| 1 << IMAGE_EFFECT_NEGATIVE),
			/* this should be replace by querymenu */
		.step = 1,
...

	{
		...
		.maximum = (1 << METERING_MATRIX
			| 1 << METERING_CENTER | 1 << METERING_SPOT),
		.step = 1,
...
#endif
	{
		...
		.maximum = (1 << ANTI_BANDING_AUTO
			| 1 << ANTI_BANDING_50HZ | 1 << ANTI_BANDING_60HZ
			| 1 << ANTI_BANDING_OFF),
		.step = 1,
...
static int camdrv_ss_sr030pc50_enum_frameintervals(
	struct v4l2_subdev *sd, struct v4l2_frmivalenum *fival)
{
	...
	for (i = 0;
	i < ARRAY_SIZE(sr030pc50_supported_preview_framesize_list);
	i++) {
		if ((sr030pc50_supported_preview_framesize_list[i].width
			== fival->width) &&
		    (sr030pc50_supported_preview_framesize_list[i].height
			== fival->height)) {
			size =
				sr030pc50_supported_preview_framesize_list[i]
				.index;
			break;
		}
	}
...
static long camdrv_ss_sr030pc50_ss_ioctl(
...
int camdrv_ss_sr030pc50_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if (state->mode_switch == CAMERA_PREVIEW_TO_CAMCORDER_PREVIEW) {
		...
		err = camdrv_ss_i2c_set_config_register(client,
			sr030pc50_init_regs,
			ARRAY_SIZE(sr030pc50_init_regs), "init_regs");
		if (err < 0) {
			CAM_ERROR_PRINTK(
				"%s :sr030pc50_init_regs IS FAILED\n",
				__func__);
			return -EIO;
		}
	}
...
/*
	...
	if (err < 0) {
		CAM_ERROR_PRINTK(
		"%s : camdrv_ss_set_preview_size is FAILED !!\n", __func__);
		return -EIO;
	}
...
static int camdrv_ss_sr030pc50_get_iso_speed_rate(struct v4l2_subdev *sd)
{
	...
	CAM_INFO_PRINTK(
		"camdrv_ss_sr030pc50_get_iso_speed_rate, GainValue = %d, isospeedrating = %d\n",
...
static int camdrv_ss_sr030pc50_sensor_power(int on)
{
	...
	if (on) {
		...
		regulator_set_voltage(VCAM_A_2_8_V,
			VCAM_A_2_8V_REGULATOR_uV,
			VCAM_A_2_8V_REGULATOR_uV);
		regulator_set_voltage(VCAM_IO_1_8_V,
			VCAM_IO_1_8V_REGULATOR_uV,
			VCAM_IO_1_8V_REGULATOR_uV);
#ifdef VCAM0_IO_1_8V_REGULATOR_NEEDED
...
void camdrv_ss_sr030pc50_smartStayChangeInitSetting(
	struct camdrv_ss_sensor_cap *sensor)
{
	sensor->init_regs = sr030pc50_init_regs_smart_stay;
	sensor->rows_num_init_regs =
		ARRAY_SIZE(sr030pc50_init_regs_smart_stay);
...
bool camdrv_ss_sensor_init_sub(bool bOn, struct camdrv_ss_sensor_cap *sensor)
{
	...
	sensor->supported_number_of_preview_sizes =
		ARRAY_SIZE(sr030pc50_supported_preview_framesize_list);

	sensor->supported_capture_framesize_list =
		sr030pc50_supported_capture_framesize_list;
	sensor->supported_number_of_capture_sizes =
		ARRAY_SIZE(sr030pc50_supported_capture_framesize_list);

	sensor->preview_size_640x480_regs =
		sr030pc50_preview_size_640x480_regs;
	sensor->rows_num_preview_size_640x480_regs =
		ARRAY_SIZE(sr030pc50_preview_size_640x480_regs);

	sensor->fmts = sr030pc50_fmts;
	sensor->rows_num_fmts =
		ARRAY_SIZE(sr030pc50_fmts);
	...
	sensor->enum_frameintervals =
		camdrv_ss_sr030pc50_enum_frameintervals;

	sensor->set_preview_start =
		camdrv_ss_sr030pc50_set_preview_start;/*aska*/

	sensor->get_exif_sensor_info =
		camdrv_ss_sr030pc50_get_sensor_param_for_exif;
	sensor->getEsdStatus = camdrv_ss_sr030pc50_get_esd_status;


	/*REGS and their sizes*/
	/* List all the capabilities of sensor.
	List all the supported register setting tables */

	/*Normal CAM Preview*/
	sensor->init_regs = sr030pc50_init_regs;
	sensor->rows_num_init_regs = ARRAY_SIZE(sr030pc50_init_regs);

	sensor->smartStayChangeInitSetting =
		camdrv_ss_sr030pc50_smartStayChangeInitSetting;
	...
	sensor->rows_num_effect_normal_regs =
		ARRAY_SIZE(sr030pc50_effect_normal_regs);

	sensor->effect_mono_regs =
		sr030pc50_effect_mono_regs;
	sensor->rows_num_effect_mono_regs =
		ARRAY_SIZE(sr030pc50_effect_mono_regs);


	sensor->effect_negative_regs =
		sr030pc50_effect_negative_regs;
	sensor->rows_num_effect_negative_regs =
		ARRAY_SIZE(sr030pc50_effect_negative_regs);

	sensor->effect_sepia_regs = sr030pc50_effect_sepia_regs;
	sensor->rows_num_effect_sepia_regs =
		ARRAY_SIZE(sr030pc50_effect_sepia_regs);

	/*wb*/
	sensor->wb_auto_regs =
		sr030pc50_wb_auto_regs;
	sensor->rows_num_wb_auto_regs =
		ARRAY_SIZE(sr030pc50_wb_auto_regs);

	sensor->wb_daylight_regs =
		sr030pc50_wb_daylight_regs;
	sensor->rows_num_wb_daylight_regs =
		ARRAY_SIZE(sr030pc50_wb_daylight_regs);

	sensor->wb_cloudy_regs =
		sr030pc50_wb_cloudy_regs;
	sensor->rows_num_wb_cloudy_regs =
		ARRAY_SIZE(sr030pc50_wb_cloudy_regs);

	sensor->wb_incandescent_regs =
		sr030pc50_wb_incandescent_regs;
	sensor->rows_num_wb_incandescent_regs =
		ARRAY_SIZE(sr030pc50_wb_incandescent_regs);

	sensor->wb_fluorescent_regs =
		sr030pc50_wb_fluorescent_regs;
	sensor->rows_num_wb_fluorescent_regs =
		ARRAY_SIZE(sr030pc50_wb_fluorescent_regs);

	/*metering*/
	sensor->metering_matrix_regs =
		sr030pc50_metering_matrix_regs;
	sensor->rows_num_metering_matrix_regs =
		ARRAY_SIZE(sr030pc50_metering_matrix_regs);

	sensor->metering_center_regs =
		sr030pc50_metering_center_regs;
	sensor->rows_num_metering_center_regs =
		ARRAY_SIZE(sr030pc50_metering_center_regs);

	sensor->metering_spot_regs =
		sr030pc50_metering_spot_regs;
	sensor->rows_num_metering_spot_regs =
		ARRAY_SIZE(sr030pc50_metering_spot_regs);

	/*EV*/
	sensor->ev_minus_4_regs =
		sr030pc50_ev_minus_4_regs;
	sensor->rows_num_ev_minus_4_regs =
		ARRAY_SIZE(sr030pc50_ev_minus_4_regs);

	sensor->ev_minus_3_regs =
		sr030pc50_ev_minus_3_regs;
	sensor->rows_num_ev_minus_3_regs =
		ARRAY_SIZE(sr030pc50_ev_minus_3_regs);

	sensor->ev_minus_2_regs =
		sr030pc50_ev_minus_2_regs;
	sensor->rows_num_ev_minus_2_regs =
		ARRAY_SIZE(sr030pc50_ev_minus_2_regs);

	sensor->ev_minus_1_regs =
		sr030pc50_ev_minus_1_regs;
	sensor->rows_num_ev_minus_1_regs =
		ARRAY_SIZE(sr030pc50_ev_minus_1_regs);

	sensor->ev_default_regs =
		sr030pc50_ev_default_regs;
	sensor->rows_num_ev_default_regs =
		ARRAY_SIZE(sr030pc50_ev_default_regs);

	sensor->ev_plus_1_regs =
		sr030pc50_ev_plus_1_regs;
	sensor->rows_num_ev_plus_1_regs =
		ARRAY_SIZE(sr030pc50_ev_plus_1_regs);

	sensor->ev_plus_2_regs =
		sr030pc50_ev_plus_2_regs;
	sensor->rows_num_ev_plus_2_regs =
		ARRAY_SIZE(sr030pc50_ev_plus_2_regs);

	sensor->ev_plus_3_regs =
		sr030pc50_ev_plus_3_regs;
	sensor->rows_num_ev_plus_3_regs =
		ARRAY_SIZE(sr030pc50_ev_plus_3_regs);

	sensor->ev_plus_4_regs =
		sr030pc50_ev_plus_4_regs;
	sensor->rows_num_ev_plus_4_regs =
		ARRAY_SIZE(sr030pc50_ev_plus_4_regs);


	/*contrast*/
	sensor->contrast_minus_2_regs =
		sr030pc50_contrast_minus_2_regs;
	sensor->rows_num_contrast_minus_2_regs =
		ARRAY_SIZE(sr030pc50_contrast_minus_2_regs);

	sensor->contrast_minus_1_regs =
		sr030pc50_contrast_minus_1_regs;
	sensor->rows_num_contrast_minus_1_regs =
		ARRAY_SIZE(sr030pc50_contrast_minus_1_regs);

	sensor->contrast_default_regs =
		sr030pc50_contrast_default_regs;
	sensor->rows_num_contrast_default_regs =
		ARRAY_SIZE(sr030pc50_contrast_default_regs);

	sensor->contrast_plus_1_regs =
		sr030pc50_contrast_plus_1_regs;
	sensor->rows_num_contrast_plus_1_regs =
		ARRAY_SIZE(sr030pc50_contrast_plus_1_regs);

	sensor->contrast_plus_2_regs =
		sr030pc50_contrast_plus_2_regs;
	sensor->rows_num_contrast_plus_2_regs =
		ARRAY_SIZE(sr030pc50_contrast_plus_2_regs);
	...
	sensor->rows_num_vt_mode_regs =
		ARRAY_SIZE(sr030pc50_vt_mode_regs);
