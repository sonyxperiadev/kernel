...
static struct camdrv_ss_sensor_cap   sensor={0};
 ...
 * Returns 0 on success, <0 on error
 ***************************************************************************/
static int camdrv_ss_i2c_set_config_register(struct i2c_client *client, 
                                         regs_t reg_buffer[], 
          				                 int num_of_regs, 
          				                 char *name)
{
...
static int camdrv_ss_s_fmt(struct v4l2_subdev *sd, struct v4l2_mbus_framefmt *fmt)
{
	...
	else 
	{
		...
		state->preview_framesize_index = sensor.supported_preview_framesize_list[index].index;
	 	CAM_INFO_MSG(&client->dev, "%s %s : preview_framesize_index=%d  \n",sensor.name, __func__,state->preview_framesize_index);
					...
					struct v4l2_frmivalenum *fival)
{
	...
	if (fival->index >= 1)
	{
		CAM_ERROR_MSG(&client->dev, "%s %s : interval->index =%d is >=1 \n",sensor.name,__func__,fival->index);
		return -EINVAL;
	}
...
static int camdrv_ss_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->id) 
	{
		...
		case V4L2_CID_CAMERA_ZOOM:
		{
			...
			printk("BILLA V4L2_CID_CAMERA_ZOOM success \n");
			break;
        }
...
static int camdrv_ss_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	//printk("Error !!!!, camdrv_ss_s_stream is empty\n");
	return 0;
}
