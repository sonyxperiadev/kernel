 ...
 * bandwidth(BASELINE_DIMENSIONS_LEGACY.width, BASELINE_DIMENSIONS_LEGACY.height)
 */
const struct {
	int height, width;
} BASELINE_DIMENSIONS_LEGACY = {
	.width = 1280,
	.height = 720,
};
 ...
 */
const unsigned long LEGACY_NOMINAL_BW_MBPS = 6000 /* ideally 320 Mhz */,
	LEGACY_SVS_BW_MBPS = 2000 /* ideally 100 Mhz */;
...
static unsigned long __calculate_decoder(struct vidc_bus_vote_data *d,
	...
	width = max(d->input_width, BASELINE_DIMENSIONS_LEGACY.width);
	height = max(d->input_height, BASELINE_DIMENSIONS_LEGACY.height);
	...
	if (width * height * fps >= 3840 * 2160 * 60)
		vmem.total = FP_INT(LEGACY_NOMINAL_BW_MBPS);
	else if (width * height * fps >= 3840 * 2160 * 30)
		vmem.total = FP_INT(LEGACY_SVS_BW_MBPS);
...
static int __init msm_vidc_bw_vmem_gov_init(void)
{
	int c = 0, rc = 0;
...
module_init(msm_vidc_bw_vmem_gov_init);

static void __exit msm_vidc_bw_vmem_gov_exit(void)
{
	int c = 0;
...
module_exit(msm_vidc_bw_vmem_gov_exit);
