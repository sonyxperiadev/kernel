...
static RPC_XdrInfo_t ATC_Prim_dscrm[] = {
	...
	{MSG_AT_COMMAND_REQ, "MSG_AT_COMMAND_REQ", (xdrproc_t) xdr_AtCmdInfo_t,
	 sizeof(AtCmdInfo_t), AT_PARM_BUFFER_LEN}
	,
	{MSG_AT_COMMAND_IND, "MSG_AT_COMMAND_IND", (xdrproc_t) xdr_AtCmdInfo_t,
	 sizeof(AtCmdInfo_t), 8000}
	,
	{MSG_AT_REGISTER_REQ, "MSG_AT_REGISTER_REQ",
	 (xdrproc_t) xdr_AtRegisterInfo_t, sizeof(AtRegisterInfo_t), 0}
	,
	{MSG_AT_AUDIO_REQ, "MSG_AT_AUDIO_REQ", (xdrproc_t) xdr_u_char,
	 sizeof(Boolean), 0}
	,
...
static long ATC_KERNEL_Ioctl(struct file *filp, unsigned int cmd, UInt32 arg)
{
	...
	case ATC_KERNEL_SEND_AT_CMD:
		{
			ATC_KERNEL_ATCmd_t *inAtCmd =
			    (ATC_KERNEL_ATCmd_t *) arg;
			...
			newCmdQueueItem =
			    kmalloc(sizeof(AT_CmdQueue_t), GFP_KERNEL);
			if (!newCmdQueueItem) {
			...
			newCmdQueueItem->mATCmd.fATCmdStr =
			    kmalloc(strLen + 1, GFP_KERNEL);
			...
			list_add_tail(&newCmdQueueItem->mList,
				      &sModule.mCmdQueue.mList);
...
//                              UInt16 len = ATC_KERNEL_RESULT_BUFFER_LEN_MAX - 1;
			...
			if (copy_from_user
			    (&atRespU, (ATC_KERNEL_ATResp_t *) arg,
			     sizeof(atRespU))) {
				retVal = -1;
				break;
			}
			...
			if (copy_to_user
			    (atRespU.buffPtr, respItem->mATResp.buffPtr,
			     respItem->mATResp.dataLen) != 0) {
	...
	case ATC_KERNEL_REG_AT_TERMINAL:
		{
			ATC_KERNEL_AtRegisterInfo_t *inRegInfo =
			    (ATC_KERNEL_AtRegisterInfo_t *) arg;
			...
			ATC_RegisterCPTerminal(inRegInfo->channel,
					       inRegInfo->unsolicited);
			break;
		}
...
static bool_t xdr_AtCmdInfo_t(XDR * xdrs, AtCmdInfo_t * data)
{
	if (xdr_int16_t(xdrs, &data->len)) {
		...
		return (xdr_bytes
			(xdrs, (char **)(void *)&data->buffer, &len, 0xFFFF)
...
bool_t xdr_AtRegisterInfo_t(XDR * xdrs, AtRegisterInfo_t * data)
{
	XDR_LOG(xdrs, "AtRegisterInfo_t")
	    ...
	    return (_xdr_u_char(xdrs, &data->channel, "channel") &&
		    _xdr_u_char(xdrs, (u_char *) & data->unsolicited,
				"unsolicited"));
}
...
static void ATC_ATRPCInit(void)
{
	...
	if (0 == sModule.mRPCHandle) {
		...
		params.table_size =
		    (sizeof(ATC_Prim_dscrm) / sizeof(RPC_XdrInfo_t));
	...
	case MSG_AT_COMMAND_IND:
		{
			...
			ATC_AddRespToQueue(atResponse->channel, pMsg->msgId,
					   atResponse->buffer, atResponse->len);
			break;
		}
...
static int __init ATC_KERNEL_ModuleInit(void)
{
	...
	if (register_chrdev
	    (BCM_AT_MAJOR, ATC_KERNEL_MODULE_NAME, &sFileOperations) < 0) {
		ATC_KERNEL_TRACE(("register_chrdev failed\n"));
		ATC_Cleanup();
		return -1;
	}

	sModule.mDriverClass =
	    class_create(THIS_MODULE, ATC_KERNEL_MODULE_NAME);
	...
	drvdata =
	    device_create(sModule.mDriverClass, NULL, MKDEV(BCM_AT_MAJOR, 0),
			  NULL, ATC_KERNEL_MODULE_NAME);
	if (IS_ERR(drvdata)) {
