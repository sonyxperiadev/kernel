...
typedef enum {
	AT_ACCESS_UNKNOWN,
			/**< unknown */
	AT_ACCESS_REGULAR,
			/**< invoke or set; examples: at+cops=1,2; ate; ate0 */
...
typedef enum {
	AT_CMDTYPE_UNKNOWN,
			/**< unknown */
	AT_CMDTYPE_BASIC,
			/**< basic command; examples: ATE, ATE0 */
	AT_CMDTYPE_EXTENDED,
			 /**< extended command; examples: AT+COPS=1,2,3 */
	AT_CMDTYPE_SPECIAL
			/**< special command accepts everything as arg; example: ATD12345; */
} AT_CmdTypes_t;
...
typedef enum {
	...
	AT_OPTION_USER_DEFINED_TEST_RSP = 0x04,
					    /**< user-defined TEST response implemented in handler */
...
*/
typedef AT_Status_t(*AT_CmdHndlr_t) (AT_CmdId_t cmdId,
				     UInt8 chan, UInt8 accessType, ...);
