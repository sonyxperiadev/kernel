...
uint32 GetHuk(uint8 *huk)
{
	...
	for (addr = CUS_ROW_START;
	     (addr <= CUS_ROW_END) && (read_len < HUK_LEN); ++addr) {
		if (otp_read_word(addr, 1, &data) == 0) {
			if (data & OTP_LOTP_RDATA_LOTP_RDATA_FAIL)
				continue;
	...
	pr_err("OTP not supported. GetHuk() Failed!!!\n");
	return 0;
