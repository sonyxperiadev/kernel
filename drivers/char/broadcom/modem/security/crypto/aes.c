...
void MixColumns(uint8 data[4][4])
{
	...
	for (y = 0; y < 4; y++) {
		newOutput[0][y] =
		    ModMult(data[0][y], 2) ^ ModMult(data[1][y],
						     3) ^ data[2][y] ^
		    data[3][y];
		newOutput[1][y] =
		    data[0][y] ^ ModMult(data[1][y], 2) ^ ModMult(data[2][y],
								  3) ^
		    data[3][y];
		newOutput[2][y] =
		    data[0][y] ^ data[1][y] ^ ModMult(data[2][y],
						      2) ^ ModMult(data[3][y],
								   3);
		newOutput[3][y] =
		    ModMult(data[0][y],
			    3) ^ data[1][y] ^ data[2][y] ^ ModMult(data[3][y],
								   2);
	}
...
void InvMixColumns(uint8 data[4][4])
{
	...
	for (y = 0; y < 4; y++) {
		newOutput[0][y] =
		    ModMult(data[0][y], 0xe) ^ ModMult(data[1][y],
						       0xb) ^
		    ModMult(data[2][y], 0xd) ^ ModMult(data[3][y], 0x9);
		newOutput[1][y] =
		    ModMult(data[0][y], 0x9) ^ ModMult(data[1][y],
						       0xe) ^
		    ModMult(data[2][y], 0xb) ^ ModMult(data[3][y], 0xd);
		newOutput[2][y] =
		    ModMult(data[0][y], 0xd) ^ ModMult(data[1][y],
						       0x9) ^
		    ModMult(data[2][y], 0xe) ^ ModMult(data[3][y], 0xb);
		newOutput[3][y] =
		    ModMult(data[0][y], 0xb) ^ ModMult(data[1][y],
						       0xd) ^
		    ModMult(data[2][y], 0x9) ^ ModMult(data[3][y], 0xe);
	}
...
/*         printf("ERROR: AES key size\n"); */
	...
	while (i < Nk) {
		w[i] =
		    key[4 * i] << 24 | key[4 * i + 1] << 16 | key[4 * i +
								  2] << 8 |
		    key[4 * i + 3];
		i++;
	}
	      ...
	      const uint8 *key, uint32 keySize, uint32 operation)
{
	...
	while (dataLen >= AES_BLOCK_SIZE) {
		if (operation == AES_OPERATION_ENCRYPT) {
			/* Do the encryption */
			Cipher(input + dataIndex, testOutput, testRoundKey,
			       keySize);
		} else {
			/* Do the decryption */
			InvCipher(input + dataIndex, testOutput, testRoundKey,
				  keySize);
		}
	   ...
	   uint32 inMode, uint32 inOperation)
{
	if (inMode == AES_MODE_ECB) {
		return AesEcb
			(outDataPtr, inDataPtr, inDataSize, inKeyPtr, inKeySize,
			 inOperation);
