...
void SysRpc_OpenRegulator(PMU_SIMLDO_t ldo)
{
	...
	else
	{
	    ...
	    KRIL_DEBUG(DBG_INFO," **regulator_get (dev=%s) PASS handle=%p ret=%d\n", curReg->devName, curReg->handle, ret);
	}
}
...
Result_t Handle_CAPI2_SYSRPC_PMU_IsSIMReady(RPC_Msg_t* pReqMsg, PMU_SIMLDO_t simldo)
{
	...
	if(!IS_ERR(curReg->handle))
	{
		...
		KRIL_DEBUG(DBG_INFO,"  Handle_CAPI2_PMU_IsSIMReady ldo=%d handle=%p ret=%d\n\n", (int)simldo, curReg->handle, (int)ret);
	}
	else
		KRIL_DEBUG(DBG_INFO," regulator_is_enabled Invalid Handle  %d\n", (int)IS_ERR(curReg->handle));
...
Result_t Handle_CAPI2_SYSRPC_PMU_ActivateSIM(RPC_Msg_t* pReqMsg, PMU_SIMLDO_t simldo, PMU_SIMVolt_t volt)
{
    ...
    KRIL_DEBUG(DBG_INFO," enter Handle_CAPI2_PMU_ActivateSIM ldo=%d handle=%p active=%d\n", simldo, curReg->handle, curReg->isSimInit);
    ...
    switch (volt)
    {
        ...
        case PMU_SIM0P0Volt:
        {
            ...
            {
                int ret;
                KRIL_DEBUG(DBG_INFO," ** PMU_SIM0P0Volt - turn off regulator (FORCE)\n");
        ...
        default:
        {
            KRIL_DEBUG(DBG_INFO," unrecognized value for volt %d\n", (int)volt);
            break;
        }
