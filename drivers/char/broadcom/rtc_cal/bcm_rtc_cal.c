...
static u32 rtc_cal_test_ratio = 0;
static u32 rtc_cal_cp_ratio = 0;

static rtc_cal_TC_t sRTC_TC = {0};
...
static unsigned long RTCCAL_Ratio_max = 0;
static unsigned long RTCCAL_Range = 0;
static unsigned long RTCCAL_Threshold = 0;
static unsigned long RTCCAL_Int_Measure = 0;
static unsigned long RTCCAL_Int_Calc = 0;
static unsigned long RTCCAL_Slippage = 0;
static unsigned long RTCCAL_PeriodicUpdate = 0;
static unsigned long RTCCAL_Method = 0;

static s64 rtccal_period = 0;
static bool rtc_correction = false;
static struct timer_list rtccal_tm_update;
static struct timer_list rtccal_tm_correct;

static Boolean IsTimerRun = FALSE;
static Boolean IsCorrectionTimerRun = FALSE;
...
static struct bcmpmu_rtc *rtc_cal_bcmpmu = NULL;
...
extern void RTCSC_InitRpc(void);
...
static const unsigned char sYearCode[100] = {0,1,2,3,5,6,0,1,3,4,5,6,1,2,3,4,6,0,1,2,\
...
static const unsigned char sMonthCode[12] = {0,3,3,6,1,4,6,2,5,0,3,5};
...
#if 0
static void rtc_cal_pr_time_hw(struct rtc_time *tm)
{
	if(rtc_cal_debug == 0)
		return;
	pr_rtcdebug("%s: time=%d.%d.%d.%d.%d.%d",
		__func__, 
...
static void rtc_cal_pr_time_sw(rtctime_t *rtm)
{
	if(rtc_cal_debug == 0)
		return;
	pr_rtcdebug("%s: time=%d.%d.%d.%d.%d.%d",
		__func__, 
		rtm->year,rtm->mon,rtm->mday,
		rtm->hour,rtm->min,rtm->sec);
...
static void rtc_cal_pr_vars(void)
{
	...
	pr_rtcdebug("\t\thw_time:%lu, \tthz_acc:%lu, \tslip_acc:%llu, \terr_acc:%llu, \tflag_err_acc:%d",pRTC_TC->time_hw_time, pRTC_TC->time_hz_acc,pRTC_TC->slippage_acc,pRTC_TC->time_err_acc,pRTC_TC->flag_err_acc);
	pr_rtcdebug("\t\thz_curr:%lu, \thz_prev:%lu, \thz_intv:%lu, \terr_curr:%llu, \tflag_err_curr:%d",pRTC_TC->time_hz_curr,pRTC_TC->time_hz_prev,pRTC_TC->time_hz_interval,pRTC_TC->time_err_curr,pRTC_TC->flag_err_curr);
	pr_rtcdebug("\t\tratio_curr:%lu, \tflag_ratio:%d, \ttime_next:%lu, \thz_delt:%lu",pRTC_TC->time_ratio_curr,pRTC_TC->flag_ratio_curr,pRTC_TC->time_next,pRTC_TC->time_hz_delt);
}

//******************************************************************************
// Function Name:	rtc_util_isleapyear
//
// Description:		This function returns TRUE if the passed year is a leap year in 
...
//
//******************************************************************************
unsigned char rtc_util_calcweekday(rtctime_t	*time)
{
	...
	weekday = (6 + sYearCode[year-RTC_YEARBASE] + sMonthCode[month-1] + day + leapCode) % 7;
...
//
// Description:		This function returns the number of days in a particular month of a year 
//					
// unsigned int year - Year in number. For example, pass 2008 for Year 2008. 
// unsigned char month - Month in number from 1 to 12. 
// 
//******************************************************************************
unsigned char rtc_util_getnumofdaysinmon(unsigned int year, unsigned char month)
{
	static const unsigned char num_of_days_in_month[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
...
// 
//******************************************************************************
unsigned long rtc_util_convert_rtctime_t2sec(rtctime_t * pTimeSrc)
{
    unsigned char x;
...
// 
//******************************************************************************
void rtc_util_convert_sec2rtctime_t(rtctime_t *pDest, unsigned long time)
{
	...
	pDest->mday = (days >= rtc_util_getnumofdaysinmon(RTC_YEARBASE + y, m)) ? 1 : (days + 1);
	rtc_util_calcweekday(pDest);
}
...
// 
//******************************************************************************
void rtc_util_convert_tm2rtctime_t(rtctime_t *pDest, struct rtc_time *tm)
{
	...
	pDest->min = tm->tm_min;
	pDest->sec = tm->tm_sec;
	pr_rtcdebug("%s:IN:Y=%d,M/M/W=%d/%d/%d,H:M:S=%d:%d:%d",__func__,tm->tm_year,tm->tm_mon,tm->tm_mday,tm->tm_wday,tm->tm_hour,tm->tm_min,tm->tm_sec);
	pr_rtcdebug("%s:OUT:Y=%d,M/M/W=%d/%d/%d,H:M:S=%d:%d:%d",__func__,pDest->year,pDest->mon,pDest->mday,pDest->week,pDest->hour,pDest->min,pDest->sec);
}
...
// 
//******************************************************************************
void rtc_util_convert_rtctime_t2tm(struct rtc_time *pDest, rtctime_t *tm)
{
	...
	pDest->tm_min = tm->min;
	pDest->tm_sec = tm->sec;
	pr_rtcdebug("%s:IN:Y=%d,M/M/W=%d/%d/%d,H:M:S=%d:%d:%d",__func__,tm->year,tm->mon,tm->mday,tm->week,tm->hour,tm->min,tm->sec);
	pr_rtcdebug("%s:OUT:Y=%d,M/M/W=%d/%d/%d,H:M:S=%d:%d:%d",__func__,pDest->tm_year,pDest->tm_mon,pDest->tm_mday,pDest->tm_wday,pDest->tm_hour,pDest->tm_min,pDest->tm_sec);
}
...
**/
bool rtc_cal_msg(rtc_cal_msg_id_t id, unsigned long val1, unsigned long val2, unsigned long val3, unsigned long val4)
{
...
void bcm_rtc_cal_timerratio(unsigned long ratio, unsigned int temp)
{
	...
	pr_rtcdebug("%s:ratio=%lu,ratio_curr/time=%lu,flag=%u",__func__,ratio,pRTC_TC->time_ratio_curr,pRTC_TC->flag_ratio_curr);
}
...
static u64 rtc_cal_tc_updatevals(unsigned long time)
{
	...
	time_total = pRTC_TC->time_hw_time*HZ + pRTC_TC->time_hz_acc + pRTC_TC->time_hz_curr;

	time_left = pRTC_TC->time_hz_delt - pRTC_TC->time_hw_time*HZ;
	pr_rtcdebug("%s: hw=%lu, hz_acc=%lu, hz_curr=%lu",__func__,pRTC_TC->time_hw_time*HZ,pRTC_TC->time_hz_acc,pRTC_TC->time_hz_curr);
	pr_rtcdebug("%s: t=%lu, l=%lu, delt=%lu",__func__,time_total,time_left,pRTC_TC->time_hz_delt);
	...
	{
		pr_rtcdebug("%s:NO01 %lu",__func__,time_left-pRTC_TC->time_hz_curr);
	...
	pr_rtcdebug("%s: hz_curr=%lu, hz_acc=%lu, time_left=%lu",__func__,pRTC_TC->time_hz_curr,pRTC_TC->time_hz_acc,time_left);


	mul64to128(&rtc_mul1, &rtc_mul, rtc_m, rtc_n);
	rtc_slip = div64(rtc_mul,(unsigned long)RTC_MULF);

	pr_rtcdebug("%s: m=%llu, n=%llu, mul=%llu,mul1=%llu",__func__,rtc_m,rtc_n,rtc_mul,rtc_mul1);

	pr_rtcdebug("%s: slip=%llu, err_acc=%llu, slip_acc=%llu",__func__,rtc_slip,pRTC_TC->time_err_acc,pRTC_TC->slippage_acc);

	if((rtc_slip<pRTC_TC->slippage_acc)&&(rtc_slip>pRTC_TC->time_err_acc))
	{
	...
	else
	{
		if(rtc_slip<pRTC_TC->slippage_acc)
			pRTC_TC->time_err_acc = pRTC_TC->slippage_acc - rtc_slip;
		else
			pRTC_TC->time_err_acc = 0;
		pRTC_TC->slippage_acc -= rtc_slip;
		pRTC_TC->flag_err_acc = pRTC_TC->flag_ratio_curr;
	...
	pr_rtcdebug("%s: flag_acc=%d, time_acc=%llu",__func__,pRTC_TC->flag_err_acc,pRTC_TC->time_err_acc);
...
unsigned long rtc_cal_calc_slippage(void)
{
	...
	{
		add128(&temp2,&temp1,dummy1,pRTC_TC->time_err_acc,dummy2,pRTC_TC->time_err_curr);
		pRTC_TC->slippage_flag = pRTC_TC->flag_ratio_curr;
	}
	else if(pRTC_TC->flag_err_acc == true) // neg
	{
		if(pRTC_TC->time_err_acc>pRTC_TC->time_err_curr)
		{
			sub128(&temp2,&temp1,dummy1,pRTC_TC->time_err_acc,dummy2,pRTC_TC->time_err_curr);
			pRTC_TC->slippage_flag = pRTC_TC->flag_err_acc;
		}
		else
		{
			sub128(&temp2,&temp1,dummy1,pRTC_TC->time_err_curr,dummy2,pRTC_TC->time_err_acc);
			pRTC_TC->slippage_flag = pRTC_TC->flag_err_curr;
		}
	}
	else if(pRTC_TC->flag_err_acc == false) // pos
	{
		if(pRTC_TC->time_err_acc>pRTC_TC->time_err_curr)
		{
			sub128(&temp2,&temp1,dummy1,pRTC_TC->time_err_acc,dummy2,pRTC_TC->time_err_curr);
...
void rtc_cal_tc_update(void)
{
	...
	{
		pRTC_TC->time_hz_interval = rtc_jiffies + 0xffffffff - rtc_jiffies_prev;
	}
	else
	{
	...
	if(rtc_update==TRUE)
	{
		...
		rtc_total_hz = pRTC_TC->time_hz_curr + pRTC_TC->time_hz_acc + rtc_slippage *HZ / RTC_ONE_SEC ;
		rtc_time_temp1 = rtc_total_hz + HZ; // HZ
		rtc_time_temp2 = rtc_time_temp1 / HZ;// sec
		rtc_time_delt = rtc_time_temp1 - rtc_time_temp2 * HZ;

		pr_rtcdebug("%s:total=%lu,temp1=%lu,temp2=%lu,delt=%lu",__func__,rtc_total_hz,rtc_time_temp1,rtc_time_temp2,rtc_time_delt);
	...
	if((RTCCAL_PeriodicUpdate)&&( (rtccal_period/HZ) > RTCCAL_PeriodicUpdate))
	{
		if(rtc_update == FALSE)
		{
			unsigned long rtc_total_hz;
...
/*
 * API for handling ratio event. RPC on AP is calling this API to forward ratio event
 */
void rtc_cal_tc_get_time(struct rtc_time *tm)
{
	...
	if(pRTC_TC->slippage_flag)
		rtc_time_hz = pRTC_TC->time_hz_curr + pRTC_TC->time_hz_acc - temp_hz;
	else
		rtc_time_hz = pRTC_TC->time_hz_curr + pRTC_TC->time_hz_acc + temp_hz;
	rtc_time = pRTC_TC->time_hw_time;
	rtc_time += (rtc_time_hz / HZ );

	pr_rtcdebug("%s:hw_time=%lu,hz_curr=%lu,hz_acc=%lu,slipp=%lu,flag=%d",__func__,pRTC_TC->time_hw_time,pRTC_TC->time_hz_curr,pRTC_TC->time_hz_acc,pRTC_TC->slippage_ms,pRTC_TC->slippage_flag);
	...
	while( TRUE )
	{
		...
		switch(msg.id)
			...
			case RTCCAL_ID_SETTIME:		// set time access from MMI or  upper layer
				pr_rtcdebug("RTCCAL_ID_SETTIME : msg.val1=%lu",msg.val1);
			...
			case RTCCAL_ID_CALEVENT:		// Cal event from CP
				pr_rtcdebug("RTCCAL_ID_CALEVENT : val1=%lu,val2=%lu",msg.val1,msg.val2);
				rtc_cal_tc_update();
				rtc_cal_tc_savevals();
				bcm_rtc_cal_timerratio(msg.val1, (unsigned int)(msg.val2));
				rtc_cal_updatetimerstart();
				break;

			case RTCCAL_ID_UPDATETIME:
				pr_rtcdebug("RTCCAL_ID_UPDATETIME : time=%lu",msg.val1);
				rtc_cal_tc_update();
				...
				rtc_cal_tc_start(msg.val1, rtc_slip_prev, rtc_slip_flag);
				rtc_cal_updatetimerstart();
				break;

			case RTCCAL_ID_TIMERCB:
				pr_rtcdebug("RTCCAL_ID_TIMERCB : val1=%lu",msg.val1);
				rtc_cal_tc_update();
				break;
			...
			case RTCCAL_ID_GETTIME: // read and set initial time
				...
				pr_rtcdebug("RTCCAL_ID_GETTIME : time_sec=%lu",time_sec);
...
bool rtc_cal_init(void)
{
	...
	pr_rtcdebug("rtc_cal_init : RTCCAL_Ratio=%lu",RTCCAL_Ratio);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Ratio_min=%lu",RTCCAL_Ratio_min);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Ratio_max=%lu",RTCCAL_Ratio_max);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Range=%lu",RTCCAL_Range);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Threshold=%lu",RTCCAL_Threshold);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Int_Measure=%lu",RTCCAL_Int_Measure);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Int_Calc=%lu",RTCCAL_Int_Calc);
	pr_rtcdebug("rtc_cal_init : RTCCAL_Slippage=%lu",RTCCAL_Slippage);
	pr_rtcdebug("rtc_cal_init : RTCCAL_PeriodicUpdate=%lu",RTCCAL_PeriodicUpdate);

	memset(&sRTC_TC, 0, sizeof(rtc_cal_TC_t));
	pRTC_TC = &sRTC_TC;
...
{		
	if(IsRTCRun)
		{
		pr_rtcdebug("rtc_cal_run : already started");
		return;
		}
...
/*
 * API for setting time to RTC calibration component. Broadcom RTC device driver needs to call this API for RTC calibration feature.
 */
...
/*
 * API for reading time from RTC calibration component. Broadcom RTC device driver needs to call this API for RTC calibration feature.
 */
...
/*
 * API for setting register value of rtc calibration in the PMU device. HW dependent API.
 */
...
/*
 * API for reading register value of rtc calibraiton in the PMU device. HW dependent API.
 */
...
/*
 * API for disabling/enabling rtc calibration feature in the PMU device. HW dependent API.
 */
				...
				size_t count, loff_t *ppos)
{
	...
	// provide parameters for current calibration process
...
static struct file_operations rtc_cal_parameter_fops =
{
				...
				size_t count, loff_t *ppos)
{
	...
	len += snprintf(buf+len, sizeof(buf)-len,
			"\terr_read1/2 = %d/%d\t", pRTC_TC->err_read1,pRTC_TC->err_read2);
	len += snprintf(buf+len, sizeof(buf)-len,
...
static struct file_operations rtc_cal_info_fops =
{
...
static struct file_operations rtc_cal_read_time_fops =
{
...
static struct file_operations rtc_cal_test_fops =
{
...
static struct file_operations rtc_cal_test_fops1 =
{
...
static struct file_operations rtc_cal_test_fops2 =
{
...
int __init bcm_rtc_cal_debug_init(void)
{
    ...
    if(!debugfs_create_u32("debug", S_IRUSR|S_IWUSR, dent_rtc_cal_root_dir, &rtc_cal_debug))
		return -ENOMEM;

    if(!debugfs_create_file("parameters", S_IRUSR, dent_rtc_cal_root_dir, NULL, &rtc_cal_parameter_fops))
		return -ENOMEM;

    if(!debugfs_create_file("read_time", S_IRUSR, dent_rtc_cal_root_dir, NULL, &rtc_cal_read_time_fops))
		return -ENOMEM;

    if(!debugfs_create_file("info", S_IRUSR, dent_rtc_cal_root_dir, NULL, &rtc_cal_info_fops))
		return -ENOMEM;

    if(!debugfs_create_file("test", S_IRUSR, dent_rtc_cal_root_dir, NULL, &rtc_cal_test_fops))
		return -ENOMEM;

    if(!debugfs_create_file("test_min", S_IRUSR, dent_rtc_cal_root_dir, NULL, &rtc_cal_test_fops1))
		return -ENOMEM;

    if(!debugfs_create_file("test_max", S_IRUSR, dent_rtc_cal_root_dir, NULL, &rtc_cal_test_fops2))
		return -ENOMEM;

    if(!debugfs_create_u32("ratio", S_IRUSR|S_IWUSR, dent_rtc_cal_root_dir, &rtc_cal_test_ratio))
		return -ENOMEM;

    if(!debugfs_create_u32("cp_ratio", S_IRUSR, dent_rtc_cal_root_dir, &rtc_cal_cp_ratio))
		return -ENOMEM;
...
/*
 * API for initializing rtc calibration component. Broadcom RTC device driver needs to call this API when initialization device driver.
 */
...
/*
 * API for shutting down rtc caliration component. Broadcom RTC device driver needs to call this API when shutdown device driver.
 */
