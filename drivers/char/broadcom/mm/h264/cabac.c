...
static void cabac_write(cabac_device_t *cabac, u32 reg, u32 value)
{
	return mm_write_reg(cabac->vaddr, (reg-0), value);
}

static u32 cabac_read(cabac_device_t *cabac, u32 reg)
{
	return mm_read_reg(cabac->vaddr, (reg-0));
}

static void print_job_struct(cabac_job_t* job)
{
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
static void print_regs(cabac_device_t *cabac)
{
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	pr_debug("H264_REGCS_REGCABAC2BINSIMGCTXLAST_OFFSET: 0x%x\n",
				cabac_read(cabac,H264_REGCS_REGCABAC2BINSIMGCTXLAST_OFFSET));
	pr_debug("H264_REGCS_REGCABAC2BINSRDCONTEXTBASEADDR_OFFSET: 0x%x\n",
				cabac_read(cabac,H264_REGCS_REGCABAC2BINSRDCONTEXTBASEADDR_OFFSET));
	pr_debug("H264_REGCS_REGCABAC2BINSWRCONTEXTBASEADDR_OFFSET: 0x%x\n",
				cabac_read(cabac,H264_REGCS_REGCABAC2BINSWRCONTEXTBASEADDR_OFFSET));
	pr_debug("H264_REGC2_REGCABAC2BINSCOMMANDBUFFERADDR_OFFSET: 0x%x\n",
				cabac_read(cabac,H264_REGC2_REGCABAC2BINSCOMMANDBUFFERADDR_OFFSET));
	pr_debug("H264_REGC2_REGCABAC2BINSCOMMANDBUFFERCOUNT_OFFSET: 0x%x\n",
				cabac_read(cabac,H264_REGC2_REGCABAC2BINSCOMMANDBUFFERCOUNT_OFFSET));
...
static int cabac_abort(void* device_id, mm_job_post_t* job)
{
	...
	while(cabac_read(id,H264_REGC2_REGCABAC2BINSCTL_OFFSET) & 0X407 ||
		cabac_read(id,H264_REGC2_REGCABAC2BINSCOMMANDBUFFERCOUNT_OFFSET) & 0x7FF);
...
mm_job_status_e cabac_start_job(void* device_id , mm_job_post_t* job, u32 profmask)
{
	cabac_device_t* id = (cabac_device_t*)device_id;
	cabac_job_t* jp = (cabac_job_t*)job->data;
	int i = 0;
...
cabac_device_t* cabac_device = NULL;
...
int cabac_init(MM_CORE_HW_IFC* core_param)
{
	...
	cabac_device = kmalloc(sizeof(cabac_device_t), GFP_KERNEL);
