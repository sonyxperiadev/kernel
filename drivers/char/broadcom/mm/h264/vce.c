...
static void vce_write(vce_device_t *vce, u32 reg, u32 value)
{
	return mm_write_reg(vce->vaddr, reg, value);
}

static u32 vce_read(vce_device_t *vce, u32 reg)
{
...
{
	vce_launch_info_t* vce_info;
	vce_info = (vce_launch_info_t*) ((u32*)job + *((u32*)job+2));
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
static void print_regs(vce_device_t *vce)
{
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
mm_job_status_e vce_start_job(void *device_id , mm_job_post_t *job, u32 prfmask)
{
	...
	vce_transfer_type_t transfer_dir;
	u32 vce_addr_offset;
	u32 transfer_size;
	...
	vce_info = (vce_launch_info_t*) ((u32*)jp + *((u32*)jp+2));
	...
	switch( job->status ){
		case MM_JOB_STATUS_READY:
			...
			transfer_ptr = (u32*)((u32*) jp + vce_info->download_start_offset);
			for(i=0; i<(vce_info->numdownloads); i++) {
				...
				for(j=0;j<transfer_size;j++) {
					vce_write(id,(VCE_DATA_MEM_OFFSET+vce_addr_offset+j*4),
		...
		case MM_JOB_STATUS_RUNNING:
			...
			/* Handle Upload*/
			transfer_ptr = (u32*)((u32*) jp + vce_info->upload_start_offset);
