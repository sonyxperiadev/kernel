...
static void mcin_write(mcin_device_t *mcin, u32 reg, u32 value)
{
	return mm_write_reg(mcin->vaddr, (reg-H264_MCODEIN_CONTROL_OFFSET), value);
}

static u32 mcin_read(mcin_device_t *mcin, u32 reg)
{
	return mm_read_reg(mcin->vaddr, (reg-H264_MCODEIN_CONTROL_OFFSET));
}

static void print_job_struct(mcin_job_t* job)
{
	int i = 0;
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
static void print_regs(mcin_device_t *mcin)
{
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
mm_job_status_e mcin_start_job(void* device_id , mm_job_post_t* job, u32 profmask)
{
	mcin_device_t* id = (mcin_device_t*)device_id;
	mcin_job_t* jp = (mcin_job_t*)job->data;
	...
	switch( job->status ){
		case MM_JOB_STATUS_READY:
			...
			mcin_write(id,H264_MCODEIN_OUTMARKOFFSET_OFFSET,jp->out_mark_offset);
			...
			if (jp->mcin_config.start_mask == 0xff)
				...
				/* Startcode occupies partial byte; preserve it to retain remainder */
...
mcin_device_t* mcin_device = NULL;
...
int mcin_init(MM_CORE_HW_IFC* core_param)
{
	...
	mcin_device = kmalloc(sizeof(mcin_device_t), GFP_KERNEL);
	if(mcin_device == NULL){
