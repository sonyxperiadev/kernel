...
typedef struct {
	volatile u32 cme_version;
	volatile void *vaddr;
} cme_device_t;

static inline void cme_write(cme_device_t *cme, u32 reg, u32 value)
{
	return mm_write_reg(cme->vaddr, (reg-H264_CME_CURY_OFFSET), value);
}

static inline u32 cme_read(cme_device_t *cme, u32 reg)
{
	return mm_read_reg(cme->vaddr, (reg-H264_CME_CURY_OFFSET));
}

static void print_job_struct(cme_job_t* job)
{
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
...
static void print_regs(cme_device_t *cme)
{
	pr_debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	...
	switch( job->status ){
		case MM_JOB_STATUS_READY:
			...
			if (id->cme_version < 0xCE2B && jp->hradius_mb  > 4)
				jp->hradius_mb = 4; /*2708A0 can only search up to +/- 4*/
...
int cme_init(MM_CORE_HW_IFC* core_param)
{
	...
	cme_device = kmalloc(sizeof(cme_device_t), GFP_KERNEL);
	if(cme_device == NULL){
