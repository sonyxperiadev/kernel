...
#define SINT4P12(x) ((sint4p12_t)(((x)>0)?((x)*4096+0.5):((x)*4096-0.5)))
#define UINT4P12(x) ((uint0p16_t)((x)*4096+0.5))
...
typedef struct isp_lines_avail_params_s {
   uint16_t lines_avail;
   uint8_t pad[2];
} ISP_LINES_AVAIL_PARAMS_T;


// ISP version parameters.

typedef struct isp_version_params_s {
   uint8_t revision;
   uint8_t major_version;
   uint8_t pad[2];
} ISP_VERSION_PARAMS_T;


// Frame parameters.

typedef struct isp_frame_params_s {
   uint16_t width;
   uint16_t height;
   ...
   uint8_t pad[3];
#if HERA_ISP
   // For Hera, we'll munge bayer_en and ycbcr_en to enable only those blocks that exist.
   // The requested enables we'll store here, so that they can be fed back unmunged.
...
typedef struct isp_tile_offset_params_s {
   ...
   uint8_t pad[1];
} ISP_TILE_OFFSET_PARAMS_T;

// Bayer input formatter parameters.

typedef struct isp_raw_input_params_s {
   void *address;
   uint32_t stride;
   void *end_address;
...
} ISP_RAW_INPUT_PARAMS_T;


// Coefficients used for converting RGB into YCbCr and vice versa.

typedef struct isp_y_g_coeffs_params_s {
   sint4p12_t y_coeffs[2]; // B-G and R-G coefficients to make Y from R, G and B
#if HERA_ISP
   uint4p12_t cb_scale;
   uint4p12_t cr_scale;
   uint16_t cb_offset;
   uint16_t cr_offset;
#endif
} ISP_Y_G_COEFFS_PARAMS_T;

// YUV input formatter.

typedef struct isp_yuv_input_params_s {
   int32_t /*ISP_CS_FORMAT_T*/ cs_format;
   uint8_t bytes_sample;
   uint8_t pad1[3];
// \todo - consider just using ISP_DATA_FORMAT_T for symmetry with the output formatters
   ...
   void *gu_end_address;
   void *bv_end_address;
} ISP_YUV_INPUT_PARAMS_T;

// Colour conversion parameters.

typedef struct isp_conversion_params_s {
   int16_t matrix[3][3];  // actually agnostic about the fixed/fractional parts here
   uint8_t pad[2];
   int32_t offsets[3];
} ISP_CONVERSION_PARAMS_T;

// High resolution resize parameters.

typedef struct isp_hresize_params_s {
   // Fields starting with an underscore do not actually have registers in the hardware,
   // so should not be used in the software simulator.
   ...
   int32_t /*ISP_INTERP_FILTER_T*/ interp_filter_mode; // type of resize, e.g. bilinear etc.
   uint16_t wscalef_x;                     // Whole part of the scaling factor in the horizontal direction
   uint16_t fscalef_x;                     // Fractional part of the scaling factor in the horizontal direction
   uint16_t normf_x;                       // Normalization factor for the horizontal direction
   uint16_t wscalef_y;                     // Whole part of the scaling factor in the vertical direction
   uint16_t fscalef_y;                     // Fractional part of the scaling factor in the vertical direction
   uint16_t normf_y;                       // Normalization factor for the vertical direction
   uint16_t output_width;                   // width of output window
   uint16_t output_height;                  // height of output window
} ISP_HRESIZE_PARAMS_T;

// Low resolution resize parameters.

typedef struct isp_lresize_params_s {
   // Fields starting with an underscore do not actually have registers in the hardware,
   // so should not be used in the software simulator.
   ...
   uint16_t _output_width;               // Output Image Width for new trapezoidal resizer
   uint16_t _output_height;              // Output Image Height for new trapezoidal resizer
#else
   uint16_t norm_x;                      // Normalization factor for the horizontal direction
   uint16_t norm_y;                      // Normalization factor for the vertical direction
   uint16_t lnorm_x;                     // Normalization factor for the horizontal direction for the last column
   uint16_t lnorm_y;                     // Normalization factor for the vertical direction for the last row
#endif
   ...
   uint8_t pad[3];
#endif
} ISP_LRESIZE_PARAMS_T;

// Output formatter parameters.

typedef struct isp_output_params_s {
   uint8_t data_format;
...
#endif
   uint16_t _buffer_height; // used only by ISP control s/w, not used by the h/w itself
   // This gets passed to the ISP as the vflip flag - it's the version of the flag that's
...
typedef struct {
	...
	uint32_t tile_addr;
    uint32_t tile_count;
    ISP_FRAME_PARAMS_T frame;
	ISP_TILE_OFFSET_PARAMS_T tile_offsets;
	ISP_LINES_AVAIL_PARAMS_T lines_avail;
	ISP_Y_G_COEFFS_PARAMS_T y_g_coeffs;
	ISP_RAW_INPUT_PARAMS_T raw_input;
	ISP_YUV_INPUT_PARAMS_T yuv_input;
	ISP_CONVERSION_PARAMS_T yuv_conversion;
	ISP_HRESIZE_PARAMS_T hresize;
	ISP_CONVERSION_PARAMS_T conversion;
	ISP_LRESIZE_PARAMS_T lresize;
	ISP_OUTPUT_PARAMS_T lo_res_output;
	ISP_OUTPUT_PARAMS_T hi_res_output;
} isp_job_post_t;
