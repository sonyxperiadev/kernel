...
*******************************************************************************/
#define pr_fmt(fmt) "<%s> %s:" fmt "\n",mm_dvfs->mm_common->single_wq_name,__func__
...
int mm_dvfs_notification_handler(struct notifier_block* block,unsigned long param, void* data)
{
	...
	mm_dvfs_t *mm_dvfs = container_of(block, mm_dvfs_t, mm_fmwk_notifier_blk);
...
static void dvfs_start_timer(mm_dvfs_t *mm_dvfs)
{
	...
	if(mm_dvfs->current_mode == NORMAL)
		mod_timer(&mm_dvfs->dvfs_timeout, jiffies+msecs_to_jiffies(mm_dvfs->dvfs.T1));
	if(mm_dvfs->current_mode == TURBO)
		mod_timer(&mm_dvfs->dvfs_timeout, jiffies+msecs_to_jiffies(mm_dvfs->dvfs.T2));
}
...
static void dvfs_work(struct work_struct* work)
{
	...
	if(mm_dvfs->suspend_requested)	{
		...
		if(mm_dvfs->timer_state) del_timer_sync(&mm_dvfs->dvfs_timeout);
	...
	if( (mm_dvfs->dvfs.is_dvfs_on == false) && 
		...
		mm_dvfs->requested_mode = mm_dvfs->dvfs.user_requested_mode;
		pi_mgr_dfs_request_update(&(mm_dvfs->dev_dfs_node), mm_dvfs->requested_mode);
		return;
		}
	...
	if(mm_dvfs->timer_state == false) {
		...
		setup_timer(&(mm_dvfs->dvfs_timeout), dvfs_timeout_callback, (unsigned long)mm_dvfs);
...
//	percnt = percnt/temp;
	...
	pr_debug("dvfs_hw_on_dur %lld dvfs timeslot %d percnt %d", mm_dvfs->hw_on_dur, temp, percnt);
	 ...
	 if(!update->is_read) {
	 	switch(update->type){
			case MM_DVFS_UPDATE_ENABLE:
				if(param > 1) pr_err("Enter 0/1 ");
				else mm_dvfs->dvfs.is_dvfs_on = param;
				if(mm_dvfs->dvfs.is_dvfs_on) {
					mm_dvfs->mm_fmwk_notifier_blk.notifier_call = mm_dvfs_notification_handler;
					atomic_notifier_chain_register(&mm_dvfs->mm_common->notifier_head, &mm_dvfs->mm_fmwk_notifier_blk);
					}
				else{
			...
			case MM_DVFS_UPDATE_SUSPEND:
				if(param > 1) pr_err("Enter 0/1 ");
				else mm_dvfs->dvfs.enable_suspend_resume= param;
				break;
			case MM_DVFS_UPDATE_REQ_MODE:
				if(param > 2) pr_err("Enter 0/1/2 ");
				else mm_dvfs->dvfs.user_requested_mode = param;
				break;
			case MM_DVFS_UPDATE_T1:
				max = mm_dvfs->dvfs.T2/4;
				if(param > max) pr_err("Enter value <= %u ",max);
				else mm_dvfs->dvfs.T1 = param;
				break;
			case MM_DVFS_UPDATE_P1:
				max =100;
				min = (mm_dvfs->dvfs.P2*TURBO_RATE)/NORMAL_RATE;
				if( (param > max) || (param < min)) pr_err("Enter value between %u and %u ",min,max);
				else mm_dvfs->dvfs.P1 = param;
				break;
			case MM_DVFS_UPDATE_T2:
				min = mm_dvfs->dvfs.T1*4;
				if(param < min) pr_err("Enter value >= %u ",min);
				else mm_dvfs->dvfs.T2 = param;
				break;
			case MM_DVFS_UPDATE_P2:
				min =100*NORMAL_RATE/TURBO_RATE;
				max = (mm_dvfs->dvfs.P1*NORMAL_RATE)/TURBO_RATE;
				if( (param > max) || (param < min)) pr_err("Enter value between %u and %u ",min,max);
				else mm_dvfs->dvfs.P2 = param;
				break;
...
void* mm_dvfs_init(mm_common_t* mm_common, const char *dev_name, MM_DVFS_HW_IFC *dvfs_params)
{
	...
	if(mm_dvfs->dvfs.is_dvfs_on) {
		mm_dvfs->mm_fmwk_notifier_blk.notifier_call = mm_dvfs_notification_handler;
		atomic_notifier_chain_register(&mm_common->notifier_head, &mm_dvfs->mm_fmwk_notifier_blk);
		}

	mm_dvfs->dvfs_dir = debugfs_create_dir("dvfs", mm_dvfs->mm_common->debugfs_dir);
