...
void printispregs2(struct isp_device_t *isp)
{
...
int isp2_start(struct isp_device_t *isp)
{
	int ret = 0;
	uint32_t ctrl;
	ctrl = isp_read(ISP_CTRL_OFFSET);
	...
	case MM_JOB_STATUS_READY:
		{
			...
			ret = isp2_program(isp, job_params);
			if (ret != 0) {
				pr_err("isp2_program failed with %d", ret);
			...
			ret = isp2_start(isp);
			if (ret != 0) {
				pr_err("isp2_start failed with %d", ret);
				job->status = MM_JOB_STATUS_ERROR;
				return MM_JOB_STATUS_ERROR;
			}
...
int __init mm_isp2_init(void)
{
	...
	dvfs_param.user_requested_mode = PI_OPP_TURBO;
...
module_init(mm_isp2_init);
module_exit(mm_isp2_exit);
