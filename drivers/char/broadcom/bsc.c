   ...
   switch (cmd) {
      case BSC_IOCTL_REGISTER:
      {
         ...
         if (copy_from_user(&param, (void *)arg, sizeof(param)) != 0) {
            printk(KERN_ERR "BSC: copy_from_user failed for "
         ...
         if (copy_from_user(param.data, (void *)usr_data, param.len) != 0) {
            printk(KERN_ERR "BSC: copy_from_user failed for "
      ...
      case BSC_IOCTL_UNREGISTER:
      {
         ...
         if (strncpy_from_user(name, (void *)arg, BSC_NAME_LEN) < 0) {
            printk(KERN_ERR "BSC: strncpy_from_user failed for "
      ...
      case BSC_IOCTL_QUERY:
      {
         ...
         if (copy_to_user(usr_data, (void *)param.data, param.len) != 0) {
		...
		unsigned long count, void *data)
{
   ...
   if (param.data == NULL) {
      printk(KERN_ERR "Unable to allocate memory for data of size %d bytes\n",
            param.len);
      return count;
   }
   rc = bsc_register(param.name, param.data, param.len);
		...
		int *eof, void *data)
{
   ...
   len += sprintf(buffer + len, "List of current Board Specific Configurations:\n");
...
static int proc_init(void)
{
   ...
   proc_register = create_proc_entry(PROC_ENTRY_REGISTER, 0644, gProc.parent_dir);
   ...
   proc_unregister = create_proc_entry(PROC_ENTRY_UNREGISTER, 0644, gProc.parent_dir);
   ...
   proc_query = create_proc_entry(PROC_ENTRY_QUERY, 0644, gProc.parent_dir);
   if (proc_query == NULL) {
      rc = -ENOMEM;
      goto proc_del_unreg;
   }
