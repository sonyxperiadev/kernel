...
typedef struct
{
   ...
   ICP_STATS stats; /* Read/write by kernel, read only by user via proc entry */
}
ICP_STATE;
...
static void icp_create_proc(void)
{
   ...
   entry = create_proc_entry("hostUsage0", 0, NULL);
   if (num_online_cpus() != 1)
   {
      entry = create_proc_entry("hostUsage1", 0, NULL);
   }
}
...
static void ipc_load( int cpu )
{
   ...
   //KNLLOG("loops_this_slice=%u load_this_slice=%u lt->loadPeriod=%u lt->timeSlices=%u lt->loadPercentage=%u lt->load=%u", 
   //      loops_this_slice, load_this_slice, lt->loadPeriod, lt->timeSlices, lt->loadPercentage, lt->load);
   ...
   if ( write )
   {
      ...
      if (!lt->timerStarted && lt->enable)
      {
         if (lt->useThread)
         {
            ...
            lt->threadPid = kernel_thread(ipc_load_test_thread, &o->cpu_enum, 0);
            sched_setaffinity( lt->threadPid, &cpu_mask );
            cpumask_clear_cpu(cpu, &cpu_mask);
         }
         if (lt->loadPeriod > MAX_LOAD_PERIOD)
            lt->loadPeriod = MAX_LOAD_PERIOD;
         lt->timer.expires = jiffies + msecs_to_jiffies(lt->loadPeriod);
         add_timer(&lt->timer);
         lt->timerStarted = 1;
      }
      else if (lt->timerStarted && !lt->enable)
      {
         ...
         if (lt->useThread)
         {
            if (lt->threadPid >= 0)
            {
               kill_proc_info(SIGTERM, SEND_SIG_PRIV, lt->threadPid);
               wait_for_completion(&lt->threadCompletion);
            }
            lt->threadPid = -1;
         }
      }
...
static int __init host_cpu_usage_init(void)
{
   ...
   for (i=0; i<num_online_cpus(); i++)
   {
      ...
      {
         o->task = kthread_run( hostIcpThread, &o->cpu_enum, "hosticp/%d", i );
         if ( IS_ERR( o->task ))
         {
      ...
      printk("called host_cpu_usage_init for CPU%d\n", i);
   }
   return 0;
}
...
int __exit host_cpu_usage_exit(void)
{
   ...
   icp_remove_proc();
   kthread_stop(obj[0].task);
   kthread_stop(obj[1].task);
...
int hostIcpThread(void *data)
{
   ...
   printk("*************Starting host ICP thread for CPU%d **************   \n", cpu);
   while( 1 )
   {
      set_current_state(  TASK_INTERRUPTIBLE );
      schedule_timeout( ( (HZ * statep->updateIntervalMsec) / 1000 ) + 1 );
      ...
      }
      {
         ...
         memcpy(&temp[0], &statsp->countHistory[0], HISTORY_SIZE*sizeof(unsigned int) );
         memcpy(&statsp->countHistory[1], &temp[0], (HISTORY_SIZE-1)*sizeof(unsigned int));
         statsp->countHistory[0]=statsp->currCount;
      }
      get_idle_profile( &datap->idle_handle, &datap->idle, &datap->total );
      ...
      {
         statsp->currCount =  (1000 * datap->idle)/(datap->total);
      }
      else
      {
         statsp->currCount =  (datap->idle)/(datap->total / 1000);
      }
      ...
      statsp->avgCount = ((statsp->currCount*statep->alphaFactor) \
                     +(statsp->avgCount * ((1 << 8) - statep->alphaFactor))) >> 8;
      ...
      if ((statep->resetTimer > statep->resetInterval) && (statep->resetInterval))
      {
