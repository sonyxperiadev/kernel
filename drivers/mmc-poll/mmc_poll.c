...
mmc_write_blocks(struct mmc *mmc, ulong start, lbaint_t blkcnt, const void *src)
{
	...
	for (i = 0; i < blkcnt; i++) {
		 ...
		 *
		 * Looking at the interrupt status register I always read
		 * 0x18000 - BIT 15 - 1 and BIT 16 -1 meaning Command Time out
		 * error. After banging my head to root cause it I settled for
		 * the tested and proven hack of driver developers "DELAY".
		 */
		mdelay(100);
		debug("wrote block %d \r", i);
	}
	debug("\r\n");
...
int mmc_poll_send_op_cond(struct mmc *mmc)
{
	int timeout = 1000;
	struct mmc_cmd cmd;
	int err;
...
void mmc_poll_set_clock(struct mmc *mmc, uint clock)
{
	if (clock > mmc->f_max)
		clock = mmc->f_max;
...
void mmc_poll_set_bus_width(struct mmc *mmc, uint width)
{
	mmc->bus_width = width;
...
int mmc_startup(struct mmc *mmc)
{
	...
	if (IS_SD(mmc)) {
		...
		if (mmc->card_caps & MMC_MODE_HS)
			mmc_poll_set_clock(mmc, 50000000);
		else
			mmc_poll_set_clock(mmc, 25000000);
	} else {
		if (mmc->card_caps & MMC_MODE_4BIT) {
			...
			mmc_poll_set_bus_width(mmc, 8);
		}

		if (mmc->card_caps & MMC_MODE_HS) {
			if (mmc->card_caps & MMC_MODE_HS_52MHz)
				mmc_poll_set_clock(mmc, 52000000);
			else
				mmc_poll_set_clock(mmc, 26000000);
		} else
			mmc_poll_set_clock(mmc, 20000000);
	}
...
int mmc_poll_send_if_cond(struct mmc *mmc)
{
	struct mmc_cmd cmd;
	int err;
...
int mmc_init(struct mmc *mmc)
{
	...
	mmc_poll_set_bus_width(mmc, 1);
	mmc_poll_set_clock(mmc, 1);

	/* Reset the Card */
	err = mmc_poll_go_idle(mmc);
	...
	debug("sd_send_op_cond returned  %d \r\n", err);
